...
def main(args):
    ...
    with open("host/cmake/debian-pv/changelog") as cl:
        ...
        if args.nightly:
            ...
            uhd_version = "4.8.0.16" #uhd_version[1]
        else:
    ...
    if not args.nobuild:
        ...
        result = subprocess.run(shlex.split(
            "sudo pbuilder build --buildresult ./result uhdpv_{}-0ubuntu1~{}1.dsc; sudo pbuilder build --buildresult ./result uhdpv_{}-0ubuntu1~{}1.dsc".format(uhd_version, args.release, uhd_version, args.release)), cwd=args.buildpath)
        if result.returncode:
