...
int UHD_SAFE_MAIN(int argc, char *argv[]){
    ...
    if (vm.count("lo-freq") && vm.count("dsp-freq")) { //with default of 0.0 this will always be true
        for(size_t n = 0; n < channel_nums.size(); n++) {
            ...
            std::cout << std::format("Setting ch{} RX Freq: {} MHz...", channel, (freq/1e6));
    ...
    if (vm.count("bw")) {
        for(size_t n = 0; n < channel_nums.size(); n++) {
            size_t channel = channel_nums[n];
            std::cout << std::format("Setting ch{} RX Bandwidth: {} MHz...\n", channel, (bw/1e6));

            usrp->set_rx_bandwidth(bw, channel);
            std::cout << std::format("Actual ch{} RX Bandwidth: {} MHz...\n\n", channel, (usrp->get_rx_bandwidth(channel)/1e6));
        }
    }
