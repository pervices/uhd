...
int UHD_SAFE_MAIN(int argc, char* argv[])
{
    ...
    desc.add_options()
        ...
        ("rx_cpu", po::value<std::string>(&rx_cpu)->default_value("sc16"), "specify the host/cpu sample mode for RX")
        ("tx_cpu", po::value<std::string>(&tx_cpu)->default_value("sc16"), "specify the host/cpu sample mode for TX")
    ...
    std::cout << std::endl
              << boost::format("Benchmark rate summary:\n"
                               ...
                               "  Num timeouts (Tx):        %u\n" //-- need to check
                               "  Num timeouts (Rx):        %u\n" //-- need to check
                               "  Expected duration (Tx):   %.3f\n"
                               "  Actual duration (Tx):     %.3f\n"
                               "  Expected duration (Rx):   %.3f\n"
                               "  Actual duration (Rx):     %.3f\n")
