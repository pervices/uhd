...
int UHD_SAFE_MAIN(int argc, char* argv[])
{
    ...
    if (vm.count("help")) {
        std::cout << std::format("UHD RX_TX continuous stream %s") % desc << std::endl;
        return ~0;
    }
    ...
    std::cout << std::format("Actual TX Rate: %f Msps...") % (actual_tx_rate / 1e6)
              << std::endl
              << std::endl;
    ...
    for(size_t ch = 0; ch < tx_channel_nums.size(); ch++){
        std::cout << std::format("Setting ch%i TX Freq: %f MHz...") % tx_channel_nums[ch] % (tx_freqs[ch]/1e6) << std::endl;
        uhd::tune_request_t tx_tune_request(tx_freqs[ch]);
        usrp->set_tx_freq(tx_tune_request, tx_channel_nums[ch]);
        std::cout << std::format("Actual ch%i TX Freq: %f MHz...") % tx_channel_nums[ch] % (usrp->get_tx_freq(tx_channel_nums[ch])/1e6) << std::endl << std::endl;

        std::cout << std::format("Setting ch%i RX Freq: %f MHz...") % rx_channel_nums[ch] % (rx_freqs[ch]/1e6) << std::endl;
        uhd::tune_request_t rx_tune_request(rx_freqs[ch]);
        usrp->set_rx_freq(rx_tune_request, rx_channel_nums[ch]);
        std::cout << std::format("Actual ch%i RX Freq: %f MHz...") % rx_channel_nums[ch] % (usrp->get_rx_freq(rx_channel_nums[ch])/1e6) << std::endl << std::endl;
    }
    ...
    for(size_t ch = 0; ch < rx_channel_nums.size(); ch++){
        //set the Rx rf gain
        std::cout << std::format("Setting ch%i RX Gain: %f dB...") % rx_channel_nums[ch] % rx_gains[ch] << std::endl;
        usrp->set_rx_gain(rx_gains[ch], rx_channel_nums[ch]);
        std::cout << std::format("Actual ch%i RX Gain: %f dB...") % rx_channel_nums[ch] % usrp->get_rx_gain(rx_channel_nums[ch]) << std::endl << std::endl;
            
        //set the Tx rf gain
        std::cout << std::format("Setting ch%i TX Gain: %f dB...") % tx_channel_nums[ch] % tx_gains[ch] << std::endl;
        usrp->set_tx_gain(tx_gains[ch], tx_channel_nums[ch]);
        std::cout << std::format("Actual ch%i TX Gain: %f dB...") % tx_channel_nums[ch] % usrp->get_tx_gain(tx_channel_nums[ch]) << std::endl << std::endl;
    }
