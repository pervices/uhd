...
int UHD_SAFE_MAIN(int argc, char* argv[])
{
    ...
    if (vm.count("help")) {
        std::cout << std::format("UHD TX samples from file %s") % desc << std::endl;
        return ~0;
    }
    ...
    if(manual_lo) {
        std::cout << std::format("Setting TX Freq %f using LO Offset: %f MHz...") % (freq / 1e6) % (lo_offset / 1e6) << std::endl;
    ...
    for (std::size_t channel : channel_nums) {
        ...
        if(manual_lo) {
            std::cout << std::format("Actual TX LO: %f MHz...") % (tune_result.actual_rf_freq / 1e6)
            << std::endl;
        }
        std::cout << std::format("Actual TX Freq: %f MHz...") % ((tune_result.actual_rf_freq + tune_result.actual_dsp_freq) / 1e6)
        << std::endl
        << std::endl;
    }
    ...
    if (vm.count("power")) {
        ...
        for (std::size_t channel : channel_nums) {
            std::cout << std::format("Setting TX Gain: %f dB...") % gain << std::endl;
            usrp->set_tx_gain(gain, channel);
            std::cout << std::format("Actual TX Gain: %f dB...")
    ...
    if (vm.count("bw")) {
        std::cout << std::format("Setting TX Bandwidth: %f MHz...") % (bw / 1e6)
        << std::endl;
        for (std::size_t channel : channel_nums) {
            usrp->set_tx_bandwidth(bw, channel);
            std::cout << std::format("Actual TX Bandwidth: %f MHz...")
    ...
    for (std::size_t channel : channel_nums) {
        ...
        std::cout << std::format("Checking TX: %s ...") % lo_locked.to_pp_string()
        << std::endl;
    ...
    std::cout << std::format("Checking TX: %s ...") % mimo_locked.to_pp_string()
    << std::endl;
    UHD_ASSERT_THROW(mimo_locked.to_bool());
        ...
        std::cout << std::format("Checking TX: %s ...") % ref_locked.to_pp_string()
        << std::endl;
