...
int UHD_SAFE_MAIN(int argc, char *argv[]){
    ...
    if (vm.count("help")){
        std::cout << std::format("UHD TX Ramp Test {}", desc) << std::endl;
        return ~0;
    }
    ...
    for(size_t ch = 0; ch < channel_nums.size(); ch++) {
        std::cout << std::format("Setting ch{} TX Rate: {} Msps...", channel_nums[ch], (rate/1e6)) << std::endl;
        ...
        std::cout << std::format("Actual ch{} TX Rate: {} Msps...", channel_nums[ch], (rate/1e6)) << std::endl << std::endl;
    }
