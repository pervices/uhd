...
int UHD_SAFE_MAIN(int argc, char *argv[]){
    ...
    if(cpu_format == "sc16") {
        send_loop<int16_t>(tx_stream, rate, spb, first, last, increment, total_num_samps, wave_freq, wave_type_s, ampl * SHRT_MAX);
    } else if (cpu_format == "fc32") {
        send_loop<float>(tx_stream, rate, spb, first, last, increment, total_num_samps, wave_freq, wave_type_s, ampl);
    }
