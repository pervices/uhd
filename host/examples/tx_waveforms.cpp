...
int UHD_SAFE_MAIN(int argc, char *argv[]){
    ...
    std::cout << std::format("Actual TX Rate: %f Msps...") % (actual_rate/1e6) << std::endl << std::endl;

    for(size_t ch = 0; ch < channel_nums.size(); ch++) {
        std::cout << std::format("Setting TX Freq: %f MHz...") % (freq/1e6) << std::endl;
        uhd::tune_request_t tune_request;
        if(vm.count("lo-offset")) {
            std::cout << std::format("Setting TX LO Offset: %f MHz...") % (lo_offset / 1e6) << std::endl;
    ...
    for (std::size_t channel : channel_nums) {
        ...
        if (vm.count("bw")) {
            std::cout << std::format("Setting TX Bandwidth: %f MHz...") % bw
                      << std::endl;
