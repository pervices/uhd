...
int main() {
	...
	uhd::fs_path tx_fe_path = "/mboards/0/dboards/A/tx_frontends/A";
	...
	actual_dac_nco_freq = tree->access<double>( tx_fe_path / "dac" / "nco" ).get();
	actual_dsp_nco_freq = tree->access<double>( tx_dsp_path / "nco" ).get();

	if ( actual_lo_freq != expected_lo_freq ) {
		std::cout << "Actual LO: " << actual_lo_freq << ", Expected LO: " << expected_lo_freq << std::endl;
	}
	if ( actual_dac_nco_freq != expected_dac_nco_freq ) {
		std::cout << "Actual DAC NCO: " << actual_dac_nco_freq << ", Expected DAC NCO: " << expected_dac_nco_freq << std::endl;
	}
	if ( actual_dsp_nco_freq != expected_dsp_nco_freq ) {
		std::cout << "Actual DSP NCO: " << actual_dac_nco_freq << ", Expected DSP NCO: " << expected_dsp_nco_freq << std::endl;
	}
