...
int UHD_SAFE_MAIN(int argc, char* argv[])
{
    ...
    // std::cout << std::format("Using Device: {}", usrp->get_pp_string()) <<
    // std::endl;
    ...
    std::cout << std::format("Actual RX Rate: {} Msps...", (actual_rx_rate / 1e6))
              << std::endl;

    double rx_time = nsamps / actual_rx_rate;
    if (from_eob) {
        std::cout << std::format("Will add {} seconds to timespec for RX samples...",
                         rx_time)
                  << std::endl;
    }
    ...
    for (size_t nrun = 0; nrun < nruns; nrun++) {
        ...
        if (verbose) {
            std::cout << std::format(
                             "Run {}: Got packet: {} samples, {} full secs, {} frac secs",
        ...
        if (verbose) {
            std::cout << std::format("Sent {} samples", num_tx_samps) << std::endl;
        }
