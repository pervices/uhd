...
public:
        ...
        _wave_table(wave_table_len)
    {
        ...
        std::vector<uint16_t> real_wave_table(wave_table_len);
        if (wave_type == "CONST"){
        ...
        for (size_t i = 0; i < wave_table_len; i++){
            ...
            _wave_table[i] = std::complex<uint16_t>(
    ...
    inline uint32_t operator()(const size_t index) const{
        return ((_wave_table[index % wave_table_len].real() << 16) | _wave_table[index % wave_table_len].imag() << 0);
    }

private:
    std::vector<std::complex<uint16_t> > _wave_table;
};
