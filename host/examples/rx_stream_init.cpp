...
template<typename samp_type> void recv_to_file(
    ...
    if (md.error_code == uhd::rx_metadata_t::ERROR_CODE_TIMEOUT) {
            std::cerr << std::format("First packet timed out. It is likely that no data will be received") << std::endl;
        }
...
int run_exec(std::string argument) {
    ...
    if(child_pid == 0) {
        execvp(args[0], args);
        std::cerr << std::format("Failed to launch: %s" ) % args[0] << std::endl;
        int n = 0;
        while(args[n]!=NULL) {
...
int UHD_SAFE_MAIN(int argc, char *argv[]){
    ...
    if (vm.count("lo-freq") && vm.count("dsp-freq")) { //with default of 0.0 this will always be true
        ...
        std::cout << std::format("Actual RX Freq: %f MHz...") % (usrp->get_rx_freq(channel)/1e6) << std::endl << std::endl;
    } else {
    ...
    if (vm.count("gain")) {
        ...
        std::cout << std::format("Actual RX Gain: %f dB...") % usrp->get_rx_gain(channel) << std::endl << std::endl;
    }

    //set the IF filter bandwidth
    if (vm.count("bw")) {
        std::cout << std::format("Setting RX Bandwidth: %f MHz...") % (bw/1e6) << std::endl;
        usrp->set_rx_bandwidth(bw, channel);
        std::cout << std::format("Actual RX Bandwidth: %f MHz...") % (usrp->get_rx_bandwidth(channel)/1e6) << std::endl << std::endl;
    }
