...
int UHD_SAFE_MAIN(int argc, char* argv[])
{
    ...
    std::cout << std::format(" Difference between paired reads: %f us")
    ...
    std::cout << std::format(" Span      : %f us\n"
              ...
              << std::endl;
    std::cout << std::format(" Difference of response time 1: %f us")
                     % ((response_time1 - cmd_time1).get_real_secs() * 1e6)
              << std::endl;
    std::cout << std::format(" Difference of response time 2: %f us")
    ...
    if (md.error_code != uhd::rx_metadata_t::ERROR_CODE_NONE) {
        throw std::runtime_error(str(std::format("Receiver error %s") % md.strerror()));
    }
    std::cout << std::format(" Received packet: %u samples, %u full secs, %f frac secs")
                     % num_rx_samps % md.time_spec.get_full_secs()
                     % md.time_spec.get_frac_secs()
              << std::endl;
    std::cout << std::format(" Stream time was: %u full secs, %f frac secs")
                     % stream_time.get_full_secs() % stream_time.get_frac_secs()
              << std::endl;
    std::cout << std::format(" Difference between stream time and first packet: %f us")
