...
int UHD_SAFE_MAIN(int argc, char* argv[])
{
    ...
    std::cout << std::endl;
    std::cout << std::format("Creating the transmit usrp device with: %s...") % tx_args
              << std::endl;
    uhd::usrp::multi_usrp::sptr tx_usrp = uhd::usrp::multi_usrp::make(tx_args);
    std::cout << std::endl;
    std::cout << std::format("Creating the receive usrp device with: %s...") % rx_args
              << std::endl;
    ...
    for (size_t ch = 0; ch < tx_channel_nums.size(); ch++) {
        ...
        // set the rf gain
        if (vm.count("tx-gain")) {
            std::cout << std::format("Setting TX Gain: %f dB...") % tx_gain
                      << std::endl;
        ...
        if (vm.count("tx-bw")) {
            std::cout << std::format("Setting TX Bandwidth: %f MHz...") % tx_bw
                      << std::endl;
    ...
    for (size_t ch = 0; ch < rx_channel_nums.size(); ch++) {
        ...
        if (vm.count("rx-gain")) {
            std::cout << std::format("Setting RX Gain: %f dB...") % rx_gain
                      << std::endl;
        ...
        if (vm.count("rx-bw")) {
            std::cout << std::format("Setting RX Bandwidth: %f MHz...") % (rx_bw / 1e6)
                      << std::endl;
    ...
    if ((ref == "mimo")
        ...
        std::cout << std::format("Checking RX: %s ...") % mimo_locked.to_pp_string()
                  << std::endl;
        UHD_ASSERT_THROW(mimo_locked.to_bool());
    }
    if ((ref == "external")
        ...
        std::cout << std::format("Checking RX: %s ...") % ref_locked.to_pp_string()
                  << std::endl;
        UHD_ASSERT_THROW(ref_locked.to_bool());
    }
