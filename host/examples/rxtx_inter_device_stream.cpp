...
int UHD_SAFE_MAIN(int argc, char *argv[]){
    ...
    if(!use_a && !use_b) {
        ...
        if(use_a) {
            std::cout << std::format("Creating the usrp device with: %s...") % a_args << std::endl;
        ...
        if(loopback_mode) {
            ...
            std::cout << std::format("Creating the usrp device with: %s...") % b_args << std::endl;
    ...
    if(use_a) {
        if(a_rx_channel_nums.size() != 0) {
            for(size_t n = 0; n < a_rx_channel_nums.size(); n++) {
                std::cout << std::format("Setting ch %lu A RX Rate: %f Msps...") % n % (ab_rates[n]/1e6) << std::endl;
                ...
                std::cout << std::format("Actual ch %lu A RX Rate: %f Msps...") % n % (actual_ab_rates[n]/1e6) << std::endl << std::endl;
            }
        }
        if(a_tx_channel_nums.size() != 0 && !loopback_mode) {
            for(size_t n = 0; n < a_tx_channel_nums.size(); n++) {
                std::cout << std::format("Setting ch %lu A TX Rate: %f Msps...") % n % (ba_rates[n]/1e6) << std::endl;
                ...
                std::cout << std::format("Actual ch %lu A TX Rate: %f Msps...") % n % (actual_ba_rates[n]/1e6) << std::endl << std::endl;
            }
        }
    }
    ...
    if(use_b) {
        if(b_rx_channel_nums.size() != 0 && !loopback_mode) {
            for(size_t n = 0; n < b_rx_channel_nums.size(); n++) {
                std::cout << std::format("Setting ch %lu B RX Rate: %f Msps...") % n % (ba_rates[n]/1e6) << std::endl;
                ...
                std::cout << std::format("Actual ch %lu B RX Rate: %f Msps...") % n % (actual_rate/1e6) << std::endl << std::endl;
            }
        }
        if(b_tx_channel_nums.size() != 0) {
            for(size_t n = 0; n < b_tx_channel_nums.size(); n++) {
                std::cout << std::format("Setting ch %lu B TX Rate: %f Msps...") % n % (ab_rates[n]/1e6) << std::endl;
                ...
                std::cout << std::format("Actual ch %lu B TX Rate: %f Msps...") % n % (actual_rate/1e6) << std::endl << std::endl;
            }
        }
    }
    ...
    if(use_a) {
        std::cout << std::format("Using A Device: %s") % a_usrp->get_pp_string() << std::endl;
    }
    if(use_b) {
        std::cout << std::format("Using B Device: %s") % b_usrp->get_pp_string() << std::endl;
    }
