//This test has been copied from 16t with only the number of tx and rx channels changed. It has not been verified if the test works yet. 2021-06-14
...
#include "../lib/usrp/cyan_8t/cyan_8t_fw_common.h"
#include "../lib/usrp/cyan_8t/cyan_8t_impl.hpp"

constexpr size_t NUM_RX_CHANNELS = 0;
constexpr size_t NUM_TX_CHANNELS = 8;
...
public:
    ...
    : fd({-1, -1, -1})
    {
        ...
        properties["fpga/board/flow_control/sfpc_port"] = std::to_string(port[FC2]);
        properties["fpga/board/flow_control/sfpd_port"] = std::to_string(port[FC3]);
        ...
        properties["fpga/link/sfpc/ip_addr"] = "127.0.0.1";
        properties["fpga/link/sfpc/mac_addr"] = "00:50:C2:85:3f:aa";
        properties["fpga/link/sfpc/pay_len"] = "9000";

        properties["fpga/link/sfpd/ip_addr"] = "127.0.0.1";
        properties["fpga/link/sfpd/mac_addr"] = "00:50:C2:85:3f:22";
        properties["fpga/link/sfpd/pay_len"] = "9000";
...
BOOST_AUTO_TEST_CASE(test_set_master_clock_rate){
    auto usrp = get_usrp();
    double good_rate = MASTER_CLOCK_RATE/2;
    //double bad_rate = -42;
...
BOOST_AUTO_TEST_CASE(test_get_master_clock_rate){
    auto usrp = get_usrp();
    double good_rate = DSP_CLOCK_RATE;
    size_t good_mboard = 0;
    //size_t bad_mboard = -1;
...
BOOST_AUTO_TEST_CASE(test_get_master_clock_rate_range){
    ...
    auto range = ranges[0];
    BOOST_CHECK_EQUAL(DSP_CLOCK_RATE, range.start());
    BOOST_CHECK_EQUAL(DSP_CLOCK_RATE, range.stop());
...
BOOST_AUTO_TEST_CASE(test_get_rx_rates){
    ...
    for(size_t chan = 0; chan < NUM_RX_CHANNELS; ++chan) {
        ...
        BOOST_CHECK_EQUAL(1, ranges.size());
        auto range = ranges[0];
        BOOST_CHECK_EQUAL(MASTER_CLOCK_RATE/CLOCK_RATE_STEPS, range.start());
        BOOST_CHECK_EQUAL(MASTER_CLOCK_RATE, range.stop());
...
BOOST_AUTO_TEST_CASE(test_get_tx_num_channels){
    auto usrp = get_usrp();
    BOOST_CHECK_EQUAL(NUM_TX_CHANNELS, usrp->get_tx_num_channels());
}
...
BOOST_AUTO_TEST_CASE(test_get_tx_rates){
    auto usrp = get_usrp();
    for(size_t chan = 0; NUM_TX_CHANNELS != 0 && chan < NUM_TX_CHANNELS; ++chan) {
        auto ranges = usrp->get_tx_rates(chan);
        BOOST_CHECK_EQUAL(1, ranges.size());
        auto range = ranges[0];
        BOOST_CHECK_EQUAL(MASTER_CLOCK_RATE/CLOCK_RATE_STEPS, range.start());
        BOOST_CHECK_EQUAL(double(MASTER_CLOCK_RATE), range.stop());
        BOOST_CHECK_EQUAL(1, range.step());
    }
    // test non-happy paths?
}
