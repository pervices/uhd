...
public:
        ...
        uint32_t addr, uint32_t data, uhd::time_spec_t /*time*/, bool /*ack*/) override
    {
        if (addr >= fft_block_control::REG_RESET_ADDR_V1) {
            throw uhd::assertion_error(
                str(std::format("Trying to write register %08x which is only supported "
                                  "for FFT Block v1")
                    % addr));
            ...
            throw uhd::assertion_error(
                str(std::format("Trying to write to read-only register %08x") % addr));
        ...
        UHD_LOG_TRACE("TEST", str(std::format("poke [%04x] = %08x") % addr % data));
    ...
    void _peek_cb(uint32_t addr, uhd::time_spec_t /*time*/) override
    {
        if (addr >= fft_block_control::REG_RESET_ADDR_V1) {
            throw uhd::assertion_error(
                str(std::format("Trying to read register %08x which is only supported "
                                  "for FFT Block v1")
                    % addr));
        }
        ...
        UHD_LOG_TRACE(
            "TEST", str(std::format("peek [%04x] = %08x") % addr % read_memory[addr]));
    }
    void reset()
    {
        fft_was_reset = true;
