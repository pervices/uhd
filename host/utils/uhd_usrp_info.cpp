...
int UHD_SAFE_MAIN(int argc, char* argv[])
{
	...
	if (hw_info) {
		...
		if (not time_eeprom.contains("UNKNOWN: EEPROM not programmed or does not exist"))
		...
		if (current_serial.compare(next_serial) != 0 && not current_serial.empty()) {
			...
			if (not rx_eeprom.contains("UNKNOWN: EEPROM not programmed or does not exist"))
		...
		if (current_serial.compare(next_serial) != 0 && not current_serial.empty()) {
			...
			if (not tx_eeprom.contains("UNKNOWN: EEPROM not programmed or does not exist"))
