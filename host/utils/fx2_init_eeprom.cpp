...
int UHD_SAFE_MAIN(int argc, char* argv[])
{
    ...
    if (vm.count("help")) {
        std::cout << "USRP EEPROM initialization " << desc << std::endl;
        return EXIT_FAILURE;
    }
    ...
    if (module_found) {
        std::cout << std::format("Found the '{}' module. Unloading it.\n", module);
    ...
    if (vm.count("image")) {
        ...
        if (!vm.count("type")) {
            std::cerr << "ERROR: Image file not specified and type of "
                    "device not given. Cannot use built-in images.\n";
            return EXIT_FAILURE;
        }

        std::cout << std::format("Using built-in image for \"{}\".\n", type);
        ...
        if (vm["type"].as<std::string>() == "usrp1") {
            ...
            std::cerr << std::format("ERROR: Unsupported device type \"{}\" specified "
