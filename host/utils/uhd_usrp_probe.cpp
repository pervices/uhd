...
{
    std::stringstream ss;
    ss << std::format("  _____________________________________________________")
       << std::endl;
    ss << std::format(" /") << std::endl;
...
{
    std::stringstream ss;
    ss << std::format("%s DSP: %s") % type % path.leaf() << std::endl;
    ss << std::endl;
    meta_range_t freq_range = tree->access<meta_range_t>(path / "freq/range").get();
    ss << std::format("Freq range: %.3f to %.3f MHz") % (freq_range.start() / 1e6)
              % (freq_range.stop() / 1e6)
       << std::endl;
    ;
    return ss.str();
...
{
    std::stringstream ss;
    ss << std::format("%s Frontend: %s") % type % path.leaf() << std::endl;

    ss << std::format("Name: %s") % (tree->access<std::string>(path / "name").get())
       << std::endl;
    ss << std::format("Antennas: %s")
              % prop_names_to_pp_string(
                  tree->access<std::vector<std::string>>(path / "antenna/options").get())
       << std::endl;
    if (tree->exists(path / "sensors")) {
        ss << std::format("Sensors: %s")
    ...
    for (const std::string& name : gain_names) {
        ...
        ss << std::format("Gain range %s: %.1f to %.1f step %.1f dB") % name
    ...
    ss << std::format("Connection Type: %s")
              % (tree->access<std::string>(path / "connection").get())
       << std::endl;
    ss << std::format("Uses LO offset: %s")
    ...
    const std::string& type, property_tree::sptr tree, const fs_path& path)
{
    ...
    if (tree->exists(path / "name")) {
        ss << std::format("%s Codec: %s") % type % path.leaf() << std::endl;
...
{
    std::stringstream ss;
    ss << std::format("%s Dboard: %s") % type % name << std::endl;
    ...
    if (tree->exists(path / (prefix + "_eeprom"))) {
        ...
        if (db_eeprom.id != usrp::dboard_id_t::none())
            ss << std::format("ID: %s") % db_eeprom.id.to_pp_string() << std::endl;
        if (not db_eeprom.serial.empty())
            ss << std::format("Serial: %s") % db_eeprom.serial << std::endl;
        ...
        if (type == "TX" and tree->exists(path / "gdb_eeprom")) {
            ...
            if (gdb_eeprom.id != usrp::dboard_id_t::none())
                ss << std::format("ID: %s") % gdb_eeprom.id.to_pp_string() << std::endl;
            if (not gdb_eeprom.serial.empty())
                ss << std::format("Serial: %s") % gdb_eeprom.serial << std::endl;
...
{
    std::stringstream ss;
    ss << std::format("Mboard: %s") % (tree->access<std::string>(path / "name").get())
       << std::endl;
    ...
    if (tree->exists(path / "eeprom")) {
        ...
        for (const std::string& key : mb_eeprom.keys()) {
            if (not mb_eeprom[key].empty())
                ss << std::format("%s: %s") % key % mb_eeprom[key] << std::endl;
        }
    } else {
...
{
    std::stringstream ss;
    ss << std::format("Device: %s") % (tree->access<std::string>("/name").get())
       << std::endl;
...
int UHD_SAFE_MAIN(int argc, char* argv[])
{
    ...
    if (vm.count("help")) {
        std::cout << std::format("UHD USRP Probe %s") % desc << std::endl;
        return EXIT_FAILURE;
    }
    ...
    if (vm.count("range")) {
        ...
        std::cout << std::format("%.1f:%.1f:%.1f") % range.start() % range.step()
                         % range.stop()
                  << std::endl;
        return EXIT_SUCCESS;
    }
