    ...
    const uint16_t vid, const uint16_t pid, const bool user_supplied = false)
{
    ...
    try {
        ...
        if (handles.empty()) {
            if (user_supplied) {
                std::cerr << (std::format("Failed to open device with VID 0x%04x and "
        ...
        if (!handles.empty()) {
            handle = handles[0];
            std::cout << (std::format("Device opened (VID=0x%04x,PID=0x%04x)") % vp.vid
                          % vp.pid)
                             .str()
                      << std::endl;
        }
    ...
    b200_iface::sptr& b200, uint16_t addr, uint16_t offset, size_t num_bytes)
{
    ...
    UHD_LOGGER_DEBUG("B2XX_FX3")
        << "Reading EEPROM at address " << std::format("0x%X") % (int)eeprom_addr
...
int blank_eeprom(b200_iface::sptr& b200, uint16_t addr, uint16_t offset, size_t num_bytes)
{
    ...
    UHD_LOGGER_DEBUG("B2XX_FX3")
        << "Blanking EEPROM at address " << std::format("0x%X") % (int)eeprom_addr
        << " with " << (int)num_bytes << " bytes";
    try {
    ...
    uint16_t offset)
{
    ...
    const uint16_t eeprom_addr = offset | (uint16_t(addr) << 8);
    UHD_LOGGER_DEBUG("B2XX_FX3")
        << "Writing EEPROM at address " << std::format("0x%X") % (int)eeprom_addr
...
int32_t main(int32_t argc, char* argv[])
{
    ...
    try {
        ...
        std::cout << std::format("B2xx Utility Program %s") % visible << std::endl;
        return ~0;
    }
    ...
    if (vm.count("read-eeprom")) {
        ...
        for (int i = 0; i < 8; i++)
            std::cout << i << ": " << std::format("0x%X") % (int)data[i] << std::endl;
