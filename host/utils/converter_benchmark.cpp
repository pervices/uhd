        ...
        for (size_t i = 0; i < buf.size(); i++) {
            if (type == "sc8") {
                ...
                throw uhd::runtime_error(
                    str(std::format("Cannot handle data type: %s") % type));
            }
        }
...
std::string void_ptr_to_hexstring(const void* v_ptr, size_t index)
{
    ...
    return str(std::format("%X") % ptr[index]);
}
...
std::string item_to_hexstring(const void* v_ptr, size_t index, const std::string& type)
{
    if (type == "fc32") {
        ...
        return str(std::format("<unhandled data type: %s>") % type);
    }
}
...
int UHD_SAFE_MAIN(int argc, char* argv[])
{
    ...
    if (vm.count("help")) {
        std::cout << std::format("UHD Converter Benchmark Tool %s") % desc << std::endl
                  << std::endl;
        std::cout
    ...
    if (not debug_mode) {
        ...
        for (priority_type prio_i : conv_list.keys()) {
            double duration = run_benchmark(conv_list[prio_i],
                ...
                n_samples,
                iterations);
            std::cout << std::format("%i,%d,%d,%d,%d") % prio_i % (duration * 1000)
