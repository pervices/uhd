...
private:
    ...
    UHD_INLINE size_t send_multiple_packets(
        ...
        do {
            ...
            if(
                true || ///* Packet is in the future*/ (int64_t)packet_header_infos[packets_sent].tsf >= ( _clock_sync->get_device_time().to_ticks(_TICK_RATE) + (int64_t)(drop_lead * _TICK_RATE) ) ||
