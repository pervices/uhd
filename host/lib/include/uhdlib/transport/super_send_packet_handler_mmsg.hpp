    ...
    UHD_INLINE size_t send(
        ...
        if(actual_samples_sent == 0) {
            ...
            for(size_t ch_i = 0; ch_i < _NUM_CHANNELS; ch_i++) {
                // TODO: verify ch_send_buffer_info_group[ch_i].sample_cache.data() + actual_samples_sent gives the correct value
...
private:
    ...
    UHD_INLINE size_t send_multiple_packets(
        ...
        // TODO: verify this is correct, it may be the source of discontinuities
        if(metadata_.has_time_spec) {
