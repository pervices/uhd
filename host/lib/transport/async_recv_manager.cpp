...
flush_complete((uint8_t*) aligned_alloc(cache_line_size, _num_ch * padded_uint_fast8_t_size))
{
    ...
    int64_t ch_per_thread = 1;//(int64_t) std::ceil( ( MAX_RESOURCE_FRACTION * total_rx_channels) / (double)num_cores );
...
void async_recv_manager::recv_loop(async_recv_manager* const self, const std::vector<int> sockets_, const size_t ch_offset) {
    ...
    while(true) [[likely]] {
        ...
        const int r = recvmmsg(socket, (mmsghdr*) self->access_mmsghdr_buffer(0/*ch*/, ch_offset, 0/*b[ch]*/), packets_to_recv, MSG_DONTWAIT, 0);
