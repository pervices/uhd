...
recv_packet_handler_mmsg:: recv_packet_handler_mmsg(
...
void recv_packet_handler_mmsg::issue_stream_cmd(const stream_cmd_t& stream_cmd)
{
    ...
    for (size_t chan_i = 0; chan_i < _NUM_CHANNELS; chan_i++) {
        ...
        if (!_channel_active[chan_i] and stream_cmd.stream_mode != stream_cmd_t::STREAM_MODE_STOP_CONTINUOUS) {
            ...
            if (stream_cmd.stream_mode != stream_cmd_t::STREAM_MODE_NUM_SAMPS_AND_DONE) {
                ...
                if (r == -1) {
                    ...
                    if (err == EWOULDBLOCK) {
                        throw uhd::runtime_error("Another instance of UHD is currently using channel " + std::to_string(_channels[chan_i]) + ".");
                    } else {
        ...
        // Unlocking only happens here for STOP_CONTINUOUS, NUM_SAMPS_AND_DONE is handled in the recv function when EOB is detected (NUM_SAMPS locking is currently disabled since EOB is not implemented).
...
void recv_packet_handler_mmsg::if_hdr_unpack(const uint32_t* packet_buff, vrt::if_packet_info_t& if_packet_info) {
    ...
    uint_fast16_t has_trailer = (modified_header[0] >> 2) & 1;

    // Set has_tlr (has trailer) flag to false
    ...
    vita_length -= /* Branchless if has_trailer */ has_trailer * (_TRAILER_SIZE / 4);
