...
recv_packet_handler_mmsg:: recv_packet_handler_mmsg(const std::vector<size_t>& channels, const std::vector<int>& recv_sockets, const std::vector<std::string>& dst_ip, const size_t max_sample_bytes_per_packet, const size_t header_size, const size_t trailer_size, const std::string& cpu_format, const std::string& wire_format, bool wire_little_endian, size_t device_total_rx_channels, std::vector<uhd::usrp::stream_cmd_issuer> cmd_issuers, std::vector<int> streamer_locks)
    ...
    _channels(channels),
    _streamer_locks(streamer_locks),
...
void recv_packet_handler_mmsg::issue_stream_cmd(const stream_cmd_t& stream_cmd)
{
    ...
    for (size_t chan_i = 0; chan_i < _NUM_CHANNELS; chan_i++) {
        // Lock channel if it is not already marked as active and not STOP_CONTINUOUS
        if (!_channel_active[chan_i] and stream_cmd.stream_mode != stream_cmd_t::STREAM_MODE_STOP_CONTINUOUS) {
            // TODO: Remove this if statement once EOB flag is properly implemented in packets
            // Since EOB is not currently implemented, do not lock for NUM_SAMPS_AND_DONE since we won't know when to unlock
            // I'm putting this in a separate if statement from above to make it clear this is to be removed once EOB is implemented
            if (stream_cmd.stream_mode != stream_cmd_t::STREAM_MODE_NUM_SAMPS_AND_DONE) {
                ...
                int r = flock(_streamer_locks[_channels[chan_i]], LOCK_EX | LOCK_NB);
                if (r == -1) {
                    int err = errno;
                    // EWOULDBLOCK is expected if there is already a lock since we ran with the LOCK_NB flag.
                    if (err == EWOULDBLOCK) {
                        throw uhd::runtime_error("Attempted to lock channel but it was already locked. Does another UHD instance already have a lock?");
                    } else {
                        throw uhd::runtime_error("flock failed to lock channel with error: " + std::string(strerror(err)));
                    }
                }
            }
        }
        _stream_cmd_issuers[chan_i].issue_stream_command(stream_cmd);
        // Stream ends with stop continuous or when nsamps and done is finished.
        // Unlocking only happens here for stop continuous, nsamps and done is handled elsewhere
        if (stream_cmd.stream_mode == stream_cmd_t::STREAM_MODE_STOP_CONTINUOUS) {
            _channel_active[chan_i] = false;
            // Release channel locks
            flock(_streamer_locks[_channels[chan_i]], LOCK_UN);
        }
    }
}
...
recv_packet_streamer_mmsg::recv_packet_streamer_mmsg(const std::vector<size_t>& channels, const std::vector<int>& recv_sockets, const std::vector<std::string>& dst_ip, const size_t max_sample_bytes_per_packet, const size_t header_size, const size_t trailer_size, const std::string& cpu_format, const std::string& wire_format, bool wire_little_endian, size_t device_total_rx_channels, std::vector<uhd::usrp::stream_cmd_issuer> cmd_issuers, std::vector<int> streamer_locks)
    : recv_packet_handler_mmsg(channels, recv_sockets, dst_ip, max_sample_bytes_per_packet, header_size, trailer_size, cpu_format, wire_format, wire_little_endian, device_total_rx_channels, cmd_issuers, streamer_locks)
{
}
