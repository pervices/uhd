...
void send_packet_handler_mmsg::lock_streamer_channel(size_t channel_num) {
    ...
    int r = flock(_streamer_locks[channel_num], LOCK_EX | LOCK_NB);
    if (r == -1) {
        int err = errno;
...
void send_packet_handler_mmsg::send_eob_packet(const uhd::tx_metadata_t &metadata, double timeout) {
    ...
    for (size_t n = 0; n < _channels.size(); n++) {
        flock(_streamer_locks[n], LOCK_UN);
    }
}
