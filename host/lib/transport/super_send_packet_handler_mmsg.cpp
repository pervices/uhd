    ...
    UHD_INLINE size_t send_multiple_packets(
        ...
        for(int n = 0; n < num_packets; n++) {
            ...
            if(is_first_packet) {
                ...
                if(metadata_.has_time_spec) {
                    // Sets the timestamp based on what's specified by the user
                    packet_header_infos[n].tsf = (metadata_.time_spec + time_spec_t::from_ticks(n * _max_samples_per_packet, _sample_rate)).to_ticks(_TICK_RATE) - 15000;
                } else {
                    // Sets the timestamp to follow from the previous send
                    packet_header_infos[n].tsf = (next_send_time + time_spec_t::from_ticks(n * _max_samples_per_packet, _sample_rate)).to_ticks(_TICK_RATE) - 15000;
                }
            }
