...
void user_recv_manager::uring_init(size_t ch) {
    ...
    if(error) {
        UHD_LOG_ERROR("USER_RECV_MANAGER", "Error when initializing io_uring: " + std::string(strerror(-error)));
    ...
    *buffer_ring = io_uring_setup_buf_ring(ring, PACKET_BUFFER_SIZE, _bgid_storage[ch], 0, &ret);
    ...
    if(ret) {
        UHD_LOG_ERROR("USER_RECV_MANAGER", "Error when setting up io_uring: " + std::string(strerror(-error)));
...
void user_recv_manager::arm_recv_multishot(size_t ch, int fd) {
    struct io_uring* ring = access_io_urings(ch, 0);
...
void* user_recv_manager::allocate_hugetlb_buffer_with_fallback(size_t size) {
...
void* user_recv_manager::allocate_buffer(size_t size) {
    ...
    if(buffer != MAP_FAILED) {
        ...
        UHD_LOG_ERROR("USER_RECV_MANAGER", "Failed to allocate buffer of size " + std::to_string(size) + "bytes. Error code: " + std::string(strerror(errno)));
