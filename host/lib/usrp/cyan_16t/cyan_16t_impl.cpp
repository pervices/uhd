...
#define TREE_CREATE_RW(PATH, PROP, TYPE, HANDLER)						\
	do { _tree->create<TYPE> (PATH)								\
		...
		.add_desired_subscriber(std::bind(&cyan_16t_impl::set_ ## HANDLER, this, (PROP), ph::_1))	\
		.set_publisher(std::bind(&cyan_16t_impl::get_ ## HANDLER, this, (PROP)    ));	\
	} while(0)
...
#define TREE_CREATE_RO(PATH, PROP, TYPE, HANDLER)						\
	do { _tree->create<TYPE> (PATH)								\
    		.set( get_ ## HANDLER (PROP))							\
		.publish  (std::bind(&cyan_16t_impl::get_ ## HANDLER, this, (PROP)    ));	\
	} while(0)
    ...
    _command_time()
{
    ...
    // FOR EACH INTERFACE
    for (int j = 0; j < link_cyan_16t::num_links; j++) {
        // CHECK PING
        sprintf(cmd,"ping -c 1 -W 1 %s  > /dev/null 2>&1",link_cyan_16t::addrs[j]); 
        check = system(cmd);
        if (check!=0){
            UHD_LOG_WARNING("UHD", "Ping failed for " << link_cyan_16t::addrs[j] << ", please check " << link_cyan_16t::names[j]);
        }
        sprintf(cmd,"ip addr show | grep -B2 %s | grep -E -o \"mtu.{0,5}\" 2>&1",link_cyan_16t::subnets[j]); 
        stream = popen(cmd, "r");
        if (stream) {
            while(!feof(stream))
        ...
        check = 0;
        for (int i =0; i < 4; i++) {
            if (link_cyan_16t::mtu_ref[i] != buffer[i+4]) {
                check ++;
            }
        }
        if (check != 0) {
            UHD_LOG_WARNING("UHD", "MTU not set to recomended value of " << link_cyan_16t::mtu_ref <<  " for subnet " << link_cyan_16t::subnets[j] << " may impact data sent over " << link_cyan_16t::names[j]);
        }
    }
...
//    }
    ...
    _tree->create<subdev_spec_t>(mb_path / "rx_subdev_spec").add_coerced_subscriber(std::bind(&cyan_16t_impl::update_rx_subdev_spec, this, mb, ph::_1));
    _tree->create<subdev_spec_t>(mb_path / "tx_subdev_spec").add_coerced_subscriber(std::bind(&cyan_16t_impl::update_tx_subdev_spec, this, mb, ph::_1));
    ...
    for( size_t dspno = 0; dspno < CYAN_16T_RX_CHANNELS; dspno++ ) {
		...
		_tree->create<double> (rx_dsp_path / "rate" / "value")
			...
			.add_desired_subscriber(std::bind(&cyan_16t_impl::update_rx_samp_rate, this, mb, (size_t) dspno, ph::_1))
			.set_publisher(std::bind(&cyan_16t_impl::get_double, this, ("rx_"+lc_num+"/dsp/rate")    ));
        ...
        try {
            ...
            UHD_LOGGER_WARNING(CYAN_16T_DEBUG_NAME_C) << "Unable to bind ip adress, certain features may not work. \n IP: " << _tree->access<std::string>( rx_link_path / "ip_dest" ).get() << std::endl;
        }
    }
    ...
    for( int dspno = 0; dspno < CYAN_16T_TX_CHANNELS; dspno++ ) {
		...
		_tree->create<double> (tx_dsp_path / "rate" / "value")
			...
			.add_desired_subscriber(std::bind(&cyan_16t_impl::update_tx_samp_rate, this, mb, (size_t) dspno, ph::_1))
			.set_publisher(std::bind(&cyan_16t_impl::get_double, this, ("tx_"+lc_num+"/dsp/rate")    ));
...
void cyan_16t_impl::set_tx_gain(double gain, const std::string &name, size_t chan){
    auto mb_root = [&](size_t mboard) -> std::string {
		return "/mboards/" + std::to_string(mboard);
	};
	auto tx_rf_fe_root = [&](size_t chan) -> std::string {
    ...
    for (size_t c = 0; c < CYAN_16T_TX_CHANNELS; c++){
        set_tx_gain(gain, name, c);
    }
}

double cyan_16t_impl::get_tx_gain(const std::string &name, size_t chan) {
