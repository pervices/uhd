...
class cyan_16t_recv_packet_streamer : public sph::recv_packet_streamer {
public:
	typedef std::function<void(void)> onfini_type;

	cyan_16t_recv_packet_streamer(const size_t max_num_samps)
	: sph::recv_packet_streamer( max_num_samps )
	{
        _max_num_samps = max_num_samps;
    }

	virtual ~cyan_16t_recv_packet_streamer() {
		teardown();
	}
...
class cyan_16t_send_packet_streamer : public sph::send_packet_streamer {
public:
	...
	virtual ~cyan_16t_send_packet_streamer() {
		teardown();
	}
	...
	void pillage() {
		...
		if ( ! _pillaging ) {
			...
			_pillage_thread = std::thread( cyan_16t_send_packet_streamer::send_viking_loop, this );
			_pillaging = true;
		}
	}
...
private:
     ...
     **********************************************************************/
	static void send_viking_loop( cyan_16t_send_packet_streamer *self ) {
 ...
 * Helper Functions
 **********************************************************************/
void cyan_16t_impl::io_init(void){
...
void cyan_16t_impl::update_rx_samp_rate(const std::string &mb, const size_t dsp, const double rate_){
...
void cyan_16t_impl::update_rx_subdev_spec(const std::string &which_mb, const subdev_spec_t &spec){
...
void cyan_16t_impl::update_tx_subdev_spec(const std::string &which_mb, const subdev_spec_t &spec){
    fs_path root = "/mboards/" + which_mb + "/dboards";
 ...
 * Async Data
 **********************************************************************/
bool cyan_16t_impl::recv_async_msg(
 ...
 * Receive streamer
 **********************************************************************/
rx_streamer::sptr cyan_16t_impl::get_rx_stream(const uhd::stream_args_t &args_){
...
tx_streamer::sptr cyan_16t_impl::get_tx_stream(const uhd::stream_args_t &args_){
    stream_args_t args = args_;
    ...
    //make the new streamer given the samples per packet
    cyan_16t_send_packet_streamer::timenow_type timenow_ = std::bind( & cyan_16t_impl::get_time_now, this );
    ...
    my_streamer->set_time_now(std::bind(&cyan_16t_impl::get_time_now,this));
    ...
    for (size_t chan_i = 0; chan_i < args.channels.size(); chan_i++){
        ...
        for (const std::string &mb : _mbc.keys()) {
            ...
            if (chan < num_chan_so_far){
                ...
                my_streamer->set_xport_chan_get_buff(chan_i, std::bind(
                    &cyan_16t_send_packet_streamer::get_send_buff, my_streamerp, chan_i, ph::_1
                ));

                my_streamer->set_xport_chan_update_fc_send_size(chan_i, std::bind(
                    &cyan_16t_send_packet_streamer::update_fc_send_count, my_streamerp, chan_i, ph::_1
                ));
                my_streamer->set_xport_chan_check_flow_control(chan_i, std::bind(
                    &cyan_16t_send_packet_streamer::check_flow_control, my_streamerp, chan_i, ph::_1
                ));
