...
void cyan_64t_csv_parse(std::vector<std::string> &tokens, char* data, const char delim) {
	int i = 0;
	while (data[i]) {
		std::string token = "";
...
mboard_eeprom_t cyan_64t_impl::get_mboard_eeprom(std::string req) {
	(void)req;
...
//    }
    ...
    TREE_CREATE_ST("/name", std::string, "Cyan 64t Device");
    ...
    TREE_CREATE_ST(mb_path / "name",   std::string, "Cyan64t");
    ...
    TREE_CREATE_ST(rx_path / "spec",   std::string, "64 RX RF chains, 1000MHz BW and DC-18GHz each");
    ...
    TREE_CREATE_ST(tx_path / "spec", std::string, "64 TX RF chains, 1000MHz BW and DC-18GHz each");
    ...
    for( int dspno = 0; dspno < CYAN_64T_TX_CHANNELS; dspno++ ) {
		...
		TREE_CREATE_ST(tx_fe_path / "gain" / "range", meta_range_t,
			meta_range_t(CYAN_64T_RF_TX_GAIN_RANGE_START, CYAN_64T_RF_TX_GAIN_RANGE_STOP, CYAN_64T_RF_TX_GAIN_RANGE_STEP));
