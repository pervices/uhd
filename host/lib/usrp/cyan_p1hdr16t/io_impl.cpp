...
class cyan_p1hdr16t_recv_packet_streamer : public sph::recv_packet_streamer {
public:
	typedef std::function<void(void)> onfini_type;

	cyan_p1hdr16t_recv_packet_streamer(const size_t max_num_samps)
	: sph::recv_packet_streamer( max_num_samps )
	{
        _max_num_samps = max_num_samps;
    }

	virtual ~cyan_p1hdr16t_recv_packet_streamer() {
		teardown();
	}
...
static std::vector<std::weak_ptr<cyan_p1hdr16t_recv_packet_streamer>> allocated_rx_streamers;
static void shutdown_lingering_rx_streamers() {
	...
	for( auto & rx: allocated_rx_streamers ) {
		if ( ! rx.expired() ) {
			std::shared_ptr<cyan_p1hdr16t_recv_packet_streamer> my_streamer = rx.lock();
			if ( my_streamer ) {
...
class cyan_p1hdr16t_send_packet_streamer : public sph::send_packet_streamer {
public:
	...
	cyan_p1hdr16t_send_packet_streamer( const size_t max_num_samps )
	:
		sph::send_packet_streamer( max_num_samps, CYAN_P1HDR16T_BUFF_SIZE ),
	...
	virtual ~cyan_p1hdr16t_send_packet_streamer() {
		teardown();
	}
    ...
    size_t send(
        ...
        if ( metadata.start_of_burst ) {
            if ( metadata.time_spec < now + default_sob ) {
                metadata.time_spec = now + default_sob;
                #ifdef UHD_TXRX_DEBUG_PRINTS
                std::cout << "UHD::CYAN_P1HDR16T::Warning: time_spec was too soon for start of burst and has been adjusted!" << std::endl;
                #endif
            }
            #ifdef UHD_TXRX_DEBUG_PRINTS
            std::cout << "UHD::CYAN_P1HDR16T::Info: " << get_time_now() << ": sob @ " << metadata.time_spec << " | " << metadata.time_spec.to_ticks( CYAN_P1HDR16T_DSP_CLOCK_RATE ) << std::endl;
            #endif
      ...
      //  }
        ...
        if ( 0 == nsamps_per_buff && metadata.end_of_burst ) {
            #ifdef UHD_TXRX_DEBUG_PRINTS
            std::cout << "UHD::CYAN_P1HDR16T::Info: " << now << ": " << "eob @ " << now << " | " << now.to_ticks( CYAN_P1HDR16T_DSP_CLOCK_RATE ) << std::endl;
            #endif
    ...
    static managed_send_buffer::sptr get_send_buff( std::weak_ptr<uhd::tx_streamer> tx_streamer, const size_t chan, double timeout ){
        ...
        std::shared_ptr<cyan_p1hdr16t_send_packet_streamer> my_streamer =
            std::dynamic_pointer_cast<cyan_p1hdr16t_send_packet_streamer>( tx_streamer.lock() );
    ...
    void resize(const size_t size){
		_eprops.resize( size );
		for( auto & ep: _eprops ) {
			ep.flow_control = uhd::flow_control_nonlinear::make( 1.0, 0.8, CYAN_P1HDR16T_BUFF_SIZE );
	...
	void pillage() {
		...
		if ( ! _pillaging ) {
			...
			_pillage_thread = std::thread( cyan_p1hdr16t_send_packet_streamer::send_viking_loop, this );
			_pillaging = true;
		}
	}
...
private:
     ...
     **********************************************************************/
	static void send_viking_loop( cyan_p1hdr16t_send_packet_streamer *self ) {
		...
		for( ; ! self->_blessbless; ) {
			...
			const long long usloop = 1.0 / (double)CYAN_P1HDR16T_UPDATE_PER_SEC * 1e6;
...
static std::vector<std::weak_ptr<cyan_p1hdr16t_send_packet_streamer>> allocated_tx_streamers;
static void shutdown_lingering_tx_streamers() {
	// This is required as a workaround, because the relevent destructurs are not called
	// when you close the top block in gnu radio. Unsolved mystery for the time being.
	for( auto & tx: allocated_tx_streamers ) {
		if ( ! tx.expired() ) {
			std::shared_ptr<cyan_p1hdr16t_send_packet_streamer> my_streamer = tx.lock();
			if ( my_streamer ) {
				my_streamer->teardown();
			}
		}
 ...
 * - alignment buffer
 **********************************************************************/
struct cyan_p1hdr16t_impl::io_impl{
 ...
 * Helper Functions
 **********************************************************************/
void cyan_p1hdr16t_impl::io_init(void){
    ...
    for (const std::string &mb : _mbc.keys()) {
        _mbc[mb].rx_streamers.resize( CYAN_P1HDR16T_RX_CHANNELS );
        _mbc[mb].tx_streamers.resize( CYAN_P1HDR16T_TX_CHANNELS );
    }
}

void cyan_p1hdr16t_impl::update_rx_samp_rate(const std::string &mb, const size_t dsp, const double rate_){

    set_double( "rx_" + std::string( 1, 'a' + dsp ) + "/dsp/rate", rate_ );
    double rate = get_double( "rx_" + std::string( 1, 'a' + dsp ) + "/dsp/rate" );

    std::shared_ptr<cyan_p1hdr16t_recv_packet_streamer> my_streamer =
        std::dynamic_pointer_cast<cyan_p1hdr16t_recv_packet_streamer>(_mbc[mb].rx_streamers[dsp].lock());
    if (my_streamer.get() == NULL) return;

    my_streamer->set_samp_rate(rate);
    my_streamer->set_tick_rate( CYAN_P1HDR16T_DSP_CLOCK_RATE );
}

void cyan_p1hdr16t_impl::update_tx_samp_rate(const std::string &mb, const size_t dsp, const double rate_ ){

    set_double( "tx_" + std::string( 1, 'a' + dsp ) + "/dsp/rate", rate_ );
    double rate = get_double( "tx_" + std::string( 1, 'a' + dsp ) + "/dsp/rate" );

	std::shared_ptr<cyan_p1hdr16t_send_packet_streamer> my_streamer =
        std::dynamic_pointer_cast<cyan_p1hdr16t_send_packet_streamer>(_mbc[mb].tx_streamers[dsp].lock());
    if (my_streamer.get() == NULL) return;
...
void cyan_p1hdr16t_impl::update_rates(void){
    for (const std::string &mb : _mbc.keys()) {
...
void cyan_p1hdr16t_impl::update_rx_subdev_spec(const std::string &which_mb, const subdev_spec_t &spec){
...
void cyan_p1hdr16t_impl::update_tx_subdev_spec(const std::string &which_mb, const subdev_spec_t &spec){
    fs_path root = "/mboards/" + which_mb + "/dboards";
 ...
 * Async Data
 **********************************************************************/
bool cyan_p1hdr16t_impl::recv_async_msg(
 ...
 * Receive streamer
 **********************************************************************/
rx_streamer::sptr cyan_p1hdr16t_impl::get_rx_stream(const uhd::stream_args_t &args_){
    ...
    std::shared_ptr<cyan_p1hdr16t_recv_packet_streamer> my_streamer = std::make_shared<cyan_p1hdr16t_recv_packet_streamer>(spp);
        ...
        for (const std::string &mb : _mbc.keys()) {
            ...
            if (chan < num_chan_so_far){
                ...
                my_streamer->set_issue_stream_cmd(chan_i, std::bind(
                    &cyan_p1hdr16t_impl::set_stream_cmd, this, scmd_pre, ph::_1));
...
static void get_fifo_lvl_udp( const size_t channel, uhd::transport::udp_simple::sptr xport, double & pcnt, uint64_t & uflow, uint64_t & oflow, uhd::time_spec_t & now ) {
	...
	static constexpr double tick_period_ps = 1.0 / CYAN_P1HDR16T_DSP_CLOCK_RATE;
	...
	pcnt = (double)lvl / CYAN_P1HDR16T_BUFF_SIZE;
...
tx_streamer::sptr cyan_p1hdr16t_impl::get_tx_stream(const uhd::stream_args_t &args_){
    stream_args_t args = args_;
    ...
    //make the new streamer given the samples per packet
    cyan_p1hdr16t_send_packet_streamer::timenow_type timenow_ = std::bind( & cyan_p1hdr16t_impl::get_time_now, this );
    ...
    std::shared_ptr<cyan_p1hdr16t_send_packet_streamer> my_streamer = std::make_shared<cyan_p1hdr16t_send_packet_streamer>( spp );
    ...
    my_streamer->set_time_now(std::bind(&cyan_p1hdr16t_impl::get_time_now,this));
    ...
    for (size_t chan_i = 0; chan_i < args.channels.size(); chan_i++){
        ...
        for (const std::string &mb : _mbc.keys()) {
            ...
            if (chan < num_chan_so_far){
                ...
                my_streamer->set_xport_chan_get_buff(chan_i, std::bind(
                    &cyan_p1hdr16t_send_packet_streamer::get_send_buff, my_streamerp, chan_i, ph::_1
                ));

                my_streamer->set_xport_chan_update_fc_send_size(chan_i, std::bind(
                    &cyan_p1hdr16t_send_packet_streamer::update_fc_send_count, my_streamerp, chan_i, ph::_1
                ));
                my_streamer->set_xport_chan_check_flow_control(chan_i, std::bind(
                    &cyan_p1hdr16t_send_packet_streamer::check_flow_control, my_streamerp, chan_i, ph::_1
                ));
