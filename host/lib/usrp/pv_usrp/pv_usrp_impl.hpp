...
#ifndef INCLUDED_PV_USRP_IMPL_HPP
#define INCLUDED_PV_USRP_IMPL_HPP
...
class pv_usrp_impl : public uhd::device
{
public:
    // Cache line size
    ...
    // This is the core constructor to be called when a pv_usrp device is found
    pv_usrp_impl(const uhd::device_addr_t &);
    ~pv_usrp_impl(void);

    static device_addrs_t pv_usrp_find_with_addr(const device_addr_t &hint);
    static device_addrs_t pv_usrp_find(const device_addr_t &hint_);

    // pointers to the streams for the device
    // these functions are defined in pv_usrp_io_impl.cpp
...
private:
    ...
    static void detect_pps(pv_usrp_impl *dev);
    ...
    // set arbitrary pv_usrp properties from dev_addr_t using mappings of the form "pv_usrp:key" => "val"
    ...
    void time_diff_send( const uhd::time_spec_t & pv_usrp_now );
...
private:
    ...
    struct mb_container_type{
        ...
        std::vector<std::weak_ptr<uhd::usrp::pv_usrp_recv_packet_streamer>> rx_streamers;
        std::vector<std::weak_ptr<uhd::usrp::pv_usrp_send_packet_streamer>> tx_streamers;
...
#endif /* INCLUDED_PV_USRP_IMPL_HPP */
