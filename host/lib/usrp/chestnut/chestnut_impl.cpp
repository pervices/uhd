...
const fs_path tx_path   = CHESTNUT_MB_PATH / "tx";
const fs_path rx_path   = CHESTNUT_MB_PATH / "rx";
...
std::string chestnut_impl::rx_link_root(const size_t channel, const size_t mboard)
{
    return mb_root(mboard) + "/rx_link/" + std::to_string(channel);
}

std::string chestnut_impl::tx_link_root(const size_t channel, const size_t mboard)
{
    return mb_root(mboard) + "/tx_link/" + std::to_string(channel);
}

std::string chestnut_impl::tx_dsp_root(const size_t channel, const size_t mboard) {
...
std::string chestnut_impl::get_string(std::string req) {
    ...
    if(ret == "GET_ERROR") {
        throw uhd::lookup_error("chestnut_impl::get_string - Unable to read property on the server: " + req + "\nPlease Verify that the server is up to date");
    }
    else if (ret == "TIMEOUT") {
        throw uhd::runtime_error("chestnut_impl::get_string - UDP resp. timed out: get: " + req);
    }
    else  if(ret == "ERROR") {
        throw uhd::runtime_error("chestnut_impl::get_string - UDP unpecified error: " + req);
    }
    else {
        return ret;
    }
...
void chestnut_impl::set_string(const std::string pre, std::string data) {
    ...
    if(ret == "GET_ERROR") {
        throw uhd::lookup_error("chestnut_impl::set_string - Unable to read property on the server: " + pre + "\nPlease Verify that the server is up to date");
    }
    else if (ret == "TIMEOUT") {
        throw uhd::runtime_error("chestnut_impl::set_string - UDP resp. timed out: set: " + pre + " = " + data);
    }
    else  if(ret == "ERROR") {
        throw uhd::runtime_error("chestnut_impl::set_string - UDP unpecified error: " + pre);
    }
    else {
        return;
    }
...
double chestnut_impl::get_double(std::string req) {
    try { return boost::lexical_cast<double>( get_string(req) );
    } catch(boost::bad_lexical_cast &e) {
        UHD_LOGGER_WARNING(CHESTNUT_DEBUG_NAME_C) << "Failed to get double property: " << e.what();
    }
    return 0;
}
void chestnut_impl::set_double(const std::string pre, double data){
    set_string(pre, boost::lexical_cast<std::string>(data));
}

// wrapper for type <bool> through the ASCII Crimson interface
bool chestnut_impl::get_bool(std::string req) {
    try { return boost::lexical_cast<bool>( get_string(req) );
    } catch(boost::bad_lexical_cast &e) {
        UHD_LOGGER_WARNING(CHESTNUT_DEBUG_NAME_C) << "Failed to get bool property: " << e.what();
    }
    return 0;
}
void chestnut_impl::set_bool(const std::string pre, bool data){
    set_string(pre, boost::lexical_cast<std::string>(data));
}

// wrapper for type <int> through the ASCII Crimson interface
int chestnut_impl::get_int(std::string req) {
	try { return boost::lexical_cast<int>( get_string(req) );
    } catch(boost::bad_lexical_cast &e) {
        UHD_LOGGER_WARNING(CHESTNUT_DEBUG_NAME_C) << "Failed to get int property: " << e.what();
    }
    return 0;
}
void chestnut_impl::set_int(const std::string pre, int data){
    set_string(pre, boost::lexical_cast<std::string>(data));
}

uhd::time_spec_t chestnut_impl::get_time_now() {
...
uhd::usrp::mboard_eeprom_t chestnut_impl::get_mboard_eeprom(std::string req) {
	(void)req;
...
void chestnut_impl::set_mboard_eeprom(const std::string pre, mboard_eeprom_t data) {
	(void)pre;
	(void)data;
...
dboard_eeprom_t chestnut_impl::get_dboard_eeprom(std::string req) {
	(void)req;
...
void chestnut_impl::set_dboard_eeprom(const std::string pre, dboard_eeprom_t data) {
	(void)pre;
	(void)data;
	// no eeprom settings on Crimson
	return;
...
sensor_value_t chestnut_impl::get_sensor_value(std::string req) {
    ...
    if(req.find("lmk_lockdetect") != std::string::npos) {
        ...
        UHD_LOGGER_WARNING(CHESTNUT_DEBUG_NAME_C) << "sensor implementation not validated: " << req;
        return sensor_value_t( req, sensor_good, "good", "bad" );
    }
}
void chestnut_impl::set_sensor_value(const std::string pre, sensor_value_t data) {
...
meta_range_t chestnut_impl::get_meta_range(std::string req) {
	(void)req;
	throw uhd::not_implemented_error("set_meta_range not implemented, " CHESTNUT_DEBUG_NAME_S " does not support range settings");
}
void chestnut_impl::set_meta_range(const std::string pre, meta_range_t data) {
	(void)pre;
	(void)data;
	throw uhd::not_implemented_error("set_meta_range not implemented, " CHESTNUT_DEBUG_NAME_S " does not support range settings");
}
...
std::complex<double>  chestnut_impl::get_complex_double(std::string req) {
	(void)req;
	std::complex<double> temp;
	return temp;
}
void chestnut_impl::set_complex_double(const std::string pre, std::complex<double> data) {
	(void)pre;
	(void)data;
	return;
}
...
stream_cmd_t chestnut_impl::get_stream_cmd(std::string req) {
	(void)req;
...
void chestnut_impl::set_stream_cmd( const std::string pre, stream_cmd_t stream_cmd ) {
    ...
    if(original_nsamps_req != stream_cmd.num_samps) {
        ...
        if(stream_cmd.stream_mode != uhd::stream_cmd_t::STREAM_MODE_STOP_CONTINUOUS) {
            UHD_LOGGER_WARNING(CHESTNUT_DEBUG_NAME_S) << "Number of samples requested must be multiple of " << nsamps_multiple_rx << ". The number of samples requested has been modified to " << stream_cmd.num_samps << std::endl;
        }
    }
    ...
    if (stream_cmd.time_spec.get_real_secs() < get_time_now().get_real_secs() + 0.01 && stream_cmd.stream_mode != uhd::stream_cmd_t::STREAM_MODE_STOP_CONTINUOUS && !stream_cmd.stream_now) {
        UHD_LOGGER_WARNING(CHESTNUT_DEBUG_NAME_C) << "Requested rx start time of " + std::to_string(stream_cmd.time_spec.get_real_secs()) + " close to current device time of " + std::to_string(current_time) + ". Ignoring start time and enabling stream_now";
    ...
    size_t jesd_num = chestnut_impl::get_rx_jesd_num(ch);

	make_rx_stream_cmd_packet( stream_cmd, jesd_num, rx_stream_cmd );

    int xg_intf = chestnut_impl::get_rx_xg_intf(ch);
...
time_spec_t chestnut_impl::get_time_spec(std::string req) {
	if ( false ) {
...
void chestnut_impl::set_time_spec( const std::string key, time_spec_t value ) {
...
void chestnut_impl::detect_pps( chestnut_impl *dev ) {

    dev->_pps_thread_running = true;
    int pps_detected;

    while (! dev->_pps_thread_should_exit) {
        dev->get_tree()->access<int>(CHESTNUT_TIME_PATH / "pps_detected").set(1);
        pps_detected = dev->get_tree()->access<int>(CHESTNUT_TIME_PATH / "pps_detected").get();
...
user_reg_t chestnut_impl::get_user_reg(std::string req) {
...
void chestnut_impl::send_gpio_burst_req(const gpio_burst_req& req) {
	_time_diff_iface[0]->send(boost::asio::const_buffer(&req, sizeof(req)));
}

//TODO: implement the ability for users to access registers
void chestnut_impl::set_user_reg(const std::string key, user_reg_t value) {
...
void chestnut_impl::set_properties_from_addr() {
	...
	for( auto & prop: device_addr.keys() ) {
		if ( 0 == prop.compare( 0, crimson_prop_prefix.length(), crimson_prop_prefix ) ) {
			...
			if ( actual_string != expected_string ) {
				UHD_LOGGER_ERROR(CHESTNUT_DEBUG_NAME_C "_IMPL")
					<< __func__ << "(): "
					<< "Setting " CHESTNUT_DEBUG_NAME_S "  property failed: "
					<< "key: '"<< key << "', "
...
static constexpr double tick_period_ns = 1.0 / CHESTNUT_TICK_RATE * 1e9;
...
void chestnut_impl::make_rx_stream_cmd_packet( const uhd::stream_cmd_t & cmd, const size_t jesd_num, uhd::usrp::rx_stream_cmd & pkt ) {
...
void chestnut_impl::send_rx_stream_cmd_req( const rx_stream_cmd & req ) {
	_time_diff_iface[0]->send( boost::asio::const_buffer( & req, sizeof( req ) ) );
}

//sends a stream command over the specified sfp port (xg_intf = 0 means sfpa, =1 means spfb)
void chestnut_impl::send_rx_stream_cmd_req( const rx_stream_cmd & req,  int xg_intf) {
...
void chestnut_impl::time_diff_send( const uhd::time_spec_t & crimson_now ) {
...
void chestnut_impl::time_diff_send( const uhd::time_spec_t & crimson_now, int xg_intf) {
...
bool chestnut_impl::time_diff_recv( time_diff_resp & tdr ) {
...
bool chestnut_impl::time_diff_recv( time_diff_resp & tdr, int xg_intf ) {
...
void chestnut_impl::reset_time_diff_pid() {
...
void chestnut_impl::time_diff_process( const time_diff_resp & tdr, const uhd::time_spec_t & now ) {
...
void chestnut_impl::start_pps_dtc() {
...
void chestnut_impl::stop_pps_dtc() {
...
inline bool chestnut_impl::time_diff_converged() {
	return _time_diff_converged;
}

// Wait for convergence
void chestnut_impl::wait_for_time_diff_converged() {
    for(
        ...
        if ( (time_now - time_then).get_full_secs() > 20 ) {
            UHD_LOGGER_ERROR(CHESTNUT_DEBUG_NAME_C "_IMPL")
                << "Clock domain synchronization taking unusually long. Are there more than 1 applications controlling " CHESTNUT_DEBUG_NAME_S "?"
                << std::endl;
            throw runtime_error( "Clock domain synchronization taking unusually long. Are there more than 1 applications controlling " CHESTNUT_DEBUG_NAME_S"?" );
        }
        usleep( 100000 );
    }
}
...
void chestnut_impl::bm_thread_fn( chestnut_impl *dev ) {

    //the sfp port clock synchronization will be conducted on
    int xg_intf = 0;
    
	const uhd::time_spec_t T( 1.0 / (double) CHESTNUT_UPDATE_PER_SEC );
...
UHD_STATIC_BLOCK(register_chestnut_device)
{
    ...
    device::register_device(&chestnut_find, &chestnut_make, device::USRP);
}
...
#define TREE_CREATE_RW(PATH, PROP, TYPE, HANDLER)						\
	do { _tree->create<TYPE> (PATH)								\
		.add_desired_subscriber(std::bind(&chestnut_impl::set_ ## HANDLER, this, (PROP), ph::_1))	\
		.set_publisher(std::bind(&chestnut_impl::get_ ## HANDLER, this, (PROP)    ));	\
	} while(0)
    ...
    _use_dpdk(use_dpdk)
{
    ...
    _mbc[mb].iface = chestnut_iface::make(
		udp_simple::make_connected(
			_device_addr["addr"],
			BOOST_STRINGIZE( CHESTNUT_FW_COMMS_UDP_PORT )
		)
    );
    ...
    TREE_CREATE_RW(CHESTNUT_MB_PATH / "system/get_max_buffer_level", "system/get_max_buffer_level", double, double);
    max_buffer_level = (int64_t) (_tree->access<double>(CHESTNUT_MB_PATH / "system/get_max_buffer_level").get());
    // The number to multiply get buffer level requests by to get the actual buffer level in number of samples
    TREE_CREATE_RW(CHESTNUT_MB_PATH / "system/get_buffer_level_multiple", "system/get_buffer_level_multiple", double, double);
    buffer_level_multiple = (int64_t) (_tree->access<double>(CHESTNUT_MB_PATH / "system/get_buffer_level_multiple").get());

    TREE_CREATE_RW(CHESTNUT_MB_PATH / "system/nsamps_multiple_rx", "system/nsamps_multiple_rx", int, int);
    nsamps_multiple_rx = (_tree->access<int>(CHESTNUT_MB_PATH / "system/nsamps_multiple_rx").get());

    if (not device_addr.has_key("send_buff_size")){
        //The buffer should be the size of the SRAM on the device,
        //because we will never commit more than the SRAM can hold.
        device_addr["send_buff_size"] = boost::lexical_cast<std::string>( (size_t) (max_buffer_level * sizeof( std::complex<int16_t> ) * ((double)(MAX_ETHERNET_MTU+1)/(CHESTNUT_MAX_MTU-CHESTNUT_UDP_OVERHEAD))) );
    }

    TREE_CREATE_RO(CHESTNUT_MB_PATH / "system/num_rx", "system/num_rx", int, int);
    TREE_CREATE_RO(CHESTNUT_MB_PATH / "system/num_tx", "system/num_tx", int, int);
    num_rx_channels = (size_t) (_tree->access<int>(CHESTNUT_MB_PATH / "system/num_rx").get());
    is_num_rx_channels_set = true;
    num_tx_channels = (size_t) (_tree->access<int>(CHESTNUT_MB_PATH / "system/num_tx").get());
    ...
    TREE_CREATE_RO(CHESTNUT_MB_PATH / "system/max_rate", "system/max_rate", double, double);
    max_sample_rate = (_tree->access<double>(CHESTNUT_MB_PATH / "system/max_rate").get());

    TREE_CREATE_RO(CHESTNUT_MB_PATH / "system/otw_rx", "system/otw_rx", int, int);
    otw_rx = (_tree->access<int>(CHESTNUT_MB_PATH / "system/otw_rx").get());
    otw_rx_s = "sc" + std::to_string(otw_rx);

    TREE_CREATE_RO(CHESTNUT_MB_PATH / "system/otw_tx", "system/otw_tx", int, int);
    otw_tx = (_tree->access<int>(CHESTNUT_MB_PATH / "system/otw_tx").get());
    otw_tx_s = "sc" + std::to_string(otw_tx);

    TREE_CREATE_RO(CHESTNUT_MB_PATH / "system/flags/USE_3G_AS_1G", "system/flags/USE_3G_AS_1G", int, int);
    flag_use_3g_as_1g = (_tree->access<int>(CHESTNUT_MB_PATH / "system/flags/USE_3G_AS_1G").get());
    ...
    _tree->create<std::vector<std::string> >(CHESTNUT_MB_PATH / "time_source" / "options").set(time_sources);

    static const std::vector<double> external_freq_options = boost::assign::list_of(10e6);
    _tree->create<std::vector<double> >(CHESTNUT_MB_PATH / "clock_source" / "external" / "freq" / "options");
    static const std::vector<std::string> clock_source_options = boost::assign::list_of("internal")("external");
    _tree->create<std::vector<std::string> >(CHESTNUT_MB_PATH / "clock_source" / "options").set(clock_source_options);

    TREE_CREATE_ST("/name", std::string, CHESTNUT_DEBUG_NAME_S " Device");
    ...
    _tree->create<std::vector<size_t> >(CHESTNUT_MB_PATH / "rx_chan_dsp_mapping").set(default_rx_map);
    _tree->create<std::vector<size_t> >(CHESTNUT_MB_PATH / "tx_chan_dsp_mapping").set(default_tx_map);
    _tree->create<subdev_spec_t>(CHESTNUT_MB_PATH / "rx_subdev_spec").add_coerced_subscriber(std::bind(&chestnut_impl::update_rx_subdev_spec, this, mb, ph::_1));
    _tree->create<subdev_spec_t>(CHESTNUT_MB_PATH / "tx_subdev_spec").add_coerced_subscriber(std::bind(&chestnut_impl::update_tx_subdev_spec, this, mb, ph::_1));

    TREE_CREATE_ST(CHESTNUT_MB_PATH / "vendor", std::string, "Per Vices");
    TREE_CREATE_ST(CHESTNUT_MB_PATH / "name",   std::string, "FPGA Board");
    TREE_CREATE_RW(CHESTNUT_MB_PATH / "id",         "fpga/about/id",     std::string, string);
    TREE_CREATE_RW(CHESTNUT_MB_PATH / "serial",     "fpga/about/serial", std::string, string);
    TREE_CREATE_RW(CHESTNUT_MB_PATH / "server_version", "fpga/about/server_ver", std::string, string);
    TREE_CREATE_RW(CHESTNUT_MB_PATH / "fw_version", "fpga/about/fw_ver", std::string, string);
    TREE_CREATE_RW(CHESTNUT_MB_PATH / "hw_version", "fpga/about/hw_ver", std::string, string);
    TREE_CREATE_RW(CHESTNUT_MB_PATH / "sw_version", "fpga/about/sw_ver", std::string, string);
    TREE_CREATE_RW(CHESTNUT_MB_PATH / "imgparam/backplane_pinout", "fpga/about/imgparam/backplane_pinout", int, int);
    TREE_CREATE_RW(CHESTNUT_MB_PATH / "imgparam/ddr_used", "fpga/about/imgparam/ddr_used", int, int);
    TREE_CREATE_RW(CHESTNUT_MB_PATH / "imgparam/hps_only", "fpga/about/imgparam/hps_only", int, int);
    TREE_CREATE_RW(CHESTNUT_MB_PATH / "imgparam/num_rx", "fpga/about/imgparam/num_rx", int, int);
    TREE_CREATE_RW(CHESTNUT_MB_PATH / "imgparam/num_tx", "fpga/about/imgparam/num_tx", int, int);
    TREE_CREATE_RW(CHESTNUT_MB_PATH / "imgparam/rate", "fpga/about/imgparam/rate", int, int);
    TREE_CREATE_RW(CHESTNUT_MB_PATH / "imgparam/rtm", "fpga/about/imgparam/rtm", int, int);
    TREE_CREATE_WO(CHESTNUT_MB_PATH / "blink", "fpga/board/led", int, int);
    TREE_CREATE_RW(CHESTNUT_MB_PATH / "temp", "fpga/board/temp", std::string, string);

    TREE_CREATE_RW(CHESTNUT_MB_PATH / "user/regs", "fpga/user/regs", user_reg_t, user_reg);

    TREE_CREATE_RW(CHESTNUT_MB_PATH / "trigger/sma_dir", "fpga/trigger/sma_dir",  std::string, string);
    TREE_CREATE_RW(CHESTNUT_MB_PATH / "trigger/sma_pol", "fpga/trigger/sma_pol",  std::string, string);

    // String is used because this is a 64 bit number and won't fit in int
    TREE_CREATE_RW(CHESTNUT_MB_PATH / "gps_time", "fpga/board/gps_time", std::string, string);
    TREE_CREATE_RW(CHESTNUT_MB_PATH / "gps_frac_time", "fpga/board/gps_frac_time", std::string, string);
    TREE_CREATE_RW(CHESTNUT_MB_PATH / "gps_sync_time", "fpga/board/gps_sync_time", int, int);

    TREE_CREATE_RW(CHESTNUT_MB_PATH / "fpga/board/flow_control/sfpa_port", "fpga/board/flow_control/sfpa_port", int, int);
    TREE_CREATE_RW(CHESTNUT_MB_PATH / "fpga/board/flow_control/sfpb_port", "fpga/board/flow_control/sfpb_port", int, int);
    TREE_CREATE_RW(CHESTNUT_MB_PATH / "fpga/board/flow_control/sfpc_port", "fpga/board/flow_control/sfpc_port", int, int);
    TREE_CREATE_RW(CHESTNUT_MB_PATH / "fpga/board/flow_control/sfpd_port", "fpga/board/flow_control/sfpd_port", int, int);

    TREE_CREATE_ST(CHESTNUT_TIME_PATH / "name", std::string, "Time Board");
    TREE_CREATE_RW(CHESTNUT_TIME_PATH / "id",         "time/about/id",     std::string, string);
    TREE_CREATE_RW(CHESTNUT_TIME_PATH / "serial",     "time/about/serial", std::string, string);
    TREE_CREATE_RW(CHESTNUT_TIME_PATH / "fw_version", "time/about/fw_ver", std::string, string);
    TREE_CREATE_RW(CHESTNUT_TIME_PATH / "sw_version", "time/about/sw_ver", std::string, string);
    ...
    TREE_CREATE_RW(CHESTNUT_MB_PATH / "link_max_rate", "fpga/link/rate", double, double);

    // SFP settings
    TREE_CREATE_RW(CHESTNUT_MB_PATH / "link" / "sfpa" / "ip_addr",  "fpga/link/sfpa/ip_addr", std::string, string);
    TREE_CREATE_RW(CHESTNUT_MB_PATH / "link" / "sfpa" / "pay_len", "fpga/link/sfpa/pay_len", int, int);
    TREE_CREATE_RW(CHESTNUT_MB_PATH / "link" / "sfpb" / "ip_addr",     "fpga/link/sfpb/ip_addr", std::string, string);
    TREE_CREATE_RW(CHESTNUT_MB_PATH / "link" / "sfpb" / "pay_len", "fpga/link/sfpb/pay_len", int, int);
    TREE_CREATE_RW(CHESTNUT_MB_PATH / "link" / "sfpc" / "ip_addr",  "fpga/link/sfpc/ip_addr", std::string, string);
    TREE_CREATE_RW(CHESTNUT_MB_PATH / "link" / "sfpc" / "pay_len", "fpga/link/sfpc/pay_len", int, int);
    TREE_CREATE_RW(CHESTNUT_MB_PATH / "link" / "sfpd" / "ip_addr",     "fpga/link/sfpd/ip_addr", std::string, string);
    TREE_CREATE_RW(CHESTNUT_MB_PATH / "link" / "sfpd" / "pay_len", "fpga/link/sfpd/pay_len", int, int);

    std::string sfpa_ip = _tree->access<std::string>(CHESTNUT_MB_PATH / "link" / "sfpa" / "ip_addr").get();
    ping_check("sfpa", sfpa_ip);

    std::string sfpb_ip = _tree->access<std::string>(CHESTNUT_MB_PATH / "link" / "sfpb" / "ip_addr").get();
    ping_check("sfpb", sfpb_ip);

    std::string sfpc_ip = _tree->access<std::string>(CHESTNUT_MB_PATH / "link" / "sfpc" / "ip_addr").get();
    ping_check("sfpc", sfpc_ip);

    std::string sfpd_ip = _tree->access<std::string>(CHESTNUT_MB_PATH / "link" / "sfpd" / "ip_addr").get();
    ping_check("sfpd", sfpd_ip);

    for (int i = 0; i < NUMBER_OF_XG_CONTROL_INTF; i++) {
        std::string xg_intf = std::string(1, char('a' + i));
        int sfp_port = _tree->access<int>( CHESTNUT_MB_PATH / "fpga/board/flow_control/sfp" + xg_intf + "_port" ).get();
        std::string time_diff_ip = _tree->access<std::string>( CHESTNUT_MB_PATH / "link" / "sfp" + xg_intf / "ip_addr" ).get();
    ...
    TREE_CREATE_ST(CHESTNUT_MB_PATH / "tick_rate", double, CHESTNUT_TICK_RATE);

    TREE_CREATE_RW(CHESTNUT_TIME_PATH / "cmd", "time/clk/cmd",      time_spec_t, time_spec);
    // This line will get time spec, the time diff port must be initialized first
    TREE_CREATE_RW(CHESTNUT_TIME_PATH / "now", "time/clk/cur_time", time_spec_t, time_spec);
    TREE_CREATE_RW(CHESTNUT_TIME_PATH / "pps", "time/clk/pps",    time_spec_t, time_spec);
    TREE_CREATE_RW(CHESTNUT_TIME_PATH / "pps_detected", "time/clk/pps_detected",    int,         int);
    ...
    try {
        ...
        _tree->access<int>(CHESTNUT_TIME_PATH / "pps_detected").get();
    ...
    temp["vendor"]   = "Per Vices";
    temp["serial"]   = "";
    TREE_CREATE_ST(CHESTNUT_MB_PATH / "eeprom", mboard_eeprom_t, temp);

    // This property chooses internal or external time (usually pps) source
    TREE_CREATE_RW(CHESTNUT_MB_PATH / "time_source"  / "value",  	"time/source/set_time_source",  	std::string, string);
    // Sets whether to use internal or external clock source
    TREE_CREATE_RW(CHESTNUT_MB_PATH / "time_source"  / "freq",  	    "time/source/freq_mhz",  	int, int);
    TREE_CREATE_RW(CHESTNUT_MB_PATH / "clock_source" / "value",      "time/source/ref",	std::string, string);
    TREE_CREATE_RW(CHESTNUT_MB_PATH / "clock_source" / "external",	"time/source/ref",	std::string, string);
    TREE_CREATE_ST(CHESTNUT_MB_PATH / "clock_source" / "external" / "value", double, CHESTNUT_EXT_CLK_RATE);
    TREE_CREATE_ST(CHESTNUT_MB_PATH / "clock_source" / "output", bool, true);
    TREE_CREATE_ST(CHESTNUT_MB_PATH / "time_source"  / "output", bool, true);

    TREE_CREATE_RW(CHESTNUT_MB_PATH / "sensors" / "ref_locked", "time/status/lmk_lockdetect", sensor_value_t, sensor_value );

    // No GPSDO support on Crimson
    // TREE_CREATE_ST(CHESTNUT_MB_PATH / "sensors" / "ref_locked", sensor_value_t, sensor_value_t("NA", "0", "NA"));
    ...
    for( size_t dspno = 0; dspno < num_rx_channels; dspno++ ) {
		...
		const fs_path rx_codec_path = CHESTNUT_MB_PATH / "rx_codecs" / num;
		const fs_path rx_fe_path    = CHESTNUT_MB_PATH / "dboards" / num / "rx_frontends" / chan;
		const fs_path db_path       = CHESTNUT_MB_PATH / "dboards" / num;
		const fs_path rx_dsp_path   = CHESTNUT_MB_PATH / "rx_dsps" / dspno;
		const fs_path rx_link_path  = CHESTNUT_MB_PATH / "rx_link" / dspno;
	    ...
	    // RX bandwidth
		TREE_CREATE_ST(rx_fe_path / "bandwidth" / "value", double, (double) CHESTNUT_BW_FULL );
		TREE_CREATE_ST(rx_fe_path / "bandwidth" / "range", meta_range_t, meta_range_t( (double) CHESTNUT_BW_FULL, (double) CHESTNUT_BW_FULL ) );

		TREE_CREATE_ST(rx_fe_path / "freq", meta_range_t,
			meta_range_t((double) CHESTNUT_FREQ_RANGE_START, (double) CHESTNUT_FREQ_RANGE_STOP, (double) CHESTNUT_FREQ_RANGE_STEP));
		...
		TREE_CREATE_ST(rx_fe_path / "gain" / "range", meta_range_t,
			meta_range_t((double) CHESTNUT_RF_RX_GAIN_RANGE_START, (double) CHESTNUT_RF_RX_GAIN_RANGE_STOP, (double) CHESTNUT_RF_RX_GAIN_RANGE_STEP));
		...
		TREE_CREATE_ST(rx_dsp_path / "rate" / "range", meta_range_t,
			meta_range_t((double) CHESTNUT_RATE_RANGE_START, (double) CHESTNUT_RATE_RANGE_STOP_FULL, (double) CHESTNUT_RATE_RANGE_STEP));
		TREE_CREATE_ST(rx_dsp_path / "freq" / "range", meta_range_t,
			meta_range_t((double) CHESTNUT_DSP_FREQ_RANGE_START_FULL, (double) CHESTNUT_DSP_FREQ_RANGE_STOP_FULL, (double) CHESTNUT_DSP_FREQ_RANGE_STEP));
		TREE_CREATE_ST(rx_dsp_path / "bw" / "range",   meta_range_t,
			meta_range_t((double) CHESTNUT_DSP_BW_START, (double) CHESTNUT_DSP_BW_STOP_FULL, (double) CHESTNUT_DSP_BW_STEPSIZE));

		_tree->create<double> (rx_dsp_path / "rate" / "value")
			.set( get_double ("rx_"+lc_num+"/dsp/rate"))
			.add_desired_subscriber(std::bind(&chestnut_impl::update_rx_samp_rate, this, mb, (size_t) dspno, ph::_1))
			.set_publisher(std::bind(&chestnut_impl::get_double, this, ("rx_"+lc_num+"/dsp/rate")    ));
    ...
    for( size_t dspno = 0; dspno < num_tx_channels; dspno++ ) {
		...
		const fs_path tx_codec_path = CHESTNUT_MB_PATH / "tx_codecs" / num;
		const fs_path tx_fe_path    = CHESTNUT_MB_PATH / "dboards" / num / "tx_frontends" / chan;
		const fs_path db_path       = CHESTNUT_MB_PATH / "dboards" / num;
		const fs_path tx_dsp_path   = CHESTNUT_MB_PATH / "tx_dsps" / dspno;
		const fs_path tx_link_path  = CHESTNUT_MB_PATH / "tx_link" / dspno;
	    ...
	    // TX bandwidth
		TREE_CREATE_ST(tx_fe_path / "bandwidth" / "value", double, (double) CHESTNUT_BW_FULL );
		TREE_CREATE_ST(tx_fe_path / "bandwidth" / "range", meta_range_t, meta_range_t( (double) CHESTNUT_BW_FULL, (double) CHESTNUT_BW_FULL ) );
		...
		TREE_CREATE_ST(tx_fe_path / "gain" / "range", meta_range_t,
			meta_range_t((double) CHESTNUT_RF_TX_GAIN_RANGE_START, (double) CHESTNUT_RF_TX_GAIN_RANGE_STOP, (double) CHESTNUT_RF_TX_GAIN_RANGE_STEP));
...
// 			.set( get_double ("tx_"+lc_num+"/dsp/rate"))
			.add_desired_subscriber(std::bind(&chestnut_impl::update_tx_samp_rate, this, mb, (size_t) dspno, ph::_1))
			.set_publisher(std::bind(&chestnut_impl::get_double, this, ("tx_"+lc_num+"/dsp/rate")    ));
		...
		_mbc[mb].fifo_ctrl_xports.push_back(
			udp_simple::make_connected(
				_tree->access<std::string>( CHESTNUT_MB_PATH / "link" / sfp / "ip_addr" ).get(),
				std::to_string( _tree->access<int>( CHESTNUT_MB_PATH / "fpga" / "board" / "flow_control" / ( sfp + "_port" ) ).get() )
			)
		);
    }

	const fs_path cm_path  = CHESTNUT_MB_PATH / "cm";
	...
	if ( _bm_thread_needed ) {
		...
		_time_diff_pidc = uhd::pidc_tl(
			...
			2.0 / (double)CHESTNUT_UPDATE_PER_SEC
		);

		_time_diff_pidc.set_error_filter_length( CHESTNUT_UPDATE_PER_SEC );
...
int chestnut_impl::get_rx_jesd_num(int channel) {
    const fs_path rx_link_path  = CHESTNUT_MB_PATH / "rx_link" / channel;
...
//i.e. sfpa==0, sfpb==1...
int chestnut_impl::get_rx_xg_intf(int channel) {
    std::string sfp = get_rx_sfp(channel);
    int xg_intf = sfp.back() - 'a';
    return xg_intf;
}

std::string chestnut_impl::get_tx_sfp( size_t chan ) {
    if ( chan >= num_tx_channels ) {
        std::string error_msg = CHESTNUT_DEBUG_NAME_S " requested sfp port of non-existant channel: " + std::to_string(chan);
        throw uhd::value_error(error_msg);
    }
    if( is_tx_sfp_cached[chan] ) {
        return tx_sfp_cache[chan];
    } else {
        const fs_path tx_link_path  = CHESTNUT_MB_PATH / "tx_link" / chan;
...
std::string chestnut_impl::get_rx_sfp( size_t chan ) {
...
std::string chestnut_impl::get_tx_ip( size_t chan ) {
    if ( chan >= num_tx_channels ) {
        std::string error_msg = CHESTNUT_DEBUG_NAME_S " requested ip of non-existant channel: " + std::to_string(chan);
        throw uhd::value_error(error_msg);
    }
    if( is_tx_ip_cached[chan] ) {
        ...
        tx_ip_cache[chan] = _tree->access<std::string>( CHESTNUT_MB_PATH / "link" / sfp / "ip_addr").get();
        is_tx_ip_cached[chan] = true;
        return tx_ip_cache[chan];
    }
}

uint16_t chestnut_impl::get_tx_fc_port( size_t chan ) {
    if ( chan >= num_tx_channels ) {
        std::string error_msg = CHESTNUT_DEBUG_NAME_S " requested fc port of non-existant channel: " + std::to_string(chan);
        throw uhd::value_error(error_msg);
    }
    if( is_tx_fc_cached[chan] ) {
        return tx_fc_cache[chan];
    } else {
        const fs_path fc_port_path = CHESTNUT_MB_PATH / ("fpga/board/flow_control/" + get_tx_sfp(chan) + "_port");
...
uint16_t chestnut_impl::get_tx_udp_port( size_t chan ) {
    if ( chan >= num_tx_channels ) {
        std::string error_msg = CHESTNUT_DEBUG_NAME_S " requested udp port of non-existant channel: " + std::to_string(chan);
        throw uhd::value_error(error_msg);
    }
    if( is_tx_udp_port_cached[chan] ) {
        return tx_udp_port_cache[chan];
    } else {
        const fs_path prop_path = CHESTNUT_MB_PATH / "tx_link";
...
void chestnut_impl::get_tx_endpoint( const size_t & chan, std::string & ip_addr, uint16_t & udp_port, std::string & sfp ) {
    sfp = get_tx_sfp(chan);
...
static int select_band( const double freq ) {
	if( freq >= CHESTNUT_MID_HIGH_BARRIER )
        return HIGH_BAND;
    else if( freq >= CHESTNUT_LOW_MID_BARRIER )
        return MID_BAND;
    else
        return LOW_BAND;
}
...
double chestnut_impl::choose_lo_shift( double target_freq, int band, property_tree::sptr dsp_subtree, int xx_sign, size_t chan ) {
    ...
    double band_min_lo;
    if(band == MID_BAND) {
        band_max_lo = CHESTNUT_MID_MAX_LO;
        band_min_lo = CHESTNUT_LOW_MID_BARRIER;
    } else if (band == HIGH_BAND) {
        band_min_lo = CHESTNUT_MID_HIGH_BARRIER;
        band_max_lo = CHESTNUT_FREQ_RANGE_STOP;
    } else {
    ...
    // Preferences:
    // Preferences:
    // 1. have entire relevant range below the lo with CHESTNUT_LO_TARGET_SEPERATION of seperation
    // 2. have entire relevant range above the lo with CHESTNUT_LO_TARGET_SEPERATION of seperation
    ...
    double dsp_bw;
    if(sample_rate > max_sample_rate / CHESTNUT_MAX_DSP_RATE_FACTOR) {
    ...
    if(flag_use_3g_as_1g && RX_SIGN == xx_sign && dsp_bw != 0 && rx_rfe_rate_cache[chan]) {
        compensatory_dsp_shift = CHESTNUT_RX_NCO_SHIFT_3G_TO_1G;
    } else {
    ...
    double a = std::ceil( (relevant_range.stop() + compensatory_dsp_shift + CHESTNUT_LO_TARGET_SEPERATION) / CHESTNUT_LO_STEPSIZE );
    double candidate_1 = a * CHESTNUT_LO_STEPSIZE;
    ...
    double b = std::floor( (relevant_range.start() - compensatory_dsp_shift - CHESTNUT_LO_TARGET_SEPERATION) / CHESTNUT_LO_STEPSIZE );
    double candidate_2 = b * CHESTNUT_LO_STEPSIZE;
    ...
    // Test los that are closer to the target band than CHESTNUT_LO_TARGET_SEPERATION, but still outside the band
    while(a * CHESTNUT_LO_STEPSIZE + ((user_bw / 2.0)) > relevant_range.stop() && b * CHESTNUT_LO_STEPSIZE - (user_bw / 2.0) < relevant_range.start()) {
        a--;
        b++;

        // Test a new lo that is above the target range and slightly closer than the last check
        double candidate_3 = a * CHESTNUT_LO_STEPSIZE;
        ...
        // Test a new lo that is above the target range and slightly closer than the last check
        double candidate_4 = b * CHESTNUT_LO_STEPSIZE;
    ...
    if(flag_use_3g_as_1g && RX_SIGN == xx_sign) {
        return std::max(std::min(::round( (target_freq + CHESTNUT_RX_NCO_SHIFT_3G_TO_1G) / CHESTNUT_LO_STEPSIZE ) * CHESTNUT_LO_STEPSIZE, band_max_lo), (double) band_min_lo);
    } else {
        return std::max(std::min(::round( (target_freq) / CHESTNUT_LO_STEPSIZE ) * CHESTNUT_LO_STEPSIZE, band_max_lo), (double) band_min_lo);
    }
}

// XXX: @CF: 20180418: stop-gap until moved to server
//calculates and sets the band, nco, and lo shift
tune_result_t chestnut_impl::tune_xx_subdev_and_dsp( const double xx_sign, property_tree::sptr dsp_subtree, property_tree::sptr rf_fe_subtree, const tune_request_t &tune_request, int* gain_is_set, int* last_set_band, size_t chan ) {
    ...
    if(actual_rf_freq == 0 && target_rf_freq != 0) {
        UHD_LOG_ERROR(CHESTNUT_DEBUG_NAME_C, "Error when attempting to set lo. The PLL is likely unlocked. Rerun the update package without nolut. If this error persists contact support");
    }
    ...
    if( (tune_request.dsp_freq_policy == tune_request_t::POLICY_MANUAL || tune_request.rf_freq_policy == tune_request_t::POLICY_MANUAL) && target_dsp_freq != 0 && actual_dsp_freq == 0) {
        const double sample_rate = dsp_subtree->access<double>("/rate/value").get();
        if(sample_rate > max_sample_rate / CHESTNUT_MAX_DSP_RATE_FACTOR) {
            UHD_LOGGER_WARNING(CHESTNUT_DEBUG_NAME_C) << "Manual tune request required use of the NCO, but the NCO is disabled at above " << (max_sample_rate / CHESTNUT_MAX_DSP_RATE_FACTOR) / 1e6 << "Msps" ;
        }
    }
...
uhd::tune_result_t chestnut_impl::set_rx_freq(
...
double chestnut_impl::get_rx_freq(size_t chan) {
...
uhd::tune_result_t chestnut_impl::set_tx_freq(
	const uhd::tune_request_t &tune_request, size_t chan
) {
...
double chestnut_impl::get_tx_freq(size_t chan) {
...
void chestnut_impl::set_tx_gain(double gain, const std::string &name, size_t chan){
...
double chestnut_impl::get_tx_gain(const std::string &name, size_t chan) {
    (void) name;

    return _tree->access<double>(tx_rf_fe_root(chan) / "gain" / "value").get();
}

int64_t chestnut_impl::get_tx_buff_scale() {
    return buffer_level_multiple;
}

void chestnut_impl::set_rx_gain(double gain, const std::string &name, size_t chan) {
...
double chestnut_impl::get_rx_gain(const std::string &name, size_t chan) {
    (void) name;

    return _tree->access<double>(rx_rf_fe_root(chan) / "gain" / "value").get();
}

inline void chestnut_impl::request_resync_time_diff() {
    time_resync_requested = true;
}

void chestnut_impl::ping_check(std::string sfp, std::string ip) {
    char cmd[128];
...
double chestnut_impl::get_link_rate() {
    if(link_rate_cache == 0) {
        link_rate_cache = _tree->access<double>(CHESTNUT_MB_PATH / "link_max_rate").get();
