...
class chestnut_impl : public uhd::device
{
public:
    // shared pointer to the Crimson device
    typedef std::shared_ptr<chestnut_impl> sptr;

    // This is the core constructor to be called when a chestnut device is found
    chestnut_impl(const uhd::device_addr_t &, const bool use_dpdk);
    ~chestnut_impl(void);

    // pointers to the streams for the device
    // these functions are defined in chestnut_io_impl.cpp
...
private:
    ...
    static void detect_pps(chestnut_impl *dev);
    ...
    //uhd::chestnut_iface::sptr _iface;
	...
	static void bm_thread_fn( chestnut_impl *dev );

    struct mb_container_type{
        chestnut_iface::sptr iface;
