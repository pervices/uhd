...
public:
	...
	virtual ~cyan_8r_recv_packet_streamer() {
		teardown();
	}
...
static std::vector<std::weak_ptr<cyan_8r_recv_packet_streamer>> allocated_rx_streamers;
static void shutdown_lingering_rx_streamers() {
	...
	for( auto & rx: allocated_rx_streamers ) {
		if ( ! rx.expired() ) {
			std::shared_ptr<cyan_8r_recv_packet_streamer> my_streamer = rx.lock();
			if ( my_streamer ) {
...
public:
	...
	:
		sph::send_packet_streamer( max_num_samps, CYAN_8R_BUFF_SIZE ),
	...
	virtual ~cyan_8r_send_packet_streamer() {
		teardown();
	}
    ...
    size_t send(
        ...
        if ( metadata.start_of_burst ) {
            if ( metadata.time_spec < now + default_sob ) {
                metadata.time_spec = now + default_sob;
                #ifdef UHD_TXRX_DEBUG_PRINTS
                std::cout << "UHD::CYAN_8R::Warning: time_spec was too soon for start of burst and has been adjusted!" << std::endl;
                #endif
            }
            #ifdef UHD_TXRX_DEBUG_PRINTS
            std::cout << "UHD::CYAN_8R::Info: " << get_time_now() << ": sob @ " << metadata.time_spec << " | " << metadata.time_spec.to_ticks( CYAN_8R_DSP_CLOCK_RATE ) << std::endl;
            #endif
      ...
      //  }
        ...
        if ( 0 == nsamps_per_buff && metadata.end_of_burst ) {
            #ifdef UHD_TXRX_DEBUG_PRINTS
            std::cout << "UHD::CYAN_8R::Info: " << now << ": " << "eob @ " << now << " | " << now.to_ticks( CYAN_8R_TICK_RATE ) << std::endl;
            #endif
    ...
    static managed_send_buffer::sptr get_send_buff( std::weak_ptr<uhd::tx_streamer> tx_streamer, const size_t chan, double timeout ){
        ...
        std::shared_ptr<cyan_8r_send_packet_streamer> my_streamer =
            std::dynamic_pointer_cast<cyan_8r_send_packet_streamer>( tx_streamer.lock() );
	...
	void pillage() {
		...
		if ( ! _pillaging ) {
			...
			_pillage_thread = std::thread( cyan_8r_send_packet_streamer::send_viking_loop, this );
			_pillaging = true;
		}
	}
...
private:
     ...
     **********************************************************************/
	static void send_viking_loop( cyan_8r_send_packet_streamer *self ) {
		...
		for( ; ! self->_blessbless; ) {
			...
			for( size_t i = 0; i < self->_eprops.size(); i++ ) {
				...
				if ( (uint64_t)-1 != ep.uflow && uflow != ep.uflow ) {
					// XXX: @CF: 20170905: Eventually we want to return tx channel metadata as VRT49 context packets rather than custom packets. See usrp2/cyan_8r_io_impl.cpp
					// async_metadata_t metadata;
			...
			const long long usloop = 1.0 / (double)CYAN_8R_UPDATE_PER_SEC * 1e6;
...
static std::vector<std::weak_ptr<cyan_8r_send_packet_streamer>> allocated_tx_streamers;
static void shutdown_lingering_tx_streamers() {
	// This is required as a workaround, because the relevent destructurs are not called
	// when you close the top block in gnu radio. Unsolved mystery for the time being.
	for( auto & tx: allocated_tx_streamers ) {
		if ( ! tx.expired() ) {
			std::shared_ptr<cyan_8r_send_packet_streamer> my_streamer = tx.lock();
			if ( my_streamer ) {
				my_streamer->teardown();
			}
		}
 ...
 * - alignment buffer
 **********************************************************************/
struct cyan_8r_impl::cyan_8r_io_impl{

    cyan_8r_io_impl(void):
    ...
    // TODO: @CF: 20180301: move time diff code into cyan_8r_io_impl
};

/***********************************************************************
 * Helper Functions
 **********************************************************************/
void cyan_8r_impl::io_init(void){

	// TODO: @CF: 20180301: move time diff code into cyan_8r_io_impl
	_cyan_8r_io_impl = UHD_PIMPL_MAKE(cyan_8r_io_impl, ());

    //allocate streamer weak ptrs containers
    for (const std::string &mb : _mbc.keys()) {
        _mbc[mb].rx_streamers.resize( CYAN_8R_RX_CHANNELS );
        _mbc[mb].tx_streamers.resize( CYAN_8R_TX_CHANNELS );
    }
}

void cyan_8r_impl::update_rx_samp_rate(const std::string &mb, const size_t dsp, const double rate_){

    set_double( "rx_" + std::string( 1, 'a' + dsp ) + "/dsp/rate", rate_ );
    double rate = get_double( "rx_" + std::string( 1, 'a' + dsp ) + "/dsp/rate" );

    std::shared_ptr<cyan_8r_recv_packet_streamer> my_streamer =
        std::dynamic_pointer_cast<cyan_8r_recv_packet_streamer>(_mbc[mb].rx_streamers[dsp].lock());
...
void cyan_8r_impl::update_tx_samp_rate(const std::string &mb, const size_t dsp, const double rate_ ){
	...
	std::shared_ptr<cyan_8r_send_packet_streamer> my_streamer =
        std::dynamic_pointer_cast<cyan_8r_send_packet_streamer>(_mbc[mb].tx_streamers[dsp].lock());
    if (my_streamer.get() == NULL) return;
...
void cyan_8r_impl::update_rx_subdev_spec(const std::string &which_mb, const subdev_spec_t &spec){
...
void cyan_8r_impl::update_tx_subdev_spec(const std::string &which_mb, const subdev_spec_t &spec){
    fs_path root = "/mboards/" + which_mb + "/dboards";
 ...
 * Async Data
 **********************************************************************/
bool cyan_8r_impl::recv_async_msg(
    async_metadata_t &async_metadata, double timeout
){
    boost::this_thread::disable_interruption di; //disable because the wait can throw
    return _cyan_8r_io_impl->async_msg_fifo.pop_with_timed_wait(async_metadata, timeout);
}
...
rx_streamer::sptr cyan_8r_impl::get_rx_stream(const uhd::stream_args_t &args_){
    ...
    std::shared_ptr<cyan_8r_recv_packet_streamer> my_streamer = std::make_shared<cyan_8r_recv_packet_streamer>(spp);
...
tx_streamer::sptr cyan_8r_impl::get_tx_stream(const uhd::stream_args_t &args_){
    stream_args_t args = args_;
    ...
    std::shared_ptr<cyan_8r_send_packet_streamer> my_streamer = std::make_shared<cyan_8r_send_packet_streamer>( spp );
