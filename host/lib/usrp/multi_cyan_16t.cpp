...
#include <uhd/usrp/multi_cyan_16t.hpp>
...
#include "cyan_16t/cyan_16t_fw_common.h"
#include "cyan_16t/cyan_16t_impl.hpp"
...
double multi_cyan_16t::choose_dsp_nco_shift( double target_freq, property_tree::sptr dsp_subtree ) {
...
tune_result_t multi_cyan_16t::tune_lo_and_dsp( const double xx_sign, property_tree::sptr dsp_subtree, property_tree::sptr rf_fe_subtree, const tune_request_t &tune_request ) {
 ...
 **********************************************************************/
multi_cyan_16t::multi_cyan_16t(const device_addr_t &addr) {
    // this make will invoke the correct inherited crimson device class
    _dev  = device::make(addr, device::CYAN_16T);
    _tree = _dev  -> get_tree();
}

multi_cyan_16t::~multi_cyan_16t(void) {
    // do nothing
}

device::sptr multi_cyan_16t::get_device(void){
    return _dev;
}
...
dict<std::string, std::string> multi_cyan_16t::get_usrp_rx_info(size_t chan) {
	(void)chan;
...
dict<std::string, std::string> multi_cyan_16t::get_usrp_tx_info(size_t chan) {
	(void) chan;
...
void multi_cyan_16t::set_master_clock_rate(double rate, size_t mboard){
	(void)rate;
	(void)mboard;
    return;
}

double multi_cyan_16t::get_master_clock_rate(size_t mboard){
	(void)mboard;
    return _tree->access<double>(mb_root(0) / "tick_rate").get();
}

std::string multi_cyan_16t::get_pp_string(void){
	return "Crimson TNG";
}

// Get the Digital board name
std::string multi_cyan_16t::get_mboard_name(size_t mboard){
	(void)mboard;
    return _tree->access<std::string>(mb_root(0) / "name").get();
}

// Get the current time on Crimson
time_spec_t multi_cyan_16t::get_time_now(size_t mboard){
	(void)mboard;
	return _tree->access<time_spec_t>(mb_root(0) / "time/now").get();
}

// Get the time of the last PPS (pulse per second)
time_spec_t multi_cyan_16t::get_time_last_pps(size_t mboard){
	(void)mboard;
    return _tree->access<time_spec_t>(mb_root(0) / "time/pps").get();
}

// Set the current time on Crimson
void multi_cyan_16t::set_time_now(const time_spec_t &time_spec, size_t mboard){
	(void)mboard;
...
void multi_cyan_16t::set_time_next_pps(const time_spec_t &time_spec, size_t mboard){
	(void)mboard;
    _tree->access<time_spec_t>(mb_root(0) / "time/pps").set(time_spec);
    return;
}

void multi_cyan_16t::set_time_unknown_pps(const time_spec_t &time_spec){
	set_time_now( time_spec );
}

bool multi_cyan_16t::get_time_synchronized(void){
    // Not implemented
    //throw uhd::not_implemented_error("timed command feature not implemented on this hardware");
    return true;
}
void multi_cyan_16t::set_command_time(const time_spec_t &time_spec, size_t mboard){
	(void)time_spec;
	(void)mboard;
    // Not implemented
    //throw uhd::not_implemented_error("timed command feature not implemented on this hardware");
...
void multi_cyan_16t::clear_command_time(size_t mboard){
	(void)mboard;
    // Not implemented
    //throw uhd::not_implemented_error("timed command feature not implemented on this hardware");
    //return;
...
void multi_cyan_16t::issue_stream_cmd(const stream_cmd_t &stream_cmd, size_t chan){
    if (chan != ALL_CHANS){
...
void multi_cyan_16t::set_clock_config(const clock_config_t &clock_config, size_t mboard) {
	(void)mboard;
...
void multi_cyan_16t::set_time_source(const std::string &source, const size_t mboard){
	(void)mboard;
...
std::string multi_cyan_16t::get_time_source(const size_t mboard){
	(void)mboard;
    return _tree->access<std::string>(mb_root(0) / "time_source" / "value").get();
}

// get all possible time sources
std::vector<std::string> multi_cyan_16t::get_time_sources(const size_t mboard){
	(void)mboard;
...
void multi_cyan_16t::set_clock_source(const std::string &source, const size_t mboard){
	(void)source;
	(void)mboard;
...
std::string multi_cyan_16t::get_clock_source(const size_t mboard){
	(void)mboard;
    return _tree->access<std::string>(mb_root(0) / "clock_source" / "value").get();
}

// get all possible clock sources
std::vector<std::string> multi_cyan_16t::get_clock_sources(const size_t mboard){
	(void)mboard;
...
void multi_cyan_16t::set_clock_source_out(const bool enb, const size_t mboard){
	(void)enb;
	(void)mboard;
    // Not supported
    throw uhd::runtime_error("multi_cyan_16t::set_clock_source_out - not supported on this device");
}

// Set the time source output, Crimson doesn't have an SMA for time out
void multi_cyan_16t::set_time_source_out(const bool enb, const size_t mboard){
	(void)enb;
	(void)mboard;
    // Not supported
    throw uhd::runtime_error("multi_cyan_16t::set_time_source_out - not supported on this device");
}

// Crimson only has support for 1 Digital (mboard) board.
size_t multi_cyan_16t::get_num_mboards(void){
    return 1;
}

sensor_value_t multi_cyan_16t::get_mboard_sensor(const std::string &name, size_t mboard){
	(void)mboard;
    return _tree->access<sensor_value_t>(mb_root(0) / "sensors" / name).get();
}

std::vector<std::string> multi_cyan_16t::get_mboard_sensor_names(size_t mboard){
	(void)mboard;
    return _tree->list(mb_root(0) / "sensors");
}

void multi_cyan_16t::set_user_register(const boost::uint8_t addr, const boost::uint32_t data, size_t mboard){
	(void)addr;
	(void)data;
	(void)mboard;
    throw uhd::not_implemented_error("timed command feature not implemented on this hardware");
 ...
 * RX methods
 ******************************************************************/
rx_streamer::sptr multi_cyan_16t::get_rx_stream(const stream_args_t &args) {
...
void multi_cyan_16t::set_rx_subdev_spec(const subdev_spec_t &spec, size_t mboard){
	(void)spec;
	(void)mboard;
    throw uhd::runtime_error("multi_cyan_16t::set_tx_subdev_spec - not supported on this device");
}

// Get the current RX chain subdev properties, Crimson only has one mboard
subdev_spec_t multi_cyan_16t::get_rx_subdev_spec(size_t mboard){
	(void)mboard;
#ifdef PV_TATE
...
size_t multi_cyan_16t::get_rx_num_channels(void){
    return CYAN_16T_RX_CHANNELS;
}

// Get the name of the Crimson subdevice on specified channel
std::string multi_cyan_16t::get_rx_subdev_name(size_t chan){
    return "Channel " + boost::lexical_cast<std::string>(chan);
}

// Set the current RX sampling rate on specified channel
void multi_cyan_16t::set_rx_rate(double rate, size_t chan){
	if (chan != ALL_CHANS){
...
double multi_cyan_16t::get_rx_freq(size_t chan){
...
freq_range_t multi_cyan_16t::get_rx_freq_range(size_t chan){
    return _tree->access<meta_range_t>(rx_dsp_root(chan) / "freq" / "range").get();
}

// get front end RX frequency on specified channel
freq_range_t multi_cyan_16t::get_fe_rx_freq_range(size_t chan){
    return _tree->access<meta_range_t>(rx_rf_fe_root(chan) / "freq" / "range").get();
}

std::vector<std::string> multi_cyan_16t::get_rx_lo_names(size_t chan) {
...
void multi_cyan_16t::set_rx_lo_source(const std::string &src, const std::string &name, size_t chan) {
    if (_tree->exists(rx_rf_fe_root(chan) / "los")) {
        if (name == ALL_LOS) {
...
const std::string multi_cyan_16t::get_rx_lo_source(const std::string &name, size_t chan) {
    if (_tree->exists(rx_rf_fe_root(chan) / "los")) {
        if (name == ALL_LOS) {
                //Special value ALL_LOS support atomically sets the source for all LOs
...
std::vector<std::string> multi_cyan_16t::get_rx_lo_sources(const std::string &name, size_t chan) {
    if (_tree->exists(rx_rf_fe_root(chan) / "los")) {
        if (name == ALL_LOS) {
            if (_tree->exists(rx_rf_fe_root(chan) / "los" / ALL_LOS)) {
                //Special value ALL_LOS support atomically sets the source for all LOs
...
void multi_cyan_16t::set_rx_lo_export_enabled(bool enabled, const std::string &name, size_t chan) {
    if (_tree->exists(rx_rf_fe_root(chan) / "los")) {
        if (name == ALL_LOS) {
            if (_tree->exists(rx_rf_fe_root(chan) / "los" / ALL_LOS)) {
                //Special value ALL_LOS support atomically sets the source for all LOs
...
bool multi_cyan_16t::get_rx_lo_export_enabled(const std::string &name, size_t chan) {
	(void)name;
	(void)chan;
	throw uhd::runtime_error("This device does not support manual configuration of LOs");
}
double multi_cyan_16t::set_rx_lo_freq(double freq, const std::string &name, size_t chan) {
	(void)freq;
	(void)name;
	(void)chan;
	throw uhd::runtime_error("This device does not support manual configuration of LOs");
}
double multi_cyan_16t::get_rx_lo_freq(const std::string &name, size_t chan) {
	(void)name;
	(void)chan;
	throw uhd::runtime_error("This device does not support manual configuration of LOs");
}
freq_range_t multi_cyan_16t::get_rx_lo_freq_range(const std::string &name, size_t chan) {
	(void)name;
	(void)chan;
	throw uhd::runtime_error("This device does not support manual configuration of LOs");
}
// set RX frontend gain on specified channel, name specifies which IC to configure the gain for
void multi_cyan_16t::set_rx_gain(double gain, const std::string &name, size_t chan) {
	...
	if ( ALL_CHANS != chan ) {
		...
		gain = gain < CYAN_16T_RF_RX_GAIN_RANGE_START ? CYAN_16T_RF_RX_GAIN_RANGE_START : gain;
		gain = gain > CYAN_16T_RF_RX_GAIN_RANGE_STOP ? CYAN_16T_RF_RX_GAIN_RANGE_STOP : gain;
		...
		if ( 0 == _tree->access<int>(rx_rf_fe_root(chan) / "freq" / "band").get() ) {
			...
			if ( false ) {
			} else if ( CYAN_16T_RF_RX_GAIN_RANGE_START <= gain && gain <= 31.5 ) {
				// PMA is off (+0dB)
				lna_val = 0;
				// BFP is on (+20dB)
				...
				// LMH is maxed (+31.5dB)
				gain_val = 31.5;
			} else if ( 63.25 < gain && gain <= CYAN_16T_RF_RX_GAIN_RANGE_STOP ) {
				// PMA is on (+20dB)
				lna_val = 20;
				// BFP is on (+20dB)
				// PE437 is adjusted from -20 dB to 0dB
				atten_val = CYAN_16T_RF_RX_GAIN_RANGE_STOP - gain;
				// LMH is maxed (+31.5dB)
				gain_val = 31.5;
			}
		}
...
void multi_cyan_16t::set_normalized_rx_gain(double gain, size_t chan )
{
...
void multi_cyan_16t::set_rx_agc(bool enable, size_t chan) {
	(void) enable;
	(void) chan;
...
double multi_cyan_16t::get_rx_gain(const std::string &name, size_t chan){
...
double multi_cyan_16t::get_normalized_rx_gain(size_t chan) {
	(void) chan;
	throw uhd::runtime_error("This device does not support normalized rx gain");
}

// get RX frontend gain range on specified channel
gain_range_t multi_cyan_16t::get_rx_gain_range(const std::string &name, size_t chan){
	try {
		if ( ALL_GAINS != name ) {
...
std::vector<std::string> multi_cyan_16t::get_rx_gain_names(size_t chan){
	(void)chan;
    throw uhd::runtime_error("multi_cyan_16t::get_rx_gain_names - not supported on this device");
}

// Crimson does not cater to antenna specifications, this is up to the user to accomodate for
void multi_cyan_16t::set_rx_antenna(const std::string &ant, size_t chan){
	(void)ant;
	(void)chan;
    throw uhd::runtime_error("multi_cyan_16t::set_rx_antenna - not supported on this device");
}

// Crimson does not cater to antenna specifications, this is up to the user to accomodate for
std::string multi_cyan_16t::get_rx_antenna(size_t chan){
	(void)chan;
    throw uhd::runtime_error("multi_cyan_16t::get_rx_antenna - not supported on this device");
}

// Crimson does not cater to antenna specifications, this is up to the user to accomodate for
std::vector<std::string> multi_cyan_16t::get_rx_antennas(size_t chan){
	(void)chan;
    throw uhd::runtime_error("multi_cyan_16t::get_rx_antennas - not supported on this device");
}

// Set the RX bandwidth on specified channel
void multi_cyan_16t::set_rx_bandwidth(double bandwidth, size_t chan){
    _tree->access<double>(rx_dsp_root(chan) / "bw" / "value").set(bandwidth);
}
// Get the RX bandwidth on specified channel
double multi_cyan_16t::get_rx_bandwidth(size_t chan){
    return _tree->access<double>(rx_dsp_root(chan) / "bw" / "value").get();
}
// Get the RX bandwidth range on specified channel
meta_range_t multi_cyan_16t::get_rx_bandwidth_range(size_t chan){
...
dboard_iface::sptr multi_cyan_16t::get_rx_dboard_iface(size_t chan){
	(void)chan;
    throw uhd::runtime_error("multi_cyan_16t::get_rx_dboard_iface - not supported on this device");
}

sensor_value_t multi_cyan_16t::get_rx_sensor(const std::string &name, size_t chan){
	(void)chan;
    return _tree->access<sensor_value_t>(rx_rf_fe_root(0) / "sensors" / name).get();
}

std::vector<std::string> multi_cyan_16t::get_rx_sensor_names(size_t chan){
	(void)chan;
    return _tree->list(rx_rf_fe_root(0) / "sensors");
}

// Enable dc offset on specified channel
void multi_cyan_16t::set_rx_dc_offset(const bool enb, size_t chan){
    _tree->access<bool>(mb_root(0) / "rx_frontends" / chan_to_string(chan) / "dc_offset" / "enable").set(enb);
}
// Set dc offset on specified channel
void multi_cyan_16t::set_rx_dc_offset(const std::complex<double> &offset, size_t chan){
    _tree->access< std::complex<double> >(mb_root(0) / "rx_frontends" / chan_to_string(chan) / "dc_offset" / "value").set(offset);
}
void multi_cyan_16t::set_rx_iq_balance(const bool enb, size_t chan) {
    if (chan != ALL_CHANS){
...
void multi_cyan_16t::set_rx_iq_balance(const std::complex<double> &offset, size_t chan){
    _tree->access< std::complex<double> >(mb_root(0) / "rx_frontends" / chan_to_string(chan) / "iq_balance" / "value").set(offset);
}

void multi_cyan_16t::set_rx_gain_profile(const std::string& profile, const size_t chan){
    if (chan != ALL_CHANS) {
...
std::string multi_cyan_16t::get_rx_gain_profile(const size_t chan)
{
    if (chan != ALL_CHANS) {
        if (_tree->exists(rx_rf_fe_root(chan) / "gains/all/profile/value")) {
...
std::vector<std::string> multi_cyan_16t::get_rx_gain_profile_names(const size_t chan)
{
    if (chan != ALL_CHANS) {
 ...
 * TX methods
 ******************************************************************/
tx_streamer::sptr multi_cyan_16t::get_tx_stream(const stream_args_t &args) {
    return this->get_device()->get_tx_stream(args);
}

// Crimson does not support changing subdev properties because you can't add daughter boards
void multi_cyan_16t::set_tx_subdev_spec(const subdev_spec_t &spec, size_t mboard){
	(void)spec;
	(void)mboard;
    throw uhd::runtime_error("multi_cyan_16t::set_tx_subdev_spec - not supported on this device");
}
...
subdev_spec_t multi_cyan_16t::get_tx_subdev_spec(size_t mboard){
	(void)mboard;
#ifdef PV_TATE
...
size_t multi_cyan_16t::get_tx_num_channels(void){
    return CYAN_16T_TX_CHANNELS;
}

// Get the name of the Crimson subdevice on specified channel
std::string multi_cyan_16t::get_tx_subdev_name(size_t chan){
    return "Channel " + boost::lexical_cast<std::string>(chan);
}

// Set the current TX sampling rate on specified channel
void multi_cyan_16t::set_tx_rate(double rate, size_t chan){
	if (chan != ALL_CHANS){
...
double multi_cyan_16t::get_tx_freq(size_t chan){
...
// get the TX frequency on specified channel
freq_range_t multi_cyan_16t::get_tx_freq_range(size_t chan){
    return _tree->access<meta_range_t>(tx_dsp_root(chan) / "freq" / "range").get();
}

// get the TX frequency range on specified channel
freq_range_t multi_cyan_16t::get_fe_tx_freq_range(size_t chan){
...
void multi_cyan_16t::set_tx_gain(double gain, const std::string &name, size_t chan){
...
void multi_cyan_16t::set_tx_gain_profile(const std::string& profile, const size_t chan){
    if (chan != ALL_CHANS) {
...
std::string multi_cyan_16t::get_tx_gain_profile(const size_t chan)
{
    if (chan != ALL_CHANS) {
        if (_tree->exists(tx_rf_fe_root(chan) / "gains/all/profile/value")) {
            return _tree->access<std::string>(
...
std::vector<std::string> multi_cyan_16t::get_tx_gain_profile_names(const size_t chan)
{
    if (chan != ALL_CHANS) {
...
void multi_cyan_16t::set_normalized_tx_gain(double gain, size_t chan) {
    if (gain > 1.0 || gain < 0.0) {
      throw uhd::runtime_error("Normalized gain out of range, must be in [0, 1].");
    }
...
double multi_cyan_16t::get_tx_gain(const std::string &name, size_t chan){
	(void)name;
    double gain_val = _tree->access<double>(tx_rf_fe_root(chan) / "gain" / "value").get();
    return gain_val * 0.25;
}

double multi_cyan_16t::get_normalized_tx_gain(size_t chan) {
    gain_range_t gain_range = get_tx_gain_range(ALL_GAINS, chan);
...
gain_range_t multi_cyan_16t::get_tx_gain_range(const std::string &name, size_t chan){
	(void)name;
...
std::vector<std::string> multi_cyan_16t::get_tx_gain_names(size_t chan){
	(void)chan;
    throw uhd::runtime_error("multi_cyan_16t::get_tx_gain_names - not supported on this device");
}

// Crimson does not cater to antenna specifications, this is up to the user to accomodate for
void multi_cyan_16t::set_tx_antenna(const std::string &ant, size_t chan){
	(void)ant;
	(void)chan;
    throw uhd::runtime_error("multi_cyan_16t::set_tx_antenna - not supported on this device");
}

// Crimson does not cater to antenna specifications, this is up to the user to accomodate for
std::string multi_cyan_16t::get_tx_antenna(size_t chan){
	(void)chan;
    throw uhd::runtime_error("multi_cyan_16t::get_tx_antenna - not supported on this device");
}

// Crimson does not cater to antenna specifications, this is up to the user to accomodate for
std::vector<std::string> multi_cyan_16t::get_tx_antennas(size_t chan){
	(void)chan;
    throw uhd::runtime_error("multi_cyan_16t::get_tx_antennas - not supported on this device");
}

// Set the TX bandwidth on specified channel
void multi_cyan_16t::set_tx_bandwidth(double bandwidth, size_t chan){
    _tree->access<double>(tx_dsp_root(chan) / "bw" / "value").set(bandwidth);
}

// Get the TX bandwidth on specified channel
double multi_cyan_16t::get_tx_bandwidth(size_t chan){
    return _tree->access<double>(tx_dsp_root(chan) / "bw" / "value").get();
}

// Get the TX bandwidth range on specified channel
meta_range_t multi_cyan_16t::get_tx_bandwidth_range(size_t chan){
    return _tree->access<meta_range_t>(tx_dsp_root(chan) / "bw" / "range").get();
}

// There is no dboard interface available for Crimson. Everything is communicated through the mboard (Digital board)
dboard_iface::sptr multi_cyan_16t::get_tx_dboard_iface(size_t chan){
	(void)chan;
    throw uhd::runtime_error("multi_cyan_16t::get_tx_dboard_iface - not supported on this device");
}

sensor_value_t multi_cyan_16t::get_tx_sensor(const std::string &name, size_t chan){
    return _tree->access<sensor_value_t>(tx_rf_fe_root(chan) / "sensors" / name).get();
}

std::vector<std::string> multi_cyan_16t::get_tx_sensor_names(size_t chan){
    return _tree->list(tx_rf_fe_root(chan) / "sensors");
}

// Set dc offset on specified channel
void multi_cyan_16t::set_tx_dc_offset(const std::complex<double> &offset, size_t chan){
    _tree->access< std::complex<double> >(tx_rf_fe_root(chan) / "dc_offset" / "value").set(offset);
}

// set iq balance on specified channel
void multi_cyan_16t::set_tx_iq_balance(const std::complex<double> &offset, size_t chan){
    _tree->access< std::complex<double> >(tx_rf_fe_root(chan) / "iq_balance" / "value").set(offset);
}

std::vector<std::string> multi_cyan_16t::get_tx_lo_names(const size_t chan){
    std::vector<std::string> lo_names;
...
void multi_cyan_16t::set_tx_lo_source(
...
const std::string multi_cyan_16t::get_tx_lo_source(
        const std::string &name,
        const size_t chan
) {
    if (_tree->exists(tx_rf_fe_root(chan) / "los")) {
...
std::vector<std::string> multi_cyan_16t::get_tx_lo_sources(
        const std::string &name,
        const size_t chan
) {
    if (_tree->exists(tx_rf_fe_root(chan) / "los")) {
...
void multi_cyan_16t::set_tx_lo_export_enabled(
        const bool enabled,
        const std::string &name,
        const size_t chan
) {
...
bool multi_cyan_16t::get_tx_lo_export_enabled(
        const std::string &name,
        const size_t chan
) {
    if (_tree->exists(tx_rf_fe_root(chan) / "los")) {
...
double multi_cyan_16t::set_tx_lo_freq(
        const double freq,
        const std::string &name,
        const size_t chan
) {
...
double multi_cyan_16t::get_tx_lo_freq(
        const std::string &name,
        const size_t chan
) {
    if (_tree->exists(tx_rf_fe_root(chan) / "los")) {
...
freq_range_t multi_cyan_16t::get_tx_lo_freq_range(
        const std::string &name,
        const size_t chan
) {
    if (_tree->exists(tx_rf_fe_root(chan) / "los")) {
 ...
 * GPIO methods
 ******************************************************************/
std::vector<std::string> multi_cyan_16t::get_gpio_banks(const size_t mboard){
	(void)mboard;
    // Not supported
    throw uhd::runtime_error("multi_cyan_16t::get_gpio_banks - not supported on this device");
}

void multi_cyan_16t::set_gpio_attr(const std::string &bank, const std::string &attr,
	...
	(void)mboard;
    // Not supported
    throw uhd::runtime_error("multi_cyan_16t::set_gpio_attr - not supported on this device");
}
void multi_cyan_16t::set_gpio_attr(const std::string &bank, const std::string &attr, const std::string &str_value , const uint32_t mask, const size_t mboard)
{
	(void)bank;
	(void)attr;
	(void)str_value;
...
uint32_t multi_cyan_16t::get_gpio_attr(const std::string &bank, const std::string &attr, const size_t mboard)
{
	(void)bank;
	(void)attr;
	(void)mboard;
...
std::vector<std::string> multi_cyan_16t::get_gpio_string_attr(const std::string &bank, const std::string &attr, const size_t mboard)
{
	(void)bank;
	(void)attr;
	(void)mboard;
...
std::vector<std::string> multi_cyan_16t::enumerate_registers(const size_t mboard) {
...
multi_usrp::register_info_t multi_cyan_16t::get_register_info(const std::string &path, const size_t mboard) {
    if (_tree->exists(mb_root(mboard) / "registers"))
    {
        uhd::soft_regmap_accessor_t::sptr accessor =
            _tree->access<uhd::soft_regmap_accessor_t::sptr>(mb_root(mboard) / "registers").get();
...
void multi_cyan_16t::write_register(const std::string &path, const uint32_t field, const uint64_t value, const size_t mboard) {
    if (_tree->exists(mb_root(mboard) / "registers"))
    {
        uhd::soft_regmap_accessor_t::sptr accessor =
            _tree->access<uhd::soft_regmap_accessor_t::sptr>(mb_root(mboard) / "registers").get();
...
uint64_t multi_cyan_16t::read_register(const std::string &path, const uint32_t field, const size_t mboard) {
    if (_tree->exists(mb_root(mboard) / "registers"))
    {
        uhd::soft_regmap_accessor_t::sptr accessor =
            _tree->access<uhd::soft_regmap_accessor_t::sptr>(mb_root(mboard) / "registers").get();
...
std::vector<std::string> multi_cyan_16t::get_filter_names(const std::string &search_mask) {
...
filter_info_base::sptr multi_cyan_16t::get_filter(const std::string &path) {
...
void multi_cyan_16t::set_filter(const std::string &path, filter_info_base::sptr filter) {
    std::vector<std::string> possible_names = get_filter_names("");
    std::vector<std::string>::iterator it;
    it = find(possible_names.begin(), possible_names.end(), path);
    if (it == possible_names.end()) {
 ...
 ******************************************************************/
// Getting FS paths
fs_path multi_cyan_16t::mb_root(const size_t mboard)
{
    try
    {
...
fs_path multi_cyan_16t::rx_rf_fe_root(const size_t chan) {
    size_t channel;
...
fs_path multi_cyan_16t::rx_dsp_root(const size_t chan) {
    size_t channel;
    if (chan > CRIMSON_RX_CHANNELS) 	channel = 0;
    else				channel = chan;
...
fs_path multi_cyan_16t::rx_link_root(const size_t chan) {
    size_t channel;
    if (chan > CRIMSON_RX_CHANNELS) 	channel = 0;
    else				channel = chan;
...
fs_path multi_cyan_16t::tx_rf_fe_root(const size_t chan) {
    size_t channel;
...
fs_path multi_cyan_16t::tx_dsp_root(const size_t chan) {
    size_t channel;
    if (chan > CRIMSON_TX_CHANNELS) 	channel = 0;
    else				channel = chan;
...
fs_path multi_cyan_16t::tx_link_root(const size_t chan) {
    size_t channel;
    if (chan > CRIMSON_TX_CHANNELS) 	channel = 0;
    else				channel = chan;
...
fs_path multi_cyan_16t::cm_root() {
    return mb_root(0) / "cm";
}

// Channel string handling
std::string multi_cyan_16t::chan_to_string(size_t chan) {
    return "Channel_" + boost::lexical_cast<std::string>((char)(chan + 65));
}

std::string multi_cyan_16t::chan_to_alph(size_t chan) {
