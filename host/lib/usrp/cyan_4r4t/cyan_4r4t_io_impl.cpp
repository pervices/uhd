...
public:
	...
	cyan_4r4t_recv_packet_streamer(const size_t max_num_samps)
	: sph::recv_packet_streamer( max_num_samps )
	{
        _max_num_samps = max_num_samps;
    }

	virtual ~cyan_4r4t_recv_packet_streamer() {
		teardown();
	}
...
static std::vector<std::weak_ptr<cyan_4r4t_recv_packet_streamer>> allocated_rx_streamers;
static void shutdown_lingering_rx_streamers() {
	...
	for( auto & rx: allocated_rx_streamers ) {
		if ( ! rx.expired() ) {
			std::shared_ptr<cyan_4r4t_recv_packet_streamer> my_streamer = rx.lock();
			if ( my_streamer ) {
...
public:
	...
	typedef std::function<void(void)> onfini_type;
	typedef std::function<uhd::time_spec_t(int&)> timenow_type;
	...
	:
		sph::send_packet_streamer( max_num_samps, CYAN_4R4T_BUFF_SIZE ),
	...
	virtual ~cyan_4r4t_send_packet_streamer() {
		teardown();
	}
    ...
    size_t send(
        ...
        size_t nsamps_per_buff = ((size_t)(nsamps_per_buff_/CYAN_4R4T_PACKET_NSAMP_MULTIPLE)) * CYAN_4R4T_PACKET_NSAMP_MULTIPLE;

#ifdef UHD_TXRX_DEBUG_PRINTS
        if(nsamps_per_buff_ != nsamps_per_buff) {
            std::cout << "Warning: the number of samples attempted must be a multiple of " << CYAN_4R4T_PACKET_NSAMP_MULTIPLE << ", this will call will only attempt to send " << nsamps_per_buff << std::endl;
        }
#endif
    ...
    static managed_send_buffer::sptr get_send_buff( std::weak_ptr<uhd::tx_streamer> tx_streamer, const size_t chan, double timeout ){
        ...
        std::shared_ptr<cyan_4r4t_send_packet_streamer> my_streamer =
            std::dynamic_pointer_cast<cyan_4r4t_send_packet_streamer>( tx_streamer.lock() );
...
private:
     ...
     **********************************************************************/
	static void send_viking_loop( cyan_4r4t_send_packet_streamer *self ) {
		...
		for( ; ! self->_stop_streaming; ) {
			...
			const long long usloop = 1.0 / (double)CYAN_4R4T_UPDATE_PER_SEC * 1e6;
...
static std::vector<std::weak_ptr<cyan_4r4t_send_packet_streamer>> allocated_tx_streamers;
static void shutdown_lingering_tx_streamers() {
	// This is required as a workaround, because the relevent destructurs are not called
	// when you close the top block in gnu radio. Unsolved mystery for the time being.
	for( auto & tx: allocated_tx_streamers ) {
		if ( ! tx.expired() ) {
			std::shared_ptr<cyan_4r4t_send_packet_streamer> my_streamer = tx.lock();
			if ( my_streamer ) {
				my_streamer->teardown();
			}
		}
...
void cyan_4r4t_impl::io_init(void){
    ...
    for (const std::string &mb : _mbc.keys()) {
        _mbc[mb].rx_streamers.resize( CYAN_4R4T_RX_CHANNELS );
        _mbc[mb].tx_streamers.resize( CYAN_4R4T_TX_CHANNELS );
    }
}

void cyan_4r4t_impl::update_rx_samp_rate(const std::string &mb, const size_t dsp, const double rate_){

    set_double( "rx_" + std::string( 1, 'a' + dsp ) + "/dsp/rate", rate_ );
    double rate = get_double( "rx_" + std::string( 1, 'a' + dsp ) + "/dsp/rate" );

    std::shared_ptr<cyan_4r4t_recv_packet_streamer> my_streamer =
        std::dynamic_pointer_cast<cyan_4r4t_recv_packet_streamer>(_mbc[mb].rx_streamers[dsp].lock());
    if (my_streamer.get() == NULL) return;

    my_streamer->set_samp_rate(rate);
    my_streamer->set_tick_rate( CYAN_4R4T_TICK_RATE );
}
...
void cyan_4r4t_impl::update_tx_samp_rate(const std::string &mb, const size_t dsp, const double rate_ ){
	...
	std::shared_ptr<cyan_4r4t_send_packet_streamer> my_streamer =
        std::dynamic_pointer_cast<cyan_4r4t_send_packet_streamer>(_mbc[mb].tx_streamers[dsp].lock());
    if (my_streamer.get() == NULL) return;
...
void cyan_4r4t_impl::update_tx_subdev_spec(const std::string &which_mb, const subdev_spec_t &spec){
    fs_path root = "/mboards/" + which_mb + "/dboards";
 ...
 * Async Data
 **********************************************************************/
bool cyan_4r4t_impl::recv_async_msg(
 ...
 * Receive streamer
 **********************************************************************/
rx_streamer::sptr cyan_4r4t_impl::get_rx_stream(const uhd::stream_args_t &args_){
    ...
    std::shared_ptr<cyan_4r4t_recv_packet_streamer> my_streamer = std::make_shared<cyan_4r4t_recv_packet_streamer>(spp);
        ...
        for (const std::string &mb : _mbc.keys()) {
            ...
            if (chan < num_chan_so_far){
                ...
                my_streamer->set_issue_stream_cmd(chan_i, std::bind(
                    &cyan_4r4t_impl::set_stream_cmd, this, scmd_pre, ph::_1));
        ...
        for (const std::string &mb : _mbc.keys()) {
            ...
            if (chan < num_chan_so_far){
                ...
                if(jesd_status.compare(0, 4, "good")) {
                    UHD_LOGGER_WARNING(CYAN_4R4T_DEBUG_NAME_C) << "rx " << ch << ": unable to establish JESD link. This streamer will not work." << std::endl;
                }
            }
        }
    }
...
tx_streamer::sptr cyan_4r4t_impl::get_tx_stream(const uhd::stream_args_t &args_){
    stream_args_t args = args_;
    ...
    std::shared_ptr<cyan_4r4t_send_packet_streamer> my_streamer = std::make_shared<cyan_4r4t_send_packet_streamer>( spp );
    ...
    // The streamer's set_time_now function sets the function to be used for getting time, the device's set_time_now function sets the time
    my_streamer->set_time_now(std::bind(&cyan_4r4t_impl::get_time_now, this, ph::_1));
    ...
    for (size_t chan_i = 0; chan_i < args.channels.size(); chan_i++){
        ...
        for (const std::string &mb : _mbc.keys()) {
            ...
            if (chan < num_chan_so_far){
                ...
                my_streamer->set_xport_chan_get_buff(chan_i, std::bind(
                    &cyan_4r4t_send_packet_streamer::get_send_buff, my_streamerp, chan_i, ph::_1
                ));
                ...
                my_streamer->set_xport_chan_update_fc_send_size(chan_i, std::bind(
                    &cyan_4r4t_send_packet_streamer::update_fc_send_count, my_streamerp, chan_i, ph::_1
                ));
                
                my_streamer->set_xport_chan_check_flow_control(chan_i, std::bind(
                    &cyan_4r4t_send_packet_streamer::check_flow_control, my_streamerp, chan_i, ph::_1
                ));
                ...
                my_streamer->set_async_receiver(std::bind(&bounded_buffer<async_metadata_t>::pop_with_timed_wait, &(_cyan_4r4t_io_impl->async_msg_fifo), ph::_1, ph::_2));

                my_streamer->set_async_pusher(std::bind(&bounded_buffer<async_metadata_t>::push_with_pop_on_full, &(_cyan_4r4t_io_impl->async_msg_fifo), ph::_1));
    ...
    for (size_t chan_i = 0; chan_i < args.channels.size(); chan_i++){
        ...
        for (const std::string &mb : _mbc.keys()) {
            ...
            if (chan < num_chan_so_far){
                ...
                if(jesd_status.compare(0, 4, "good")) {
                    UHD_LOGGER_WARNING(CYAN_4R4T_DEBUG_NAME_C) << "tx " << ch << ": unable to establish JESD link. This streamer will not work." << std::endl;
                }
            }
        }
    }
