...
#define TREE_CREATE_RW(PATH, PROP, TYPE, HANDLER)						\
	do { _tree->create<TYPE> (PATH)								\
		...
		.add_desired_subscriber(std::bind(&cyan_4r4t_impl::set_ ## HANDLER, this, (PROP), ph::_1))	\
		.set_publisher(std::bind(&cyan_4r4t_impl::get_ ## HANDLER, this, (PROP)    ));	\
	} while(0)
...
#define TREE_CREATE_RO(PATH, PROP, TYPE, HANDLER)						\
	do { _tree->create<TYPE> (PATH)								\
    		.set( get_ ## HANDLER (PROP))							\
		.publish  (std::bind(&cyan_4r4t_impl::get_ ## HANDLER, this, (PROP)    ));	\
	} while(0)
    ...
    _command_time()
{
    ...
    if (not device_addr.has_key("send_buff_size")){
        ...
        device_addr["send_buff_size"] = boost::lexical_cast<std::string>( (size_t) (CYAN_4R4T_NUM_SEND_FRAMES * MAX_ETHERNET_MTU)) ;
    }
...
//    }
    ...
    static const std::vector<size_t> default_map CYAN_4R4T_DEFAULT_RX_MAP;

    _tree->create<std::vector<size_t> >(mb_path / "rx_chan_dsp_mapping").set(CYAN_4R4T_DEFAULT_RX_MAP);
    _tree->create<std::vector<size_t> >(mb_path / "tx_chan_dsp_mapping").set(CYAN_4R4T_DEFAULT_RX_MAP);
    _tree->create<subdev_spec_t>(mb_path / "rx_subdev_spec").add_coerced_subscriber(std::bind(&cyan_4r4t_impl::update_rx_subdev_spec, this, mb, ph::_1));
    _tree->create<subdev_spec_t>(mb_path / "tx_subdev_spec").add_coerced_subscriber(std::bind(&cyan_4r4t_impl::update_tx_subdev_spec, this, mb, ph::_1));
    ...
    for( size_t dspno = 0; dspno < CYAN_4R4T_RX_CHANNELS; dspno++ ) {
		...
		_tree->create<double> (rx_dsp_path / "rate" / "value")
			...
			.add_desired_subscriber(std::bind(&cyan_4r4t_impl::update_rx_samp_rate, this, mb, (size_t) dspno, ph::_1))
			.set_publisher(std::bind(&cyan_4r4t_impl::get_double, this, ("rx_"+lc_num+"/dsp/rate")    ));
...
// 			.set( get_double ("tx_"+lc_num+"/dsp/rate"))
			.add_desired_subscriber(std::bind(&cyan_4r4t_impl::update_tx_samp_rate, this, mb, (size_t) dspno, ph::_1))
			.set_publisher(std::bind(&cyan_4r4t_impl::get_double, this, ("tx_"+lc_num+"/dsp/rate")    ));
		...
		zcxp.num_send_frames = CYAN_4R4T_NUM_SEND_FRAMES;
...
std::string cyan_4r4t_impl::get_tx_sfp( size_t chan ) {
    if ( chan >= CYAN_4R4T_TX_CHANNELS ) {
        std::string error_msg = CYAN_4R4T_DEBUG_NAME_S " requested sfp port of non-existant channel: " + std::to_string(chan);
...
std::string cyan_4r4t_impl::get_tx_ip( size_t chan ) {
    if ( chan >= CYAN_4R4T_TX_CHANNELS ) {
        std::string error_msg = CYAN_4R4T_DEBUG_NAME_S " requested ip of non-existant channel: " + std::to_string(chan);
        throw uhd::value_error(error_msg);
    }
...
uint16_t cyan_4r4t_impl::get_tx_fc_port( size_t chan ) {
    if ( chan >= CYAN_4R4T_TX_CHANNELS ) {
        std::string error_msg = CYAN_4R4T_DEBUG_NAME_S " requested fc port of non-existant channel: " + std::to_string(chan);
        throw uhd::value_error(error_msg);
    }
...
uint16_t cyan_4r4t_impl::get_tx_udp_port( size_t chan ) {
    if ( chan >= CYAN_4R4T_TX_CHANNELS ) {
        std::string error_msg = CYAN_4R4T_DEBUG_NAME_S " requested udp port of non-existant channel: " + std::to_string(chan);
        throw uhd::value_error(error_msg);
    }
...
int64_t cyan_4r4t_impl::get_tx_buff_scale() {
    return CYAN_4R4T_BUFF_SCALE;
}
