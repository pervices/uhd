...
static std::vector<std::weak_ptr<cyan_4r4t_3g_recv_packet_streamer>> allocated_rx_streamers;
static void shutdown_lingering_rx_streamers() {
	...
	for( auto & rx: allocated_rx_streamers ) {
		if ( ! rx.expired() ) {
			std::shared_ptr<cyan_4r4t_3g_recv_packet_streamer> my_streamer = rx.lock();
			if ( my_streamer ) {
...
public:
	...
	:
		sph::send_packet_streamer( max_num_samps, CYAN_4R4T_3G_BUFF_SIZE ),
    ...
    size_t send(
      ...
      //  }
        ...
        if ( 0 == nsamps_per_buff && metadata.end_of_burst ) {
            #ifdef UHD_TXRX_DEBUG_PRINTS
            std::cout << "UHD::" CYAN_4R4T_3G_DEBUG_NAME_C "::Info: " << now << ": " << "eob @ " << now << " | " << now.to_ticks( CYAN_4R4T_3G_TICK_RATE ) << std::endl;
            #endif
    ...
    void enable_blocking_fc(uint64_t blocking_setpoint) {
        ...
        if(blocking_setpoint > 0.9 * CYAN_4R4T_3G_BUFF_SIZE) {
            blocking_setpoint = (uint64_t) (0.9 * CYAN_4R4T_3G_BUFF_SIZE);
        };
    ...
    static managed_send_buffer::sptr get_send_buff( std::weak_ptr<uhd::tx_streamer> tx_streamer, const size_t chan, double timeout ){
        ...
        std::shared_ptr<cyan_4r4t_3g_send_packet_streamer> my_streamer =
            std::dynamic_pointer_cast<cyan_4r4t_3g_send_packet_streamer>( tx_streamer.lock() );
...
private:
	...
	static void send_viking_loop( cyan_4r4t_3g_send_packet_streamer *self ) {
		...
		for( ; ! self->_stop_streaming; ) {
			...
			const long long usloop = 1.0 / (double)CYAN_4R4T_3G_UPDATE_PER_SEC * 1e6;
...
static std::vector<std::weak_ptr<cyan_4r4t_3g_send_packet_streamer>> allocated_tx_streamers;
static void shutdown_lingering_tx_streamers() {
	// This is required as a workaround, because the relevent destructurs are not called
	// when you close the top block in gnu radio. Unsolved mystery for the time being.
	for( auto & tx: allocated_tx_streamers ) {
		if ( ! tx.expired() ) {
			std::shared_ptr<cyan_4r4t_3g_send_packet_streamer> my_streamer = tx.lock();
			if ( my_streamer ) {
				my_streamer->teardown();
			}
		}
...
void cyan_4r4t_3g_impl::io_init(void){
    ...
    for (const std::string &mb : _mbc.keys()) {
        _mbc[mb].rx_streamers.resize( CYAN_4R4T_3G_RX_CHANNELS );
        _mbc[mb].tx_streamers.resize( CYAN_4R4T_3G_TX_CHANNELS );
    }
}
...
void cyan_4r4t_3g_impl::update_rx_samp_rate(const std::string &mb, const size_t dsp, const double rate_){
    ...
    std::shared_ptr<cyan_4r4t_3g_recv_packet_streamer> my_streamer =
        std::dynamic_pointer_cast<cyan_4r4t_3g_recv_packet_streamer>(_mbc[mb].rx_streamers[dsp].lock());
    if (my_streamer.get() == NULL) return;

    my_streamer->set_samp_rate(rate);
    my_streamer->set_tick_rate( CYAN_4R4T_3G_TICK_RATE );
}
...
void cyan_4r4t_3g_impl::update_tx_samp_rate(const std::string &mb, const size_t dsp, const double rate_ ){
	...
	std::shared_ptr<cyan_4r4t_3g_send_packet_streamer> my_streamer =
        std::dynamic_pointer_cast<cyan_4r4t_3g_send_packet_streamer>(_mbc[mb].tx_streamers[dsp].lock());
    if (my_streamer.get() == NULL) return;
...
rx_streamer::sptr cyan_4r4t_3g_impl::get_rx_stream(const uhd::stream_args_t &args_){
    ...
    std::shared_ptr<cyan_4r4t_3g_recv_packet_streamer> my_streamer = std::make_shared<cyan_4r4t_3g_recv_packet_streamer>(spp);
    ...
    if ( false ) {
        ...
        my_streamer->set_scale_factor(((double)(1<<11)-1)/((double)(1<<22)-1));
        ...
        for (const std::string &mb : _mbc.keys()) {
            ...
            if (chan < num_chan_so_far){
                ...
                my_streamer->set_issue_stream_cmd(chan_i, std::bind(
                    &cyan_4r4t_3g_impl::set_stream_cmd, this, scmd_pre, ph::_1));
...
static void get_fifo_lvl_udp_abs( const size_t channel, uhd::transport::udp_simple::sptr xport, uint64_t & lvl, uint64_t & uflow, uint64_t & oflow, uhd::time_spec_t & now ) {
	...
	if ( 0 == r ) {
		UHD_LOGGER_ERROR(CYAN_4R4T_3G_DEBUG_NAME_C) << "Failed to retrieve buffer level for channel " + std::string( 1, 'A' + channel ) + "\nCheck SFP port connections and cofiguration" << std::endl;
...
tx_streamer::sptr cyan_4r4t_3g_impl::get_tx_stream(const uhd::stream_args_t &args_){
    ...
    //make the new streamer given the samples per packet
    cyan_4r4t_3g_send_packet_streamer::timenow_type timenow_ = std::bind( & cyan_4r4t_3g_impl::get_time_now, this );
    ...
    my_streamer->set_time_now(std::bind(&cyan_4r4t_3g_impl::get_time_now,this));
    ...
    for (size_t chan_i = 0; chan_i < args.channels.size(); chan_i++){
        ...
        for (const std::string &mb : _mbc.keys()) {
            ...
            if (chan < num_chan_so_far){
                ...
                my_streamer->set_xport_chan_get_buff(chan_i, std::bind(
                    &cyan_4r4t_3g_send_packet_streamer::get_send_buff, my_streamerp, chan_i, ph::_1
                ));
                ...
                my_streamer->set_xport_chan_update_fc_send_size(chan_i, std::bind(
                    &cyan_4r4t_3g_send_packet_streamer::update_fc_send_count, my_streamerp, chan_i, ph::_1
                ));
                
                my_streamer->set_xport_chan_check_flow_control(chan_i, std::bind(
                    &cyan_4r4t_3g_send_packet_streamer::check_flow_control, my_streamerp, chan_i, ph::_1
                ));
                ...
                my_streamer->set_async_receiver(std::bind(&bounded_buffer<async_metadata_t>::pop_with_timed_wait, &(_cyan_4r4t_3g_io_impl->async_msg_fifo), ph::_1, ph::_2));

                my_streamer->set_async_pusher(std::bind(&bounded_buffer<async_metadata_t>::push_with_pop_on_full, &(_cyan_4r4t_3g_io_impl->async_msg_fifo), ph::_1));
