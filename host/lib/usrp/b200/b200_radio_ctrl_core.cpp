...
private:
    ...
    UHD_INLINE void send_pkt(const uint32_t addr, const uint32_t data = 0)
    {
        ...
        pkt[packet_info.num_header_words32 + 1] = (_bige) ? uhd::htonx(data)
                                                          : uhd::htowx(data);
        // UHD_LOGGER_INFO("radio_ctrl") << std::format("0x%08x, 0x%08x\n") % addr %
        ...
        while (readback or (_outstanding_seqs.size() >= _resp_queue_size)) {
            ...
            if (_resp_xport) {
                ...
                try {
                    ...
                    throw uhd::io_error(
                        str(std::format("Radio ctrl (%s) no response packet - %s")
                            % _name % ex.what()));
                }
            ...
            try {
                ...
                throw uhd::io_error(
                    str(std::format("Radio ctrl (%s) packet parse error - %s") % _name
                        % ex.what()));
            }
