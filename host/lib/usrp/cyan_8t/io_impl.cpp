...
static std::vector<boost::weak_ptr<cyan_8t_recv_packet_streamer>> allocated_rx_streamers;
static void shutdown_lingering_rx_streamers() {
	...
	for( auto & rx: allocated_rx_streamers ) {
		if ( ! rx.expired() ) {
			boost::shared_ptr<cyan_8t_recv_packet_streamer> my_streamer = rx.lock();
			if ( my_streamer ) {
...
public:
    ...
    static managed_send_buffer::sptr get_send_buff( boost::weak_ptr<uhd::tx_streamer> tx_streamer, const size_t chan, double timeout ){
        ...
        boost::shared_ptr<cyan_8t_send_packet_streamer> my_streamer =
            boost::dynamic_pointer_cast<cyan_8t_send_packet_streamer>( tx_streamer.lock() );
...
static std::vector<boost::weak_ptr<cyan_8t_send_packet_streamer>> allocated_tx_streamers;
static void shutdown_lingering_tx_streamers() {
	// This is required as a workaround, because the relevent destructurs are not called
	// when you close the top block in gnu radio. Unsolved mystery for the time being.
	for( auto & tx: allocated_tx_streamers ) {
		if ( ! tx.expired() ) {
			boost::shared_ptr<cyan_8t_send_packet_streamer> my_streamer = tx.lock();
			if ( my_streamer ) {
				my_streamer->teardown();
			}
		}
...
void cyan_8t_impl::update_rx_samp_rate(const std::string &mb, const size_t dsp, const double rate_){
    ...
    boost::shared_ptr<cyan_8t_recv_packet_streamer> my_streamer =
        boost::dynamic_pointer_cast<cyan_8t_recv_packet_streamer>(_mbc[mb].rx_streamers[dsp].lock());
...
void cyan_8t_impl::update_tx_samp_rate(const std::string &mb, const size_t dsp, const double rate_ ){
	...
	boost::shared_ptr<cyan_8t_send_packet_streamer> my_streamer =
        boost::dynamic_pointer_cast<cyan_8t_send_packet_streamer>(_mbc[mb].tx_streamers[dsp].lock());
    if (my_streamer.get() == NULL) return;
...
rx_streamer::sptr cyan_8t_impl::get_rx_stream(const uhd::stream_args_t &args_){
    ...
    boost::shared_ptr<cyan_8t_recv_packet_streamer> my_streamer = boost::make_shared<cyan_8t_recv_packet_streamer>(spp);
...
tx_streamer::sptr cyan_8t_impl::get_tx_stream(const uhd::stream_args_t &args_){
    ...
    boost::shared_ptr<cyan_8t_send_packet_streamer> my_streamer = boost::make_shared<cyan_8t_send_packet_streamer>( spp );
