...
std::string cyan_8t_impl::get_string(std::string req) {
	...
	if (ret == "TIMEOUT") 	throw uhd::runtime_error("cyan_8t_impl::get_string - UDP resp. timed out: " + req);
	else 			return ret;
}
void cyan_8t_impl::set_string(const std::string pre, std::string data) {
...
void cyan_8t_impl::set_mboard_eeprom(const std::string pre, mboard_eeprom_t data) {
	(void)pre;
	(void)data;
...
dboard_eeprom_t cyan_8t_impl::get_dboard_eeprom(std::string req) {
	(void)req;
...
void cyan_8t_impl::set_dboard_eeprom(const std::string pre, dboard_eeprom_t data) {
	(void)pre;
	(void)data;
	// no eeprom settings on Crimson
	return;
...
sensor_value_t cyan_8t_impl::get_sensor_value(std::string req) {
	(void)req;
	// no sensors on Crimson
	return sensor_value_t("NA", "0", "NA");
}
void cyan_8t_impl::set_sensor_value(const std::string pre, sensor_value_t data) {
	(void)pre;
	(void)data;
	// no sensors on Crimson
	return;
...
meta_range_t cyan_8t_impl::get_meta_range(std::string req) {
	(void)req;
	throw uhd::not_implemented_error("set_meta_range not implemented, Crimson does not support range settings");
}
void cyan_8t_impl::set_meta_range(const std::string pre, meta_range_t data) {
	(void)pre;
	(void)data;
	throw uhd::not_implemented_error("set_meta_range not implemented, Crimson does not support range settings");
}
...
std::complex<double>  cyan_8t_impl::get_complex_double(std::string req) {
	(void)req;
	std::complex<double> temp;
	return temp;
}
void cyan_8t_impl::set_complex_double(const std::string pre, std::complex<double> data) {
	(void)pre;
	(void)data;
	return;
}
...
stream_cmd_t cyan_8t_impl::get_stream_cmd(std::string req) {
	(void)req;
...
void cyan_8t_impl::set_stream_cmd( const std::string pre, const stream_cmd_t stream_cmd ) {
...
time_spec_t cyan_8t_impl::get_time_spec(std::string req) {
	if ( false ) {
...
void cyan_8t_impl::set_time_spec( const std::string key, time_spec_t value ) {
...
user_reg_t cyan_8t_impl::get_user_reg(std::string req) {
...
void cyan_8t_impl::send_gpio_burst_req(const gpio_burst_req& req) {
	_time_diff_iface[0]->send(boost::asio::const_buffer(&req, sizeof(req)));
}

void cyan_8t_impl::set_user_reg(const std::string key, user_reg_t value) {
...
void cyan_8t_impl::set_properties_from_addr() {
...
static device_addrs_t cyan_8t_find_with_addr(const device_addr_t &hint)
{
    ...
    udp_simple::sptr comm = udp_simple::make_broadcast(
        hint["addr"], BOOST_STRINGIZE(CYAN_8T_FW_COMMS_UDP_PORT));
    ...
    char buff[CYAN_8T_FW_COMMS_MTU] = {};
    ...
    for(
        ...
        if (
            ...
            //UHD_LOGGER_INFO( "CRIMSON_IMPL" ) << "Found cyan_8t at " << new_addr[ "addr" ] << " in " << ( (now - then).get_real_secs() ) << " s" << std::endl;
...
static device_addrs_t cyan_8t_find(const device_addr_t &hint_)
{
    ...
    if (hints.size() > 1)
    {
        ...
        {
            device_addrs_t found_devices_i = cyan_8t_find(hint_i);
...
#else
    if (hint.has_key("type") and hint["type"] != "cyan_8t") return addrs;
#endif
    ...
    if (hint.has_key("addr"))
    {
        ...
        try
        {
            reply_addrs = cyan_8t_find_with_addr(hint);
        }
        catch(const std::exception &ex)
        {
            UHD_LOGGER_ERROR("CRIMSON_IMPL") << "CYAN_8T Network discovery error " << ex.what() << std::endl;
        }
        catch(...)
        {
            UHD_LOGGER_ERROR("CRIMSON_IMPL") << "CYAN_8T Network discovery unknown error " << std::endl;
        }
        BOOST_FOREACH(const device_addr_t &reply_addr, reply_addrs)
        {
            device_addrs_t new_addrs = cyan_8t_find_with_addr(reply_addr);
    ...
    if (!hint.has_key("resource"))
    {
        for( auto & if_addrs: get_if_addrs() )
        {
            ...
            device_addrs_t new_addrs = cyan_8t_find(new_hint);
            addrs.insert(addrs.begin(), new_addrs.begin(), new_addrs.end());
        }
    }
...
static constexpr double tick_period_ns = 1.0 / CYAN_8T_DSP_CLOCK_RATE * 1e9;
...
void cyan_8t_impl::make_rx_stream_cmd_packet( const uhd::stream_cmd_t & cmd, const uhd::time_spec_t & now, const size_t channel, uhd::usrp::rx_stream_cmd & pkt ) {
...
void cyan_8t_impl::send_rx_stream_cmd_req( const rx_stream_cmd & req ) {
	_time_diff_iface[0]->send( boost::asio::const_buffer( & req, sizeof( req ) ) );
}

/// SoB Time Diff: send sync packet (must be done before reading flow iface)
void cyan_8t_impl::time_diff_send( const uhd::time_spec_t & crimson_now ) {
...
void cyan_8t_impl::time_diff_send( const uhd::time_spec_t & crimson_now, int xg_intf) {
...
bool cyan_8t_impl::time_diff_recv( time_diff_resp & tdr ) {
...
bool cyan_8t_impl::time_diff_recv( time_diff_resp & tdr, int xg_intf ) {
...
void cyan_8t_impl::time_diff_process( const time_diff_resp & tdr, const uhd::time_spec_t & now ) {
...
bool cyan_8t_impl::time_diff_converged() {
	return _time_diff_converged;
}

// the buffer monitor thread
void cyan_8t_impl::bm_thread_fn( cyan_8t_impl *dev ) {

	dev->_bm_thread_running = true;

    int xg_intf = 0;
	const uhd::time_spec_t T( 1.0 / (double) CYAN_8T_UPDATE_PER_SEC );
	std::vector<size_t> fifo_lvl( CYAN_8T_TX_CHANNELS );
...
static device::sptr cyan_8t_make(const device_addr_t &device_addr)
{
    return device::sptr(new cyan_8t_impl(device_addr));
}
...
UHD_STATIC_BLOCK(register_cyan_8t_device)
{
	set_log_level( uhd::log::severity_level::info );
    device::register_device(&cyan_8t_find, &cyan_8t_make, device::USRP);
}
...
#define TREE_CREATE_RW(PATH, PROP, TYPE, HANDLER)						\
	do { _tree->create<TYPE> (PATH)								\
		...
		.add_desired_subscriber(boost::bind(&cyan_8t_impl::set_ ## HANDLER, this, (PROP), _1))	\
		.set_publisher(boost::bind(&cyan_8t_impl::get_ ## HANDLER, this, (PROP)    ));	\
	} while(0)
...
#define TREE_CREATE_RO(PATH, PROP, TYPE, HANDLER)						\
	do { _tree->create<TYPE> (PATH)								\
    		.set( get_ ## HANDLER (PROP))							\
		.publish  (boost::bind(&cyan_8t_impl::get_ ## HANDLER, this, (PROP)    ));	\
	} while(0)
...
cyan_8t_impl::cyan_8t_impl(const device_addr_t &_device_addr)
:
    ...
    _command_time()
{
    ...
    if (not device_addr.has_key("send_buff_size")){
        ...
        device_addr["send_buff_size"] = boost::lexical_cast<std::string>( CYAN_8T_BUFF_SIZE * sizeof( std::complex<int16_t> ) );
    }
...
//    }
    ...
    _mbc[mb].iface = cyan_8t_iface::make(
		udp_simple::make_connected(
			_device_addr["addr"],
			BOOST_STRINGIZE( CYAN_8T_FW_COMMS_UDP_PORT )
		)
    );
    ...
    _tree->create<subdev_spec_t>(mb_path / "rx_subdev_spec").add_coerced_subscriber(boost::bind(&cyan_8t_impl::update_rx_subdev_spec, this, mb, _1));
    _tree->create<subdev_spec_t>(mb_path / "tx_subdev_spec").add_coerced_subscriber(boost::bind(&cyan_8t_impl::update_tx_subdev_spec, this, mb, _1));
    ...
    TREE_CREATE_ST(mb_path / "tick_rate", double, CYAN_8T_DSP_CLOCK_RATE );
    ...
    TREE_CREATE_ST(mb_path / "clock_source" / "external" / "value", double, CYAN_8T_EXT_CLK_RATE);
    ...
    for( size_t dspno = 0; dspno < CYAN_8T_RX_CHANNELS; dspno++ ) {
	    ...
	    // RX bandwidth
		TREE_CREATE_ST(rx_fe_path / "bandwidth" / "value", double, CYAN_8T_MASTER_CLOCK_RATE / 2.0 );
		TREE_CREATE_ST(rx_fe_path / "bandwidth" / "range", meta_range_t, meta_range_t( CYAN_8T_MASTER_CLOCK_RATE / 2.0, CYAN_8T_MASTER_CLOCK_RATE / 2.0 ) );

		TREE_CREATE_ST(rx_fe_path / "freq", meta_range_t,
			meta_range_t(CYAN_8T_FREQ_RANGE_START, CYAN_8T_FREQ_RANGE_STOP, CYAN_8T_FREQ_RANGE_STEP));
		...
		TREE_CREATE_ST(rx_fe_path / "gain" / "range", meta_range_t,
			meta_range_t(CYAN_8T_RF_RX_GAIN_RANGE_START, CYAN_8T_RF_RX_GAIN_RANGE_STOP, CYAN_8T_RF_RX_GAIN_RANGE_STEP));
		...
		case 'B':
				...
				meta_range_t(CYAN_8T_RATE_RANGE_START, CYAN_8T_RATE_RANGE_STOP, CYAN_8T_RATE_RANGE_STEP));
			TREE_CREATE_ST(rx_dsp_path / "freq" / "range", meta_range_t,
				meta_range_t(CYAN_8T_DSP_FREQ_RANGE_START, CYAN_8T_DSP_FREQ_RANGE_STOP, CYAN_8T_DSP_FREQ_RANGE_STEP));
		...
		case 'D':
			TREE_CREATE_ST(rx_dsp_path / "rate" / "range", meta_range_t,
				meta_range_t(CYAN_8T_RATE_RANGE_START, CYAN_8T_RATE_RANGE_STOP  / 2, CYAN_8T_RATE_RANGE_STEP));
			TREE_CREATE_ST(rx_dsp_path / "freq" / "range", meta_range_t,
				meta_range_t(CYAN_8T_DSP_FREQ_RANGE_START, CYAN_8T_DSP_FREQ_RANGE_STOP , CYAN_8T_DSP_FREQ_RANGE_STEP));
			TREE_CREATE_ST(rx_dsp_path / "bw" / "range",   meta_range_t,
				meta_range_t(CYAN_8T_RATE_RANGE_START, CYAN_8T_RATE_RANGE_STOP / 2, CYAN_8T_RATE_RANGE_STEP));
		...
		_tree->create<double> (rx_dsp_path / "rate" / "value")
			...
			.add_desired_subscriber(boost::bind(&cyan_8t_impl::update_rx_samp_rate, this, mb, (size_t) dspno, _1))
			.set_publisher(boost::bind(&cyan_8t_impl::get_double, this, ("rx_"+lc_num+"/dsp/rate")    ));
	    ...
	    static const size_t ip_udp_size = 0
			...
			+ 8  // UDP Header
	    ;
		const size_t bpp = CYAN_8T_MAX_MTU - ip_udp_size;
    ...
    for( int dspno = 0; dspno < CYAN_8T_TX_CHANNELS; dspno++ ) {
		std::string lc_num  = boost::lexical_cast<std::string>((char)(dspno/CYAN_8T_DSP_PER_RFE + 'a'));
        std::string lc_udp  = boost::lexical_cast<std::string>((char)(dspno/4 + 'a'));
		std::string num     = boost::lexical_cast<std::string>((char)(dspno + 'A'));
		std::string chan    = "Channel_" + std::to_string(dspno);
		//std::string chan    = "Channel_" + std::to_string(dspno % CYAN_8T_DSP_PER_RFE);
	    ...
	    // TX bandwidth
		TREE_CREATE_ST(tx_fe_path / "bandwidth" / "value"           , double                                        , CYAN_8T_MASTER_CLOCK_RATE / 2.0 );
		TREE_CREATE_ST(tx_fe_path / "bandwidth" / "range"           , meta_range_t                                  , meta_range_t( CYAN_8T_MASTER_CLOCK_RATE / 2.0, CYAN_8T_MASTER_CLOCK_RATE / 2.0 ) );

		TREE_CREATE_ST(tx_fe_path / "freq"                          , meta_range_t                                  , meta_range_t(CYAN_8T_FREQ_RANGE_START, CYAN_8T_FREQ_RANGE_STOP, CYAN_8T_FREQ_RANGE_STEP));
		...
		TREE_CREATE_ST(tx_fe_path / "freq" / "range"                , meta_range_t                                  , meta_range_t(CYAN_8T_FREQ_RANGE_START, CYAN_8T_FREQ_RANGE_STOP, CYAN_8T_FREQ_RANGE_STEP));
		TREE_CREATE_ST(tx_fe_path / "gain" / "range"                , meta_range_t                                  , meta_range_t(CYAN_8T_RF_TX_GAIN_RANGE_START, CYAN_8T_RF_TX_GAIN_RANGE_STOP, CYAN_8T_RF_TX_GAIN_RANGE_STEP));
...
#ifdef PV_TATE
        if (dspno % CYAN_8T_DSP_PER_RFE == 0) {
#endif
		// these are phony properties for Crimson
		...
		case 'B':
                ...
                default:
#endif
			TREE_CREATE_ST(tx_dsp_path / "rate" / "range"           , meta_range_t                                  , meta_range_t(CYAN_8T_RATE_RANGE_START, CYAN_8T_RATE_RANGE_STOP, CYAN_8T_RATE_RANGE_STEP));
			TREE_CREATE_ST(tx_dsp_path / "freq" / "range"           , meta_range_t                                  , meta_range_t(CYAN_8T_DSP_FREQ_RANGE_START, CYAN_8T_DSP_FREQ_RANGE_STOP, CYAN_8T_DSP_FREQ_RANGE_STEP));
			TREE_CREATE_ST(tx_dsp_path / "bw" / "range"             , meta_range_t                                  , meta_range_t(CYAN_8T_RATE_RANGE_START, CYAN_8T_RATE_RANGE_STOP, CYAN_8T_RATE_RANGE_STEP));
			break;
#ifndef PV_TATE
		case 'C':
		case 'D':
			TREE_CREATE_ST(tx_dsp_path / "rate" / "range"           , meta_range_t                                  , meta_range_t(CYAN_8T_RATE_RANGE_START, CYAN_8T_RATE_RANGE_STOP / 2, CYAN_8T_RATE_RANGE_STEP));
			TREE_CREATE_ST(tx_dsp_path / "freq" / "range"           , meta_range_t                                  , meta_range_t(CYAN_8T_DSP_FREQ_RANGE_START, CYAN_8T_DSP_FREQ_RANGE_STOP, CYAN_8T_DSP_FREQ_RANGE_STEP));
			TREE_CREATE_ST(tx_dsp_path / "bw" / "range"             , meta_range_t                                  , meta_range_t(CYAN_8T_RATE_RANGE_START, CYAN_8T_RATE_RANGE_STOP / 2, CYAN_8T_RATE_RANGE_STEP));
			break;
#endif
		}
		...
		_tree->create<double> (tx_dsp_path / "rate" / "value")
			...
			.add_desired_subscriber(boost::bind(&cyan_8t_impl::update_tx_samp_rate, this, mb, (size_t) dspno, _1))
			.set_publisher(boost::bind(&cyan_8t_impl::get_double, this, ("tx_"+lc_num+"/dsp/rate")    ));
        ...
        switch (dspno % CYAN_8T_DSP_PER_RFE) {
            case 0 :
		...
		zcxp.num_send_frames = CYAN_8T_BUFF_SIZE * sizeof( std::complex<int16_t> ) / bpp;
...
//        }
	...
	if ( _bm_thread_needed ) {
		...
		_time_diff_pidc = uhd::pidc_tl(
			...
			2.0 / (double)CYAN_8T_UPDATE_PER_SEC
		);

		_time_diff_pidc.set_error_filter_length( CYAN_8T_UPDATE_PER_SEC );
...
cyan_8t_impl::~cyan_8t_impl(void)
{
       stop_bm();
}

bool cyan_8t_impl::is_bm_thread_needed() {
	bool r = true;
...
void cyan_8t_impl::get_tx_endpoint( uhd::property_tree::sptr tree, const size_t & chan, std::string & ip_addr, uint16_t & udp_port, std::string & sfp ) {
#ifdef PV_TATE
    if (chan < 2) {
        sfp = "sfpa";
    } else if (chan < 4) {
