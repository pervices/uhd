...
//    }
    ...
    TREE_CREATE_ST(rx_path / "spec",   std::string, "N/A");
    ...
    TREE_CREATE_ST(tx_path / "spec", std::string, "8 TX RF chains, TODOMHz BW and DC-TODOGHz each");
    ...
    for( size_t dspno = 0; dspno < CYAN_8T_RX_CHANNELS; dspno++ ) {
        ...
        try {
            ...
            UHD_LOGGER_WARNING(CYAN_8T_DEBUG_NAME_C) << "Unable to bind ip adress, certain features may not work. \n IP: " << _tree->access<std::string>( rx_link_path / "ip_dest" ).get() << std::endl;
        }
    }
...
void cyan_8t_impl::set_tx_gain(double gain, const std::string &name, size_t chan){
    ...
    for (size_t c = 0; c < CYAN_8T_TX_CHANNELS; c++){
