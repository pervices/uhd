...
private:
    ...
    bool get_irq(void)
    {
        ...
        UHD_LOGGER_TRACE("TVRX")
            << std::format("TVRX2 ({}): IRQ {}", get_subdev_name(), irq);
    ...
    bool get_power_reset(void)
    {
        ...
        UHD_LOGGER_TRACE("TVRX") << std::format("TVRX2 ({}): POR {}",
                                        get_subdev_name(),
                                        static_cast<int>(_tda18272hnm_regs.por));
    ...
    sensor_value_t get_locked(void)
    {
        ...
        UHD_LOGGER_TRACE("TVRX")
            << std::format("TVRX2 ({}): locked {}", get_subdev_name(), locked);
    ...
    sensor_value_t get_temp(void)
    {
        ...
        UHD_LOGGER_TRACE("TVRX") << std::format("TVRX2 ({}): Temperature {} C",
                                        get_subdev_name(),
                                        static_cast<double>(_tda18272hnm_regs.tm_d));
...
tvrx2::tvrx2(ctor_args_t args) : rx_dboard_base(args)
{
    ...
    if (ref_clock == 64.0e6) {
        ...
        UHD_LOGGER_TRACE("TVRX")
            << std::format("TVRX2 ({}): Dividing Refclock by 4", get_subdev_name());
        ...
        UHD_LOGGER_TRACE("TVRX")
            << std::format("TVRX2 ({}): Dividing Refclock by 6", get_subdev_name());

        _freq_scalar =
            (6 * 16.0e6) / this->get_iface()->get_clock_rate(dboard_iface::UNIT_RX);
    } else if (ref_clock == 200e6) {
        UHD_LOGGER_WARNING("TVRX")
            << ("ref_clock was 200e6, setting ref_clock divider for 100e6.");
        ...
        UHD_LOGGER_WARNING("TVRX")
            << std::format(
                    "Unsupported ref_clock {:.2f}, valid options 64e6, 100e6, 200e6", ref_clock);
        _freq_scalar = 1.0;
    }
    ...
    UHD_LOGGER_TRACE("TVRX") << std::format("TVRX2 ({}): Refclock {} Hz, scalar = {}",
                                    (get_subdev_name()),
...
tvrx2::~tvrx2(void)
{
    UHD_LOGGER_TRACE("TVRX") << std::format("TVRX2 ({}): Called Destructor",
                                    (get_subdev_name()));
...
void tvrx2::send_reg(uint8_t start_reg, uint8_t stop_reg)
{
    ...
    for (uint8_t start_addr = start_reg; start_addr <= stop_reg;
        ...
        for (int i = 0; i < num_bytes; i++) {
            ...
            UHD_LOGGER_TRACE("TVRX")
                    << std::format("TVRX2 ({}, 0x{:02x}): send reg 0x{:02x}, value 0x{:04x}, start_addr = 0x{:04x}, num_bytes {}",
                            get_subdev_name(),
                            static_cast<int>(tvrx2_sd_name_to_i2c_addr[get_subdev_name()]),
...
void tvrx2::read_reg(uint8_t start_reg, uint8_t stop_reg)
{
    ...
    for (uint8_t start_addr = start_reg; start_addr <= stop_reg;
        ...
        for (uint8_t i = 0; i < num_bytes; i++) {
            ...
            UHD_LOGGER_TRACE("TVRX")
                    << std::format("TVRX2 ({}, 0x{:02x}): read reg 0x{:02x}, value 0x{:04x}, start_addr = 0x{:04x}, num_bytes {}",
                            get_subdev_name(),
                            static_cast<int>(tvrx2_sd_name_to_i2c_addr[get_subdev_name()]),
                            static_cast<int>(start_addr + i),
...
void tvrx2::tvrx2_tda18272_tune_rf_filter(uint32_t uRF)
{
    ...
    UHD_LOGGER_TRACE("TVRX") << std::format("\nTVRX2 ({}): Software Calibration:\n"
                                              "\tRF Filter Bypass = {}\n"
                                              "\tRF Filter Cap    = {}\n"
                                              "\tRF Filter Band   = {}\n"
                                              "\tGain Taper       = {}\n",
                                    ...
                                    static_cast<int>(_tda18272hnm_regs.gain_taper));
...
void tvrx2::test_rf_filter_robustness(void)
{
    ...
    robustness_message << std::format("TVRX2 ({}): RF Filter Robustness Results:",
                              (get_subdev_name()));
    for (const std::string& name : uhd::sorted(_filter_ratings.keys())) {
        robustness_message << std::format("\t{}:\tMargin = {:.2f},\tRobustness = {}",
                name,
                _filter_margins[name],
...
void tvrx2::transition_0(void)
{
    ...
    UHD_LOGGER_TRACE("TVRX")
        << std::format(
               "\nTVRX2 ({}): Transition 0: Initialize Tuner, Calibrate and Standby\n",
               get_subdev_name());
...
void tvrx2::transition_1(void)
{
    ...
    UHD_LOGGER_TRACE("TVRX") << std::format(
                                    "\nTVRX2 ({}): Transition 1: Select TV Standard\n",
                                    get_subdev_name());
...
void tvrx2::transition_3(void)
{
    ...
    UHD_LOGGER_TRACE("TVRX") << std::format(
                                    "\nTVRX2 ({}): Transition 3: Standby Mode\n",
                                    get_subdev_name());
...
void tvrx2::wait_irq(void)
{
    ...
    bool irq = get_irq();
    UHD_LOGGER_TRACE("TVRX")
            << std::format("\nTVRX2 ({}): Waiting on IRQ, subdev = {}, mask = 0x{:x}, Status: 0x{:x}\n",
                    get_subdev_name(),
                    get_subdev_name(),
                    static_cast<int>(tvrx2_sd_name_to_irq_io[get_subdev_name()]),
                    irq
            );
    ...
    UHD_LOGGER_TRACE("TVRX") <<
            std::format("\nTVRX2 ({}): IRQ Raised, subdev = {}, mask = 0x{:x}, Status: 0x{:x}, Timeout: {}\n",
                    get_subdev_name(),
                    get_subdev_name(),
                    static_cast<int>(tvrx2_sd_name_to_irq_io[get_subdev_name()]),
                    irq,
    ...
    UHD_LOGGER_TRACE("TVRX")
        << std::format(
               "\nTVRX2 ({}): Cleared IRQ, subdev = {}, mask = {:#x}, Status: {:#x}\n",
               get_subdev_name(), get_subdev_name(),
               static_cast<int>(tvrx2_sd_name_to_irq_io[get_subdev_name()]), irq);
}
...
static double gain_to_if_gain_dac(double& gain)
{
    ...
    UHD_LOGGER_TRACE("TVRX") << std::format("TVRX2 IF Gain: {} dB, dac_volts: {} V",
                                    gain, dac_volts);
...
double tvrx2::set_bandwidth(double bandwidth)
{
    ...
    UHD_LOGGER_TRACE("TVRX") << std::format(
                                    "TVRX2 ({}) Bandwidth (lp_fc): {} Hz, reg: {}",
                                    (get_subdev_name()), _bandwidth,
                                    static_cast<int>(_tda18272hnm_regs.lp_fc));
