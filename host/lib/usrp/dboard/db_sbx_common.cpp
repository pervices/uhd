...
static int rx_pga0_gain_to_iobits(double& gain)
{
    ...
    UHD_LOGGER_TRACE("SBX")
        << std::format("SBX RX Attenuation: %f dB, Code: %d, IO Bits %x, Mask: %x")
...
static int tx_pga0_gain_to_iobits(double& gain)
{
    ...
    UHD_LOGGER_TRACE("SBX")
        << std::format("SBX TX Attenuation: %f dB, Code: %d, IO Bits %x, Mask: %x")
...
sbx_xcvr::sbx_xcvr(ctor_args_t args) : xcvr_dboard_base(args)
{
    ...
    UHD_LOGGER_TRACE("SBX") << std::format("SBX GPIO Direction: RX: 0x%08x, TX: 0x%08x")
                                   % RXIO_MASK % TXIO_MASK;
}
