...
private:
    ...
    double set_tx_gain(double gain)
    {
        ...
        write_gpio();
        UHD_LOG_TRACE("UBX",
            std::format("UBX TX Gain: %f dB, Code: %d, IO Bits 0x%04x") % gain
    ...
    double set_rx_gain(double gain)
    {
        ...
        write_gpio();
        UHD_LOG_TRACE("UBX",
            std::format("UBX RX Gain: %f dB, Code: %d, IO Bits 0x%04x") % gain
                % attn_code % (attn_code << 10));
        _rx_gain = gain;
        return gain;
    }
    ...
    double set_tx_freq(double freq)
    {
        ...
        UHD_LOGGER_TRACE("UBX")
            << std::format("UBX TX: the requested frequency is %f MHz") % (freq / 1e6);
        ...
        UHD_LOGGER_TRACE("UBX")
            << std::format("UBX TX: the actual frequency is %f MHz") % (_tx_freq / 1e6);
    ...
    double set_rx_freq(double freq)
    {
        ...
        UHD_LOGGER_TRACE("UBX")
            << std::format("UBX RX: the requested frequency is %f MHz") % (freq / 1e6);
        ...
        UHD_LOGGER_TRACE("UBX")
            << std::format("UBX RX: the actual frequency is %f MHz") % (_rx_freq / 1e6);
