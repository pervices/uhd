    ...
    const std::string& ant, const size_t chan)
{
    if (ant != get_tx_antenna(chan)) {
        throw uhd::value_error(
            str(std::format("[%s] Requesting invalid TX antenna value: %s")
                % get_unique_id() % ant));
    }
    ...
    const std::string& ant, const size_t chan)
{
    ...
    if (std::find(MAGNESIUM_RX_ANTENNAS.begin(), MAGNESIUM_RX_ANTENNAS.end(), ant)
        ...
        throw uhd::value_error(
            str(std::format("[%s] Requesting invalid RX antenna value: %s")
                % get_unique_id() % ant));
    }
    ...
    const std::string band_map, const uhd::direction_t dir)
{
    ...
    for (size_t i = 0; i < band_map_split.size(); i++) {
        try {
            ...
            throw uhd::value_error(
                (std::format("error while converting given frequency string %s "
                               "to a double value")
                    % band_map_split.at(i))
                    .str());
        }
