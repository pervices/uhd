    ...
    void update_regs(void)
    {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            UHD_LOGGER_TRACE("TVRX")
                    << std::format("tvrx: send reg 0x{:02x}, value 0x{:04x}",
                            static_cast<int>(i),
...
static double rf_gain_to_voltage(double gain, double lo_freq)
{
    ...
    UHD_LOGGER_TRACE("TVRX") << std::format("tvrx RF AGC gain: {} dB, dac_volts: {} V",
                                    gain, dac_volts);
...
static double if_gain_to_voltage(double gain)
{
    ...
    UHD_LOGGER_TRACE("TVRX") << std::format("tvrx IF AGC gain: {} dB, dac_volts: {} V",
                                    gain, dac_volts);
...
double tvrx::set_freq(double freq)
{
    ...
    UHD_LOGGER_TRACE("TVRX")
        << std::format("set_freq: target LO: {} f_ref: {} divisor: {} actual LO: {}",
