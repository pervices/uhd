        ...
        for (uint8_t start_addr = start_reg; start_addr <= stop_reg;
            ...
            for (int i = 0; i < num_bytes; i++) {
                ...
                UHD_LOGGER_TRACE("DBSRX")
                    << std::format("DBSRX: send reg {:#04x}, value {:#06x}, start_addr = "
                                     "{:#06x}, num_bytes {}",
                           static_cast<int>(start_addr + i), static_cast<int>(regs_vector[1 + i]),
                           static_cast<int>(start_addr), num_bytes);
            }
    ...
    void read_reg(uint8_t start_reg, uint8_t stop_reg)
    {
        ...
        for (uint8_t start_addr = start_reg; start_addr <= stop_reg;
            ...
            for (uint8_t i = 0; i < num_bytes; i++) {
                ...
                UHD_LOGGER_TRACE("DBSRX")
                    << std::format("DBSRX: read reg {:#04x}, value {:#06x}, start_addr = "
                                     "{:#06x}, num_bytes {}",
                           static_cast<int>(start_addr + i), static_cast<int>(regs_vector[i]), static_cast<int>(start_addr),
                           num_bytes);
            }
        }
    }
...
double dbsrx::set_lo_freq(double target_freq)
{
    ...
    for (auto ref_clock_it : uhd::reversed(uhd::sorted(clock_rates))) {
        ...
        UHD_LOGGER_TRACE("DBSRX")
            << std::format("DBSRX: trying ref_clock {} and m_divider {}", (ref_clock),
                   m);
...
done_loop:
    ...
    UHD_LOGGER_TRACE("DBSRX")
        << std::format(
               "DBSRX: choose ref_clock (current: {}, new: {}) and m_divider {}",
    ...
    UHD_LOGGER_TRACE("DBSRX")
        << std::format("DBSRX: scaler {}, actual_freq {} MHz, register bit: {}",
    ...
    UHD_LOGGER_TRACE("DBSRX") << std::format(
                                     "DBSRX: initial guess for vco {}, vtune adc {}",
    ...
    while ((_max2118_read_regs.adc == 0) or (_max2118_read_regs.adc == 7)) {
        ...
        UHD_LOGGER_TRACE("DBSRX") << std::format("DBSRX: trying vco {}, vtune adc {}",
                                         int(_max2118_write_regs.osc_band),
                                         int(_max2118_read_regs.adc));
    ...
    UHD_LOGGER_TRACE("DBSRX") << std::format("DBSRX: final vco {}, vtune adc {}",
                                     int(_max2118_write_regs.osc_band),
                                     int(_max2118_read_regs.adc));
...
static double gain_to_gc1_rfvga_dac(double& gain)
{
    ...
    UHD_LOGGER_TRACE("DBSRX") << std::format("DBSRX GC1 Gain: {} dB, dac_volts: {} V",
                                     gain, dac_volts);
...
double dbsrx::set_bandwidth(double bandwidth)
{
    ...
    UHD_LOGGER_TRACE("DBSRX") << std::format(
                                     "DBSRX Filter Bandwidth: {} MHz, m: {}, f_dac: {}\n",
                                     (_bandwidth / 1e6),
