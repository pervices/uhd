...
rhodium_cpld_ctrl::rhodium_cpld_ctrl(write_spi_t write_fn, read_spi_t read_fn)
{
    _write_reg_fn = [write_fn](const uint8_t addr, const uint32_t data) {
        UHD_LOG_TRACE("RH_CPLD",
            str(std::format("Writing to CPLD: 0x%02X -> 0x%04X") % uint32_t(addr)
                % data));
        const uint32_t spi_transaction = 0 | ((addr & 0x7F) << 17) | data;
        UHD_LOG_TRACE(
            "RH_CPLD", str(std::format("SPI Transaction: 0x%04X") % spi_transaction));
        write_fn(spi_transaction);
    };
    _write_raw_fn = [write_fn](const uint32_t data) {
        UHD_LOG_TRACE("RH_CPLD", str(std::format("Writing to CPLD: 0x%06X") % data));
        UHD_LOG_TRACE("RH_CPLD", str(std::format("SPI Transaction: 0x%06X") % data));
        write_fn(data);
    };
    _read_reg_fn = [read_fn](const uint8_t addr) {
        UHD_LOG_TRACE("RH_CPLD",
            str(std::format("Reading from CPLD address 0x%02X") % uint32_t(addr)));
