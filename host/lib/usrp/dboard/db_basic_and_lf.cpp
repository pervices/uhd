    ...
    const std::string db_name([this]() {
        switch (get_rx_id().to_uint16()) {
            ...
            case BASIC_RX_RFNOC_PID:
                return std::format("{} ({})", "BasicRX", get_subdev_name());
            case LF_RX_PID:
            case LF_RX_RFNOC_PID:
                return std::format("{} ({})", "LFRX", get_subdev_name());
            default:
    ...
    const std::string db_name([this]() {
        switch (get_tx_id().to_uint16()) {
            ...
            case BASIC_TX_RFNOC_PID:
                return std::format("{} ({})", "BasicTX", get_subdev_name());
            case LF_TX_PID:
            case LF_TX_RFNOC_PID:
                return std::format("{} ({})", "LFTX", get_subdev_name());
            default:
                UHD_THROW_INVALID_CODE_PATH();
        }
    }());
