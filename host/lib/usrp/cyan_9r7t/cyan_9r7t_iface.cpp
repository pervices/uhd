 ...
 * Structors
 **********************************************************************/
cyan_9r7t_iface::cyan_9r7t_iface(udp_simple::sptr ctrl_transport):
 ...
 * Peek and Poke
 **********************************************************************/
// Never call this function by itself, always call through cyan_9r7t_impl::get/set()
// else it will mess up the protocol with the sequencing and will contian no error checks.
void cyan_9r7t_iface::poke_str(std::string data) {
...
// Never call this function by itself, always call through cyan_9r7t_impl::get/set(),
// else it will mess up the protocol with the sequencing and will contian no error checks.
std::string cyan_9r7t_iface::peek_str( float timeout_s ) {
    uint32_t iseq;
...
std::string cyan_9r7t_iface::peek_str() {
	return peek_str( 8 );
}

/***********************************************************************
 * Public make function for cyan_9r7t interface
 **********************************************************************/
cyan_9r7t_iface::sptr cyan_9r7t_iface::make(udp_simple::sptr ctrl_transport){
    return cyan_9r7t_iface::sptr(new cyan_9r7t_iface(ctrl_transport));
}

/***********************************************************************
 * Helper Functions
 **********************************************************************/
void cyan_9r7t_iface::parse(std::vector<std::string> &tokens, char* data, const char delim) {
	int i = 0;
	while (data[i]) {
		std::string token = "";
