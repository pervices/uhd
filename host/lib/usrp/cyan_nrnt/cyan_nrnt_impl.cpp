...
void cyan_nrnt_impl::lock_xx_channel_streaming(const size_t channel_num, const uhd::direction_t xx_sign) {
    ...
    _use_dpdk(use_dpdk)
{
    ...
    try {
        ...
        // Create or open lockfile unique to the device using the device type and time board serial number (ex/ cyan_nrnt_<serial>)
        ...
        if (r == -1) {
            ...
            if (err == EWOULDBLOCK) {
                UHD_LOG_ERROR(CYAN_NRNT_DEBUG_NAME_C, "Another instance of UHD has already been created with this device and may cause unexpected behaviour. "
        ...
        UHD_LOG_ERROR(CYAN_NRNT_DEBUG_NAME_C, "Unable to determine if another instance of UHD has already been created with this device.\n" + std::string(e.what()));
    }
    ...
    TREE_CREATE_RW(PV_DEVICE_MB_PATH / "system/get_max_buffer_level", "system/get_max_buffer_level", double, double);
    max_buffer_level = (int64_t) (_tree->access<double>(PV_DEVICE_MB_PATH / "system/get_max_buffer_level").get());
    // The number to multiply get buffer level requests by to get the actual buffer level in number of samples
    TREE_CREATE_RW(PV_DEVICE_MB_PATH / "system/get_buffer_level_multiple", "system/get_buffer_level_multiple", double, double);
    buffer_level_multiple = (int64_t) (_tree->access<double>(PV_DEVICE_MB_PATH / "system/get_buffer_level_multiple").get());

    TREE_CREATE_RW(PV_DEVICE_MB_PATH / "system/nsamps_multiple_rx", "system/nsamps_multiple_rx", int, int);
    nsamps_multiple_rx = (_tree->access<int>(PV_DEVICE_MB_PATH / "system/nsamps_multiple_rx").get());

    TREE_CREATE_RW(PV_DEVICE_MB_PATH / "system/nsamps_multiple_tx", "system/nsamps_multiple_tx", int, int);
    ...
    TREE_CREATE_RO(PV_DEVICE_MB_PATH / "system/num_rx", "system/num_rx", int, int);
    TREE_CREATE_RO(PV_DEVICE_MB_PATH / "system/num_tx", "system/num_tx", int, int);
    ...
    // Create/open channel and streaming lockfiles for all available channels but do not handle locking here
    ...
    for (size_t n = 0; n < num_tx_channels; n++) {
        ...
        try {
            ...
            // If no tx board serial number was found, use the time board instead or log an error message if neither could be used
            if (serial_num.empty() && !timeboard_serial.empty()) {
                serial_num = timeboard_serial;
            } else if (serial_num.empty() && timeboard_serial.empty()) {
            ...
            // has already been initialized using this channel. tx<ch>_<serial>_streaming is locked when the channel is actively streaming.
            ...
            // Create or open the channel lockfile
            int channel_lock_fd = open(channel_lock_path.c_str(), O_CREAT | O_RDONLY, S_IRUSR | S_IRGRP | S_IROTH);
            if(channel_lock_fd == -1) {
                int err = errno;
                throw uhd::runtime_error("Opening channel lockfile failed with error: " + std::string(strerror(err)));
            }
            tx_channel_lock_fd[n] = channel_lock_fd;

            // Create or open the streaming lockfile
            int streaming_lock_fd = open(streaming_lock_path.c_str(), O_CREAT | O_RDONLY, S_IRUSR | S_IRGRP | S_IROTH);
            if(streaming_lock_fd == -1) {
                int err = errno;
                throw uhd::runtime_error("Opening streaming lockfile failed with error: " + std::string(strerror(err)));
            }
            tx_streaming_lock_fd[n] = streaming_lock_fd;
        } catch (uhd::runtime_error &e) {
            // If lockfiles could not be opened/created, continue anyway without checking for locks but log an error so the user is aware
            UHD_LOG_ERROR(CYAN_NRNT_DEBUG_NAME_C, "Could not initialize lock for channel " + channel_name + ".\n" + std::string(e.what()));
        }
    }
    ...
    for (size_t n = 0; n < num_rx_channels; n++) {
        ...
        try {
            ...
            std::string serial_num = rx_serial.substr(0, rx_serial.find('\n'));
            // If no rx board serial number was found, use the time board instead or log an error message if neither could be used
            if (serial_num.empty() && !timeboard_serial.empty()) {
                serial_num = timeboard_serial;
            } else if (serial_num.empty() && timeboard_serial.empty()) {
                throw uhd::runtime_error("Failed to get serial number for rx or time board.");
            }

            // There will be two lockfiles for each channel. rx<ch>_<serial> is an advisory lockfile used to indicate another streamer
            // has already been initialized using this channel. rx<ch>_<serial>_streaming is locked when the channel is actively streaming.
            std::string channel_lock_path = "/tmp/uhd/rx" + channel_name + "_" + serial_num;
            std::string streaming_lock_path = channel_lock_path + "_streaming";

            // Create or open the channel lockfiles
            int channel_lock_fd = open(channel_lock_path.c_str(), O_CREAT | O_RDONLY, S_IRUSR | S_IRGRP | S_IROTH);
            if(channel_lock_fd == -1) {
                int err = errno;
                throw uhd::runtime_error("Opening lockfile failed with error: " + std::string(strerror(err)));
            }
            rx_channel_lock_fd[n] = channel_lock_fd;

            // Create or open the streaming lockfiles
            int streaming_lock_fd = open(streaming_lock_path.c_str(), O_CREAT | O_RDONLY, S_IRUSR | S_IRGRP | S_IROTH);
            if(streaming_lock_fd == -1) {
                int err = errno;
                throw uhd::runtime_error("Opening lockfile failed with error: " + std::string(strerror(err)));
            }
            rx_streaming_lock_fd[n] = streaming_lock_fd;
        } catch (uhd::runtime_error &e) {
            // If lockfiles could not be opened/created, continue anyway without checking for locks but log an error so the user is aware
            UHD_LOG_ERROR(CYAN_NRNT_DEBUG_NAME_C, "Could not initialize lock for channel_" + channel_name + ".\n" + std::string(e.what()));
        }
    }
    ...
    TREE_CREATE_RO(PV_DEVICE_MB_PATH / "system/max_rate", "system/max_rate", double, double);
    max_sample_rate = (_tree->access<double>(PV_DEVICE_MB_PATH / "system/max_rate").get());

    // NOTE: otw_rx is ever made changable during runtime ensure rx_stream_cmd_issuer is updated
    TREE_CREATE_RO(PV_DEVICE_MB_PATH / "system/otw_rx", "system/otw_rx", int, int);
    otw_rx = (_tree->access<int>(PV_DEVICE_MB_PATH / "system/otw_rx").get());
    otw_rx_s = "sc" + std::to_string(otw_rx);

    TREE_CREATE_RO(PV_DEVICE_MB_PATH / "system/otw_tx", "system/otw_tx", int, int);
    otw_tx = (_tree->access<int>(PV_DEVICE_MB_PATH / "system/otw_tx").get());
    otw_tx_s = "sc" + std::to_string(otw_tx);

    TREE_CREATE_RO(PV_DEVICE_MB_PATH / "system/flags/USE_3G_AS_1G", "system/flags/USE_3G_AS_1G", int, int);
    flag_use_3g_as_1g = (_tree->access<int>(PV_DEVICE_MB_PATH / "system/flags/USE_3G_AS_1G").get());
    ...
    _tree->create<std::vector<size_t> >(PV_DEVICE_MB_PATH / "rx_chan_dsp_mapping").set(default_rx_map);
    _tree->create<std::vector<size_t> >(PV_DEVICE_MB_PATH / "tx_chan_dsp_mapping").set(default_tx_map);
    // TODO: make rx_subdev_spec and tx_subdev_spec read only and remove their respective update functions
    _tree->create<subdev_spec_t>(PV_DEVICE_MB_PATH / "rx_subdev_spec").add_coerced_subscriber(std::bind(&cyan_nrnt_impl::update_rx_subdev_spec, this, ph::_1));
    _tree->create<subdev_spec_t>(PV_DEVICE_MB_PATH / "tx_subdev_spec").add_coerced_subscriber(std::bind(&cyan_nrnt_impl::update_tx_subdev_spec, this, ph::_1));
    ...
    TREE_CREATE_RW(PV_DEVICE_MB_PATH / "imgparam/backplane_pinout", "fpga/about/imgparam/backplane_pinout", int, int);
    TREE_CREATE_RW(PV_DEVICE_MB_PATH / "imgparam/ddr_used", "fpga/about/imgparam/ddr_used", int, int);
    TREE_CREATE_RW(PV_DEVICE_MB_PATH / "imgparam/hps_only", "fpga/about/imgparam/hps_only", int, int);
    TREE_CREATE_RW(PV_DEVICE_MB_PATH / "imgparam/num_rx", "fpga/about/imgparam/num_rx", int, int);
    TREE_CREATE_RW(PV_DEVICE_MB_PATH / "imgparam/num_tx", "fpga/about/imgparam/num_tx", int, int);
    TREE_CREATE_RW(PV_DEVICE_MB_PATH / "imgparam/rate", "fpga/about/imgparam/rate", int, int);
    ...
    TREE_CREATE_RW(PV_DEVICE_MB_PATH / "fpga/board/flow_control/sfpc_port", "fpga/board/flow_control/sfpc_port", int, int);
    TREE_CREATE_RW(PV_DEVICE_MB_PATH / "fpga/board/flow_control/sfpd_port", "fpga/board/flow_control/sfpd_port", int, int);
    ...
    TREE_CREATE_RW(PV_DEVICE_MB_PATH / "link" / "sfpc" / "ip_addr",  "fpga/link/sfpc/ip_addr", std::string, string);
    TREE_CREATE_RW(PV_DEVICE_MB_PATH / "link" / "sfpc" / "pay_len", "fpga/link/sfpc/pay_len", int, int);
    TREE_CREATE_RW(PV_DEVICE_MB_PATH / "link" / "sfpd" / "ip_addr",     "fpga/link/sfpd/ip_addr", std::string, string);
    TREE_CREATE_RW(PV_DEVICE_MB_PATH / "link" / "sfpd" / "pay_len", "fpga/link/sfpd/pay_len", int, int);

    // IP and UDP port used by clock sync and stream commands
    // This must use a separate socket since it is expecting a reply
    std::string good_control_ip;
    std::string good_control_port;
    bool found_good_sfp = false;
    ...
    for (int i = 0; i < NUMBER_OF_XG_CONTROL_INTF; i++) {
        ...
        bool iface_good = ping_check("sfp" + xg_intf, control_ip);


        // First pass through the loop: set the IP and port to the first SFP port
        // Later passes: if the previous SFP ports were unresponsive and this one is responsive change to use this SFP's IP and port
        if(i == 0 || (!found_good_sfp && iface_good)) {
            found_good_sfp = iface_good;

            good_control_ip = control_ip;
            good_control_port = control_port;
        }
    }

    // Create the socket used for stream commands
    uhd::transport::udp_simple::sptr rx_stream_cmd_iface = udp_simple::make_connected( good_control_ip, good_control_port );
    ...
    device_clock_sync_info = clock_sync::make(good_control_ip, (uint16_t) std::stoul(good_control_port), CYAN_NRNT_TICK_RATE);
    ...
    TREE_CREATE_ST(PV_DEVICE_MB_PATH / "tick_rate", double, CYAN_NRNT_TICK_RATE);

    // TODO: see if time/clk/cmd is used anywhere, and if not remove it
    TREE_CREATE_RW(PV_DEVICE_TIME_PATH / "cmd", "time/clk/cmd",      time_spec_t, time_spec);
    ...
    TREE_CREATE_RW(PV_DEVICE_TIME_PATH / "now", "time/clk/cur_time", time_spec_t, time_spec);
    ...
    TREE_CREATE_RW(PV_DEVICE_MB_PATH / "time_source"  / "freq",  	    "time/source/freq_mhz",  	int, int);
    ...
    TREE_CREATE_ST(PV_DEVICE_MB_PATH / "clock_source" / "external" / "value", double, CYAN_NRNT_EXT_CLK_RATE);
    ...
    for(size_t ch = 0; ch < num_rx_channels; ch++) {
        ...
        int sfp_port = _tree->access<int>( PV_DEVICE_MB_PATH / "fpga/board/flow_control/sfp" + xg_intf + "_port" ).get();
        std::string time_diff_ip = _tree->access<std::string>( PV_DEVICE_MB_PATH / "link" / "sfp" + xg_intf / "ip_addr" ).get();
...
int cyan_nrnt_impl::get_rx_jesd_num(int channel) {
    const fs_path rx_link_path  = PV_DEVICE_MB_PATH / "rx_link" / channel;
...
std::string cyan_nrnt_impl::get_tx_ip( size_t chan ) {
    ...
    if( is_tx_ip_cached[chan] ) {
        ...
        tx_ip_cache[chan] = _tree->access<std::string>( PV_DEVICE_MB_PATH / "link" / sfp / "ip_addr").get();
...
tune_result_t cyan_nrnt_impl::tune_xx_subdev_and_dsp( const double xx_sign, property_tree::sptr dsp_subtree, property_tree::sptr rf_fe_subtree, const tune_request_t &tune_request, int* gain_is_set, int* last_set_band, size_t chan ) {
    ...
    if(actual_rf_freq == 0 && target_rf_freq != 0) {
        ...
        UHD_LOG_ERROR(CYAN_NRNT_DEBUG_NAME_C, "Error when attempting to set lo on " + xx_sign_s + " channel " + std::string(1, chan + 'A') + ". If this error persists contact support");
    }
...
void cyan_nrnt_impl::set_rx_rate(double rate, size_t chan) {
    if (chan != multi_usrp::ALL_CHANS) {
        ...
        try {
            lock_xx_channel_streaming(chan, RX_DIRECTION);
        } catch (uhd::runtime_error &err) {
            UHD_LOG_ERROR(CYAN_NRNT_DEBUG_NAME_C, "Failed to set rx rate for channel " + std::to_string(chan) + ". Is this channel already being streamed on?");
            throw err;
        } 
        ...
        // Unlock the channel
        flock(rx_streaming_lock_fd[chan], LOCK_UN);
...
void cyan_nrnt_impl::set_tx_rate(double rate, size_t chan) {
    if (chan != multi_usrp::ALL_CHANS) {
        ...
        try {
            lock_xx_channel_streaming(chan, TX_DIRECTION);
        } catch (uhd::runtime_error &err) {
            UHD_LOG_ERROR(CYAN_NRNT_DEBUG_NAME_C, "Failed to set tx rate for channel " + std::to_string(chan) + ". Is this channel already being streamed on?");
            throw err;
        }
