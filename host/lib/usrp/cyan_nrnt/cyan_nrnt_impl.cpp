...
void cyan_nrnt_impl::detect_pps_loop() {
    _pps_thread_running.store(true, std::memory_order_relaxed);

    int pps_detected;

    while (!_pps_thread_should_exit.load(std::memory_order_relaxed)) {
        get_tree()->access<int>(CYAN_NRNT_TIME_PATH / "pps_detected").set(1);
        pps_detected = get_tree()->access<int>(CYAN_NRNT_TIME_PATH / "pps_detected").get();
