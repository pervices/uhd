    ...
    _use_dpdk(use_dpdk)
{
    ...
    for(size_t ch = 0; ch < num_rx_channels; ch++) {
        ...
        rx_stream_cmd_issuer.emplace_back(_time_diff_iface[xg_intf], device_clock_sync_info, jesd_num, otw_rx, (size_t) nsamps_multiple_rx, _sfp_control_mutex[xg_intf]);
    }
}
