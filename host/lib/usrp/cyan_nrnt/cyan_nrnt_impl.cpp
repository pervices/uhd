    ...
    _use_dpdk(use_dpdk)
{
    ...
    try {
        ...
        if (r == -1) {
            ...
            if (err == EWOULDBLOCK) {
                UHD_LOG_ERROR(CYAN_NRNT_DEBUG_NAME_C, "There is already a lock placed for this device: " + lock_path + ". Is another instance of UHD already running?");
            } else {
...
void cyan_nrnt_impl::set_rx_rate(double rate, size_t chan) {
    if (chan != multi_usrp::ALL_CHANS) {
        ...
        try {
            ...
            UHD_LOG_ERROR(CYAN_NRNT_DEBUG_NAME_C, "Failed to set rx rate for channel " + std::to_string(chan));
            throw err;
        } 
...
void cyan_nrnt_impl::set_tx_rate(double rate, size_t chan) {
    if (chan != multi_usrp::ALL_CHANS) {
        ...
        try {
            ...
            UHD_LOG_ERROR(CYAN_NRNT_DEBUG_NAME_C, "Failed to set tx rate for channel " + std::to_string(chan));
            throw err;
        }
