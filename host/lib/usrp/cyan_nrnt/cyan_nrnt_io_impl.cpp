...
void cyan_nrnt_impl::update_tx_samp_rate(const size_t chan, const double rate ){
    ...
    // Get the streamer corresponding to the channel
    std::shared_ptr<pv_device_send_packet_streamer> my_streamer = _mbc.tx_streamers[chan].lock();
    // if shared_ptr is false then no streamer is using this ch
    if (!my_streamer) return;

    // Store specified channel's sample rate
...
rx_streamer::sptr cyan_nrnt_impl::get_rx_stream(const uhd::stream_args_t &args_){
    ...
    std::shared_ptr<sph::recv_packet_streamer_mmsg> my_streamer = std::shared_ptr<sph::recv_packet_streamer_mmsg>(new sph::recv_packet_streamer_mmsg(CYAN_NRNT_DEBUG_NAME_C, args.channels, recv_sockets, dst_ip, data_len, CYAN_NRNT_TRAILER_SIZE, args.cpu_format, args.otw_format, little_endian_supported, rx_channel_in_use, num_rx_channels, _mbc.iface, issuers, rx_channel_lock_fd, rx_streaming_lock_fd));
...
tx_streamer::sptr cyan_nrnt_impl::get_tx_stream(const uhd::stream_args_t &args_){
    ...
    try {
        nsamp_multiple = _tree->access<int>(PV_DEVICE_MB_PATH / "system/nsamps_multiple_tx").get();
    ...
    std::shared_ptr<pv_device_send_packet_streamer> my_streamer = std::shared_ptr<pv_device_send_packet_streamer>(new pv_device_send_packet_streamer(CYAN_NRNT_DEBUG_NAME_C, args.channels, spp, max_buffer_level , dst_ips, dst_ports, (int64_t) (CYAN_NRNT_BUFF_PERCENT * max_buffer_level), nsamp_multiple, CYAN_NRNT_TICK_RATE, CYAN_NRNT_MIN_TX_DELAY, CYAN_NRNT_UPDATE_PER_SEC, _async_msg_fifo, args.cpu_format, args.otw_format, little_endian_supported, tx_channel_in_use, _mbc.iface, device_clock_sync_info, tx_channel_lock_fd, tx_streaming_lock_fd));
