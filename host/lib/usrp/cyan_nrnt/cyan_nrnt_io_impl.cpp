        ...
        if ( _first_call_to_send || metadata.start_of_burst ) {
            ...
            if ( metadata.time_spec.get_real_secs() == 0 || !metadata.has_time_spec ) {
                ...
                if (metadata.time_spec.get_real_secs() < current_time + 0.1) {
                    UHD_LOGGER_WARNING(CYAN_NRNT_DEBUG_NAME_C) << "Requested tx start time of " + std::to_string(metadata.time_spec.get_real_secs()) + " close to current device time of " + std::to_string(current_time) + ". Shifting start time to " + std::to_string(current_time + 0.1);
