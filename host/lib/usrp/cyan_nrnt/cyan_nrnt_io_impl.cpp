...
rx_streamer::sptr cyan_nrnt_impl::get_rx_stream(const uhd::stream_args_t &args_){
    ...
    std::shared_ptr<cyan_nrnt_recv_packet_streamer> my_streamer = std::make_shared<cyan_nrnt_recv_packet_streamer>(args.channels, dst_ip, dst_port, data_len, args.cpu_format, args.otw_format, little_endian_supported, rx_channel_in_use);
...
tx_streamer::sptr cyan_nrnt_impl::get_tx_stream(const uhd::stream_args_t &args_){
    ...
    // TEST: allow sc12 to be requested for experimental FPGA
