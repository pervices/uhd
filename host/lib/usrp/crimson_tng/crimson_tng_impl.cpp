    ...
    _command_time( 0.0 )
{
    ...
    if (not device_addr.has_key("send_buff_size")){
        ...
        device_addr["send_buff_size"] = std::to_string( (size_t) (CRIMSON_TNG_BUFF_SIZE * sizeof( std::complex<int16_t> ) * ((double)(MAX_ETHERNET_MTU+1)/(CRIMSON_TNG_MAX_MTU-CRIMSON_TNG_UDP_OVERHEAD))) );
    }
    ...
    for (size_t n = 0; n < num_tx_channels; n++) {
        ...
        try {
            ...
            UHD_LOG_ERROR(product_name_c, "Could not initialize lock for channel " + channel_name + ".\n" + std::string(e.what()));
        }
    }
    ...
    device_clock_sync_info = clock_sync::make(good_control_ip, (uint16_t) std::stoul(good_control_port), _master_tick_rate);
...
void crimson_tng_impl::set_rx_rate(double rate, size_t chan) {
    if (chan != multi_usrp::ALL_CHANS) {
        ...
        try {
            ...
            UHD_LOG_ERROR(product_name_c, "Failed to set rx rate for channel " + std::to_string(chan) + ". Is this channel already being streamed on?");
            throw err;
        } 
...
void crimson_tng_impl::set_tx_rate(double rate, size_t chan) {
    if (chan != multi_usrp::ALL_CHANS) {
        ...
        try {
            ...
            UHD_LOG_ERROR(product_name_c, "Failed to set tx rate for channel " + std::to_string(chan) + ". Is this channel already being streamed on?");
            throw err;
        } 
...
void crimson_tng_impl::set_time_initiated(int64_t planned_time_s) {
