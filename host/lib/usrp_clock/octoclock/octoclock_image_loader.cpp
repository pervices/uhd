...
static void octoclock_validate_firmware_image(octoclock_session_t& session)
{
    if (not fs::exists(session.image_filepath)) {
        throw uhd::runtime_error(std::format("Could not find image at path \"{}\"",
    ...
    if (extension == ".bin") {
        ...
        throw uhd::runtime_error(
            std::format("Invalid extension \"{}\". Extension must be .hex or .bin.",
                extension));
    ...
    const std::string& filepath)
{
    ...
    if (devs.empty()) {
        ...
        for (const uhd::device_addr_t& dev : devs) {
            std::string name =
                (dev["type"] == "octoclock")
                    ? std::format("OctoClock r{}", dev.get("revision", "4"))
    ...
    if (filepath.empty()) {
        session.image_filepath =
            find_image_path(std::format("octoclock_r{}_fw.hex",
...
static void octoclock_reset_into_bootloader(octoclock_session_t& session)
{
    ...
    if (UHD_OCTOCLOCK_PACKET_MATCHES(RESET_ACK, pkt_out, pkt_in, len)) {
        ...
        uhd::device_addrs_t octoclocks = uhd::device::find(
            uhd::device_addr_t(std::format("addr={}", session.dev_addr["addr"])));
        if (octoclocks.empty()) {
...
static void octoclock_burn(octoclock_session_t& session)
{
    ...
    for (size_t i = 0; i < session.num_blocks; i++) {
        ...
        std::cout << std::format("\r -- Loading firmware: {}%% ({}/{} blocks)",
                static_cast<int>((double(i) / double(session.num_blocks)) * 100),
                i,
                session.num_blocks)
                << std::flush;
    ...
    std::cout << std::format("\r -- Loading firmware: 100%% ({}/{} blocks)\n",
...
static void octoclock_verify(octoclock_session_t& session)
{
    ...
    std::cout << std::format("\r -- Verifying firmware load: 100%% ({}/{} blocks)\n",
            session.num_blocks, session.num_blocks);
}
