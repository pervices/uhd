...
static void load_module(const std::string& file_name)
{
    if (dlopen(file_name.c_str(), RTLD_LAZY) == NULL) {
        throw uhd::os_error(
            str(std::format("dlopen failed to load \"%s\"") % file_name));
    }
}
#endif /* HAVE_DLOPEN */
...
static void load_module(const std::string& file_name)
{
    if (LoadLibrary(file_name.c_str()) == NULL) {
        throw uhd::os_error(
            str(std::format("LoadLibrary failed to load \"%s\"") % file_name));
    }
}
...
static void load_module(const std::string& file_name)
{
    throw uhd::not_implemented_error(str(
        std::format("Module loading not supported: Cannot load \"%s\"") % file_name));
}
...
static void load_module_path(const fs::path& path)
{
    if (not fs::exists(path)) {
        // std::cerr << std::format("Module path \"%s\" not found.") % path.string() <<
        // std::endl;
        return;
    }
    ...
    try {
        ...
        std::cerr << std::format("Error: %s") % err.what() << std::endl;
    }
}
