...
public:
    ...
    void register_endpoint(const sep_addr_t& addr, const sep_id_t& epid) override
    {
        ...
        UHD_LOG_DEBUG(LOG_ID,
            std::format("Bound stream endpoint with Addr=({},{}) to EPID={}",
                addr.first, addr.second, epid));
        UHD_LOG_TRACE(LOG_ID,
            std::format(
                "Stream endpoint with EPID={} can be reached by taking the path: {}",
    ...
    void setup_local_route(chdr_ctrl_xport& xport, const sep_id_t& dst_epid) override
    {
        ...
        UHD_LOG_DEBUG(LOG_ID,
            std::format("Established a route from EPID={} (SW) to EPID={}",
                xport.get_epid(), dst_epid));
        UHD_LOG_TRACE(LOG_ID,
            std::format("The destination for EPID={} has been added to all routers in "
                           "the path: {}",
        ...
        const bool reset = false) override
    {
        ...
        UHD_LOG_DEBUG(
            LOG_ID, (std::format("Initiated RX stream setup for EPID={}", epid)));
    }
        ...
        const bool fc_enabled = true) override
    {
        ...
        UHD_LOG_DEBUG(
            LOG_ID, (std::format("Finished RX stream setup for EPID={}", epid)));
